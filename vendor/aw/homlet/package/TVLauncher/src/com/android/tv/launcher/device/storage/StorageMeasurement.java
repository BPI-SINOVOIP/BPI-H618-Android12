...
public class StorageMeasurement {
    ...
    private class MeasurementHandler extends Handler {
        ...
        private void measureApproximateStorage() {
            ...
            try {
                ...
                StorageManagerVolumeProvider smvp = new StorageManagerVolumeProvider(mStorageManager);
                PrivateStorageInfo info = PrivateStorageInfo.getPrivateStorageInfo(smvp);
                mTotalSize = info.totalBytes;
                mAvailSize = info.freeBytes;
                Log.w(TAG, "TotalSize is " + mTotalSize + " freeTotalSize is " + mAvailSize);
            } catch (Exception e) {
    ...
    private static long getFolderSize(File file){
        ...
        try{
            ...
            for(int i = 0; i < len; i++){
                Log.d(TAG, "file name = " + fileList[i].getName());
    ...
    private static long getDirectorySize(IMediaContainerService imcs, File path) {
        try {
            final long size = getFolderSize(path);
