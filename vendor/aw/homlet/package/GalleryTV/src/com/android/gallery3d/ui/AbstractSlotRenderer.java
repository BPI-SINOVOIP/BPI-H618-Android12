...
public abstract class AbstractSlotRenderer implements SlotView.SlotRenderer {
    ...
    protected AbstractSlotRenderer(Context context) {
        ...
        mGifPlayIcon = new ResourceTexture(context, R.drawable.ic_gallery_play);
    ...
    protected void drawGifOverlay(GLCanvas canvas, int width, int height) {
        // Scale the gif overlay to the height of the thumbnail and put it
        // on the left side.
        ResourceTexture v = mVideoOverlay;
        float scale = (float) height / v.getHeight();
        int w = Math.round(scale * v.getWidth());
        ...
        int s = Math.min(width, height) / 6;
        mGifPlayIcon.draw(canvas, (width - s) / 2, (height - s) / 2, s, s);
    }
