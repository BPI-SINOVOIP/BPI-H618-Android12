...
public class SlotView extends GLView {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event){
        ...
        int row = mLayout.getSlotSpec().rowsLand;
        int col = mLayout.getSlotSpec().rowsPort;
        if(keyCode == KeyEvent.KEYCODE_DPAD_UP){
            if(mCurrSlot % row == (row - 1)){
                mCurrSlot -= 1;
            }
        }else if(keyCode == KeyEvent.KEYCODE_DPAD_DOWN){
            if(mCurrSlot % row == 0){
                if(mLayout.mVisibleEnd - mLayout.mVisibleStart- 1 >= mCurrSlot + 1){
                    mCurrSlot += 1;
                }
            }
        }else if(keyCode == KeyEvent.KEYCODE_DPAD_LEFT){
           if(mCurrSlot % (row * col) < (row * 1)){
               if(mLayout.mVisibleEnd - mLayout.mVisibleStart - 1 >= (mCurrSlot % row + (row * (col - 1)))){
                   int distance = mScroller.startScroll(-col * (mLayout.getSlotWidth() + pading),
                           0, mLayout.getScrollLimit());
                   if(mLayout.mVisibleStart > 0){
                       mCurrSlot = mCurrSlot % row + (row * (col - 1));
                   }
               }
            }
            else{
            ...
            if(mCurrSlot % (row * col) >= (row * (col - 1))){
                int distance = mScroller.startScroll(col * (mLayout.getSlotWidth() + pading),
                        0, mLayout.getScrollLimit());
                if(mLayout.mVisibleEnd < count){
                    mCurrSlot = mCurrSlot % row;
                }
            }
            else{
                if(mLayout.mVisibleEnd - mLayout.mVisibleStart - 1 >= mCurrSlot + row){
                    mCurrSlot += row;
                }
            }
