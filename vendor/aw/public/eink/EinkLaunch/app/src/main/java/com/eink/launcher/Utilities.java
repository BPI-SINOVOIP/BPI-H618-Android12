...
final class Utilities {
    ...
    static Bitmap createIconBitmap(Drawable icon, Context context) {
        synchronized (sCanvas) { // we share the statics :-(
            ...
            icon = makeIcon(context.getResources(), icon);
    ...
    public static Drawable makeIcon(Resources res, Drawable artwork) {

        LayerDrawable layer = null;
        Drawable[] array = new Drawable[2];
        array[1] = new BitmapDrawable(res, drawableToBitmap(artwork));

        Bitmap bmp = BitmapFactory.decodeResource(res,
                R.drawable.app_icon_bg);

        array[0] = new BitmapDrawable(res, bmp);
        layer = new LayerDrawable(array);

        int pad = 8;
        layer.setLayerInset(0, 0, 0, 0, 0);
        layer.setLayerInset(1, pad, pad, pad, pad);
