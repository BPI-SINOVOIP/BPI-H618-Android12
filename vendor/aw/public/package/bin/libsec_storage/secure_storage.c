...
{
    char ctype[PROPERTY_VALUE_MAX];

    memset(ctype, 0, PROPERTY_VALUE_MAX);
    property_get("ro.boot.boot_type",ctype,"-1");
    flash_boot_type = atol(ctype);
    if(flash_boot_type<0){
        ALOGE("Get boot type property:ro.boot.boot_type fail\n");
        return -1 ;
    }
