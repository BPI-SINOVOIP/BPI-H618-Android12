...
{
	char *options, *s, *d, *tmp, *opt, *val, *ret = NULL;
	...
	if (!no_def_opts && ntfs_strappend(&ret, def_opts))
		goto err_exit;
/// AW CODE: fix: ntfs-3g and exfat remove default_permissions mount options
#ifdef ADD_DEFAULT_PERMISSIONS
