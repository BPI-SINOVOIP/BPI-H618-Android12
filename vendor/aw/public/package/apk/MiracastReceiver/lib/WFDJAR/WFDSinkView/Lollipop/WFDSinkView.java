...
////////////////////////////////////////////////////////////////////////////////
    private class MyTSDataSource implements DataSource {
        ...
        public int readAt(long offset, byte[] buffer, int size) {
            if (!isStart) Log.d(TAG, "readAt: offset=" + offset + ", size=" + size + ", valid=" + validDataSize);
            ...
            if (isStart) {
                if (isCache) {
                    ...
                    validBufferSize += (int)offset;
                    validDataSize -= (int)offset;
                    isCache = false;
                }

                resync(); // resync the sync byte 0x47 for ATSParser
                ...
                if (cache[readPtr] != 0x47) Log.d(TAG, "readAt not a sync byte " + Integer.toHexString(cache[readPtr])); // TODO

                ret = size;
                int leftDataSize = totalBufferSize - readPtr;
                if (leftDataSize >= ret) {
                    System.arraycopy(cache, readPtr, buffer, 0, ret);
                } else {
                    System.arraycopy(cache, readPtr, buffer, 0, leftDataSize);
                    System.arraycopy(cache, 0, buffer, leftDataSize, ret - leftDataSize);
                }
                ...
                isCache = true;
                if (totalBufferSize < (readPtr + (int)offset + size)) return -1;

                if (offset % 188 != 0 || size % 188 != 0) { // hack mpeg2ts found
                    if (offset % 188 == 0 && size == 1) { // hack mpeg2ts sniff
                        buffer[0] = 0x47;
                        ret = size;
                    } else {
                        ret = 0;
                    ...
                    if (offset == 0) resync(); // resync the sync byte 0x47 for ATSParser

                    while (validDataSize < (int)offset + size) { // important
                        //Log.d(TAG, "readAt underflow " + size + " vs " + validDataSize);
                        if (cache == null || totalBufferSize <= 0)
                            return -1;
                        try {
                    ...
                    ret = size;
                    System.arraycopy(cache, readPtr + (int)offset, buffer, 0, ret);
                }
            }
