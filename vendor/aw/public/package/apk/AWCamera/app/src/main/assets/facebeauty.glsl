#extension GL_OES_EGL_image_external : require
 ...
 void main(){
  ...
  float r, g, b;
  // Eyes reshape
  positionToUse = warpScaleToUse(rightEyeCenterPosition, positionToUse, radius, scaleRatio, aspectRatio);
  positionToUse = warpScaleToUse(leftEyeCenterPosition, positionToUse, radius, scaleRatio, aspectRatio);
...
// }
        ...
        // Mesh Warp
//        positionToUse = reposition(positionToUse,rightEyeCenterPosition,leftEyeCenterPosition);

          vec4 color = texture2D(texture, positionToUse);
          r = color.r;
          g = color.g;
          b = color.b;
          r = clamp(r, 0.0, 1.0);
...
/*if((abs(rightEyeCenterPosition.x -vTextureCoord.x)<0.03) &&(abs(rightEyeCenterPosition.y -vTextureCoord.y) <=0.03) ){
            ...
            b = 0.0;
                   ...
                   if((abs(0.0 -vTextureCoord.x)<0.03) &&(abs(0.0 -vTextureCoord.y) <=0.03) ){
                    r=1.0;
                    g =0.0;
                    b = 0.0;
                    }
                    if((abs(0.0 -vTextureCoord.x)<0.03) &&(abs(1.0 -vTextureCoord.y) <=0.03) ){
                              r=1.0;
                              g =1.0;
                              b = 0.0;
                              }
                       if((abs(1.0 -vTextureCoord.x)<0.03) &&(abs(0.0 -vTextureCoord.y) <=0.03) ){
                                         r=0.0;
                                         g =1.0;
                                         b = 0.0;
                                         }
                      if((abs(1.0 -vTextureCoord.x)<0.03) &&(abs(1.0 -vTextureCoord.y) <=0.03) ){
                                        r=0.0;
                                        g =0.0;
                                        b = 1.0;
                                        }*/
