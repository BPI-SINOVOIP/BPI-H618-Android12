...
#define LOGP(...)   __android_log_print(ANDROID_LOG_DEBUG, TAG, __VA_ARGS__ )
...
/** working resolution **/
    ...
    double compose_megapix = 0.7; // -1 means ignore
    ...
    **/
    ...
    LOGD("stitchImg params2: %s", s.c_str());
...
//    }
    ...
    LOGD("All images2: %d, MODE:%s, detector_type=%s, wrap_type=%s, seam_find_type=%s, expos_comp_type=%s, ORB_FEATURES_N=%d, ORB_GRID_SIZE=%d%d",
...
//add   matcher(features, pairwise_matches, _matchingMask);
...
//add   _indices = leaveBiggestComponent(features, pairwise_matches, conf_thresh);
...
//add    compensator->feed(corners, images_warped, masks_warped);
...
/*    if (seam_find_type == "no")
    ...
    for (int img_idx = 0; img_idx < img_amount; ++img_idx) {
        ...
        LOGD("Compositing, time6: %f%s", ((getTickCount() - t) / getTickFrequency()), " sec");
        ...
        mask.release();
        LOGD("Compositing, time5: %f%s", ((getTickCount() - t) / getTickFrequency()), " sec");
        // Compensate exposure
        compensator->apply(img_idx, corners[img_idx], img_warped, mask_warped);

        img_warped.convertTo(img_warped_s, CV_16S);
        img_warped.release();
        LOGD("Compositing, time4: %f%s", ((getTickCount() - t) / getTickFrequency()), " sec");
        dilate(masks_warped[img_idx], dilated_mask, Mat());
        resize(dilated_mask, seam_mask, mask_warped.size());
        mask_warped = seam_mask & mask_warped;
        LOGD("Compositing, time3: %f%s", ((getTickCount() - t) / getTickFrequency()), " sec");
        if (!blender) {
            blender = Blender::createDefault(blend_type, true);
            Size dst_sz;
//add            Size dst_sz = resultRoi(corners, sizes).size();
           ...
           /* if (blend_width < 1.f)
            ...
            fb->setSharpness(1.f / blend_width);
            LOGD("Feather blender, sharpness: %f", ((double) fb->sharpness()));
//add            blender->prepare(corners, sizes);
        }
        LOGD("Compositing, time2: %f%s", ((getTickCount() - t) / getTickFrequency()), " sec");
    ...
    LOGD("Compositing, time1111: %f%s %d", ((getTickCount() - t) / getTickFrequency()), " sec",result.type());
    Mat result_mask;
    blender->blend(result, result_mask);
    LOGD("Compositing, time11112222: %f%s %d", ((getTickCount() - t) / getTickFrequency()), " sec",result.type());
