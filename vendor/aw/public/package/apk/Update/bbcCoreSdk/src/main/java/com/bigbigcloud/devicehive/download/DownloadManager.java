...
public final class DownloadManager {
        ...
        try {
            ...
            if (infoList != null) {
                for (DownloadInfo info : infoList) {
                    if (info.getState() == DownloadState.WAITING || info.getState() == DownloadState.STARTED) {
    ...
    public synchronized DownloadInfo startDownload(String url, String savePath,
                                           boolean autoResume, boolean autoRename,
                                           DownloadStatusListener listener) throws DbException {
        ...
        if (downloadInfo != null) {
            ...
            if (callback != null) {
                if (callback.switchListner(listener)) {
                    return downloadInfo;
                } else {
                    callback.cancel();
                }
        ...
        DownloadCallback callback = new DownloadCallback(listener, downloadInfo);
    ...
    public void autoPause(String url, String savePath, boolean autoResume, boolean autoRename, DownloadStatusListener listener) throws DbException {
        DownloadInfo downloadInfo = db.selector(DownloadInfo.class)
                .where("url", "=", url)
                .findFirst();
        if (downloadInfo == null || downloadInfo.getState() == DownloadState.STOPPED || downloadInfo.getState() == DownloadState.WAITING) {
            String fileSavePath = new File(savePath).getAbsolutePath();
            // create download info
            if (downloadInfo == null) {
                downloadInfo = new DownloadInfo();
            ...
            DownloadCallback dc = callbackMap.get(downloadInfo);
            dc.cancel();
        }
    }
