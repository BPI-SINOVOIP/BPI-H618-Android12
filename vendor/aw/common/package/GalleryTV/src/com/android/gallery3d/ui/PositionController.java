...
class PositionController {
    ...
    public synchronized void forceImageSize(int index, Size s) {
    ...
    private synchronized boolean setBoxSize(int i, int width, int height, boolean isViewSize) {
        Box b = mBoxes.get(i);
        ...
        if ((wasViewSize && !isViewSize) || !mFilmMode) {
            b.mCurrentScale = getSuitableScaleWhenInit(b);
    ...
    private synchronized boolean startOpeningAnimationIfNeeded() {
        ...
        startAnimation(mPlatform.mDefaultX, 0, b.mCurrentScale,
                ANIM_KIND_OPENING);
    ...
    private synchronized void updateScaleAndGapLimit() {
        for (int i = -BOX_MAX; i <= BOX_MAX; i++) {
            Box b = mBoxes.get(i);
    ...
    private synchronized int getDefaultGapSize(int i) {
    ...
    public synchronized void zoomIn(float tapX, float tapY, float targetScale) {
        tapX -= mViewW / 2;
        tapY -= mViewH / 2;
        Box b = mBoxes.get(0);
    ...
    public synchronized void resetToFullView() {
        Box b = mBoxes.get(0);
        startAnimation(mPlatform.mDefaultX, 0, b.mScaleMin, ANIM_KIND_ZOOM);
    }

    public synchronized void beginScale(float focusX, float focusY) {
        focusX -= mViewW / 2;
        focusY -= mViewH / 2;
        Box b = mBoxes.get(0);
    ...
    public synchronized int scaleBy(float s, float focusX, float focusY) {
        focusX -= mViewW / 2;
        focusY -= mViewH / 2;
        Box b = mBoxes.get(0);
        Platform p = mPlatform;
    ...
    public synchronized void startHorizontalSlide() {
        Box b = mBoxes.get(0);
        startAnimation(mPlatform.mDefaultX, 0, getSuitableScaleWhenInit(b), ANIM_KIND_SLIDE);
    }
    ...
    public synchronized void startCaptureAnimationSlide(int offset) {
        Box b = mBoxes.get(0);
    ...
    public synchronized void scrollPage(int dx, int dy) {
        if (!canScroll()) return;
        Box b = mBoxes.get(0);
        Platform p = mPlatform;
    ...
    public synchronized void scrollFilmX(int dx) {
        if (!canScroll()) return;
    ...
    public synchronized void scrollFilmY(int boxIndex, int dy) {
        if (!canScroll()) return;
    ...
    public synchronized boolean flingPage(int velocityX, int velocityY) {
        Box b = mBoxes.get(0);
        Platform p = mPlatform;
    ...
    public synchronized boolean flingFilmX(int velocityX) {
    ...
    public synchronized int flingFilmY(int boxIndex, int velocityY) {
        Box b = mBoxes.get(boxIndex);
    ...
    public synchronized int hitTest(int x, int y) {
    ...
    public synchronized boolean inOpeningAnimation() {
    ...
    private synchronized void convertBoxToRect(int i) {
        Box b = mBoxes.get(i);
        Rect r = mRects.get(i);
    ...
    private synchronized void initBox(int index) {
        Box b = mBoxes.get(index);
        b.mImageW = mViewW;
        b.mImageH = mViewH;
        b.mUseViewSize = true;
    ...
    private synchronized void initGap(int index) {
    ...
    private synchronized void initGap(int index, int size) {
        Gap g = mGaps.get(index);
        g.mDefaultSize = getDefaultGapSize(index);
        g.mCurrentGap = size;
        g.mAnimationStartTime = NO_ANIMATION;
    ...
    public synchronized boolean isAtMinimalScale() {
        Box b = mBoxes.get(0);
    ...
    public synchronized float getImageScale() {
        Box b = mBoxes.get(0);
        return b.mCurrentScale;
    }

    public synchronized int getImageAtEdges() {
        Box b = mBoxes.get(0);
        Platform p = mPlatform;
        calculateStableBound(b.mCurrentScale);
        int edges = 0;
    ...
    public synchronized boolean hasDeletingBox() {
        for(int i = -BOX_MAX; i <= BOX_MAX; i++) {
    ...
    private synchronized float getSuitableScaleWhenInit(Box b){
        float wFactor = 1.0f;
        float hFactor = 1.0f;
        int viewW, viewH;
    ...
    private float getMinimalScale(Box b) {
        return SCALE_LIMIT_MIN * getSuitableScaleWhenInit(b);
    }
    ...
    private synchronized boolean viewTallerThanScaledImage(float scale) {
        return mViewH >= heightOf(mBoxes.get(0), scale);
    }

    private synchronized boolean viewWiderThanScaledImage(float scale) {
    ...
    private class Platform extends Animatable {
        ...
        @Override
        public synchronized boolean startSnapback() {
