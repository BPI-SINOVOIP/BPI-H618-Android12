...
protected:
    virtual int handleResponse(WifiEvent& reply) {
        ...
        if (vendor_data == NULL || len == 0) {
            ALOGV("no vendor data in GetChannelList response; ignoring it");
            return NL_SKIP;
        }
...
static int parseScanResults(wifi_scan_result *results, int num, nlattr *attr)
{
    ...
    if (i >= num) {
        ALOGV("Got too many results; skipping some");
    }
...
public:
    ...
    int start() {
        ...
        if (result != WIFI_SUCCESS) {
            ALOGV("failed to enable full scan results; result = %d", result);
    ...
    virtual int cancel() {
        ...
        if (result != WIFI_SUCCESS) {
            ...
            result = requestResponse(request);
            if (result != WIFI_SUCCESS) {
                ALOGV("failed to disable full scan results;result = %d", result);
            }
        }
...
public:
    ...
    int start() {
        ...
        result = requestResponse(request);
        if (result != WIFI_SUCCESS) {
            ALOGV("failed to configure setup; result = %d", result);
            return result;
        }
        ...
        if (result != WIFI_SUCCESS) {
            ALOGV("failed to create scan config request; result = %d", result);
            return result;
        }

        result = requestResponse(request);
        if (result != WIFI_SUCCESS) {
            ALOGV("failed to configure scan; result = %d", result);
            return result;
        }
        ...
        if (result != WIFI_SUCCESS) {
            ALOGV("failed to create start request; result = %d", result);
            return result;
        }
        ...
        result = requestResponse(request);
        if (result != WIFI_SUCCESS) {
            ALOGV("failed to start scan; result = %d", result);
        ...
        wifi_scan_result_handler handler)
{
    ...
    if ((ie_len + offsetof(wifi_gscan_full_result_t, ie_data)) > len) {
        ALOGV("BAD event data, len %d ie_len %d fixed length %d!\n", len,
    ...
    if (!full_scan_result) {
        ALOGV("Full scan results: Can't malloc!\n");
        return NL_SKIP;
    }
...
public:
    ...
    int execute() {
        ...
        for (int i = 0; i < 10 && mRetrieved < mMax; i++) {
            ...
            if (result < 0) {
                ALOGV("failed to create request");
                return result;
            }
            ...
            if (result != WIFI_SUCCESS) {
                ALOGV("failed to retrieve scan results; result = %d", result);
                return result;
            }
    ...
    virtual int handleResponse(WifiEvent& reply) {
        ...
        if (subcmd != GSCAN_SUBCMD_SCAN_RESULTS) {
            ALOGV("Invalid response to GetScanResultsCommand; ignoring it");
            return NL_SKIP;
        }
        */
        ...
        if (vendor_data == NULL || len == 0) {
            ALOGV("no vendor data in GetScanResults response; ignoring it");
            return NL_SKIP;
        }
...
public:
    ...
    int createSetupRequest(WifiRequest& request) {
        if (epno_params.num_networks > MAX_EPNO_NETWORKS) {
            ALOGV("wrong epno num_networks:%d", epno_params.num_networks);
...
public:
    ...
    int start() {
        ...
        result = requestResponse(request);
        if (result != WIFI_SUCCESS) {
            ALOGV("failed to set ANQPO networks; result = %d", result);
    ...
    virtual int cancel() {
        ...
        if (result != WIFI_SUCCESS) {
            ...
            result = requestResponse(request);
            if (result != WIFI_SUCCESS) {
                ALOGV("failed to reset ANQPO networks;result = %d", result);
            }
        }
