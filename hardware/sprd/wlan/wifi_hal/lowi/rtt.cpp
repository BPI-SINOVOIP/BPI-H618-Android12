...
typedef enum {
    ...
    RTT_ATTRIBUTE_TARGET_RESPONDER_INFO,
...
public:
    GetRttCapabilitiesCommand(lowi_wifi_interface_handle iface, wifi_rtt_capabilities *capabitlites)
    ...
    virtual int create() {
        ...
        int ret = mMsg.create(SPRD_OUI, RTT_SUBCMD_GETCAPABILITY);
        if (ret < 0) {
            return ret;
        }
...
protected:
    virtual int handleResponse(WifiEvent& reply) {
        ...
        ALOGD("Id = %0x, subcmd = %d, len = %d, expected len = %zu", id, subcmd, len,
...
public:
    GetRttResponderInfoCommand(lowi_wifi_interface_handle iface, wifi_rtt_responder *responderInfo)
    ...
    virtual int create() {
        ...
        int ret = mMsg.create(SPRD_OUI, RTT_SUBCMD_GETAVAILCHANNEL);
        if (ret < 0) {
            return ret;
        }
...
class EnableResponderCommand : public WifiCommand
{
    ...
    wifi_rtt_responder* mResponderInfo;
public:
    EnableResponderCommand(lowi_wifi_interface_handle iface, int id, wifi_channel_info channel_hint,
    ...
    virtual int create() {
        ...
        int ret = mMsg.create(SPRD_OUI, RTT_SUBCMD_SET_RESPONDER);
        if (ret < 0) {
            return ret;
        }
        nlattr *data = mMsg.attr_start(NL80211_ATTR_VENDOR_DATA);
        ALOGI("add channel info");
        ret = mMsg.put(RTT_ATTRIBUTE_TARGET_CHAN, &mChannelInfo,
                    sizeof(wifi_channel_info));
        if (ret < 0) {
            return ret;
        }
        ALOGI("add m_max_duration_sec: %d", m_max_duration_sec);
        ret = mMsg.put_u32(RTT_ATTRIBUTE_TARGET_PERIOD, m_max_duration_sec);
        if (ret < 0) {
            return ret;
        }
        ALOGI("add responderInfo 0x%p", mResponderInfo);

        ret = mMsg.put(RTT_ATTRIBUTE_TARGET_RESPONDER_INFO, mResponderInfo,
                    sizeof(wifi_rtt_responder));
        if (ret < 0) {
            return ret;
        }
        mMsg.attr_end(data);
...
public:
    CancelResponderCommand(lowi_wifi_interface_handle iface, int id)
    ...
    virtual int create() {
        ...
        int ret = mMsg.create(SPRD_OUI, RTT_SUBCMD_CANCEL_RESPONDER);
        if (ret < 0) {
            return ret;
        }
...
class RttCommand : public WifiCommand
{
    ...
    wifi_lci_information *rttLciInfo;
    wifi_lcr_information *rttLcrInfo;
public:
    RttCommand(lowi_wifi_interface_handle iface, int id, unsigned num_rtt_config,
    ...
    RttCommand(lowi_wifi_interface_handle iface, int id)
    ...
    /* for lci setting - only in responder mode */
    RttCommand(lowi_wifi_interface_handle iface, int id, wifi_lci_information *lci)
        : WifiCommand("RttCommand", iface, id), rttLciInfo(lci)
    {
        currentIdx = 0;
        mCompleted = 0;
        totalCnt = 0;
    ...
    /* for lcr setting - only in responder mode */
    RttCommand(lowi_wifi_interface_handle iface, int id, wifi_lcr_information *lcr)
        : WifiCommand("RttCommand", iface, id), rttLcrInfo(lcr)
    {
        currentIdx = 0;
        mCompleted = 0;
        totalCnt = 0;
    ...
    int createSetupRequest(WifiRequest& request) {
        int result = request.create(SPRD_OUI, RTT_SUBCMD_SET_CONFIG);
        if (result < 0) {
            return result;
        }
    ...
    int createLciSetupRequest(WifiRequest& request) {
        int result = request.create(SPRD_OUI, RTT_SUBCMD_SET_LCI);
        if (result < 0) {
            return result;
        }
        ...
        result = request.put(RTT_ATTRIBUTE_TARGET_LCI, rttLciInfo,
				sizeof(wifi_lci_information));
        if (result < 0) {
            return result;
        }
    ...
    int createLcrSetupRequest(WifiRequest& request) {
        int result = request.create(SPRD_OUI, RTT_SUBCMD_SET_LCR);
        if (result < 0) {
            return result;
        }
        ...
        result = request.put(RTT_ATTRIBUTE_TARGET_LCR, rttLcrInfo,
				sizeof(wifi_lcr_information));
        if (result < 0) {
            return result;
        }
    ...
    int setLci() {
        ALOGD("Setting RTT LCI");
        WifiRequest request(familyId(), ifaceId());
        int result = createLciSetupRequest(request);
        if (result != WIFI_SUCCESS) {
            ALOGE("failed to create LCI setup request; result = %d", result);
            return result;
        }

        result = requestResponse(request);
        if (result != WIFI_SUCCESS) {
            ALOGE("failed to configure RTT LCI; result = %d", result);
            return result;
        }

        ALOGI("Successfully set RTT LCI operation");
        return result;
    }


    int setLcr() {
        ALOGD("Setting RTT LCR");
        WifiRequest request(familyId(), ifaceId());
        int result = createLcrSetupRequest(request);
        if (result != WIFI_SUCCESS) {
            ALOGE("failed to create LCR setup request; result = %d", result);
            return result;
        }

        result = requestResponse(request);
        if (result != WIFI_SUCCESS) {
            ALOGE("failed to configure RTT LCR; result = %d", result);
            return result;
        }

        ALOGI("Successfully set RTT LCR operation");
        return result;
    }


    int createTeardownRequest(WifiRequest& request, unsigned num_devices, mac_addr addr[]) {
        int result = request.create(SPRD_OUI, RTT_SUBCMD_CANCEL_CONFIG);
        if (result < 0) {
            return result;
        }
    ...
    int start() {
        ...
        registerVendorHandler(SPRD_OUI, RTT_EVENT_COMPLETE);
    ...
    virtual int cancel() {
        ...
        unregisterVendorHandler(SPRD_OUI, RTT_EVENT_COMPLETE);
        return WIFI_SUCCESS;
    }
                ...
                for (nl_iterator it2(it.get()); it2.has_next(); it2.next()) {
                    if (it2.get_type() == RTT_ATTRIBUTE_TARGET_MAC) {
                        ...
                        ALOGI("retrived rtt_result : \n\tburst_num :%d, measurement_number : %d, success_number : %d\n"
                                ...
                                "\trssi : %d dbm, rx_rate : %d Kbps, rtt : %lld ns, rtt_sd : %lld\n"
...
{
    lowi_wifi_handle handle = getWifiHandle(iface);
...
{
    RttCommand *cmd = new RttCommand(iface, id, lci);
    NULL_CHECK_RETURN(cmd, "memory allocation failure", WIFI_ERROR_OUT_OF_MEMORY);
    cmd->setLci();
    cmd->releaseRef();
    return WIFI_SUCCESS;
...
{
    RttCommand *cmd = new RttCommand(iface, id, lcr);
    NULL_CHECK_RETURN(cmd, "memory allocation failure", WIFI_ERROR_OUT_OF_MEMORY);
    cmd->setLcr();
    cmd->releaseRef();
    return WIFI_SUCCESS;
