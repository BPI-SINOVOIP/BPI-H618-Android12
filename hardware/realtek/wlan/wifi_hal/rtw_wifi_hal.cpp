...
wifi_error wifi_wait_for_driver_ready(void)
{
    DIR *d;
    struct dirent *de;
    uint32_t interface  = 0, count;
    uint32_t MAX_WAIT_COUNT = 20, PER_WAIT_US = 200000;

    for (count = 0; count < MAX_WAIT_COUNT; count++) {
        if (!(d = opendir("/sys/class/net"))) {
            ALOGW("Failed to fopen file: /proc/net/dev");
            return WIFI_ERROR_NOT_AVAILABLE;
        }

        while ((de = readdir(d))) {
            if (strstr(de->d_name, "wlan0") || strstr(de->d_name, "p2p0"))
                interface++;
            if (interface == 2) {
                ALOGD("Register net device wlan0/p2p0 success, time: %d ms.", count * PER_WAIT_US / 1000);
                closedir(d);
                return WIFI_SUCCESS;
            }
        }
    ...
    ALOGE("Timeout, register net device wlan0/p2p0 failed.");
