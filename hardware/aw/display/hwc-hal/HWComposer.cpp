...
extern std::shared_ptr<IDeviceFactory> createDeviceFactory();
...
int HWComposer::initializeDevices()
{
    ...
    WriteBackManager::getWbInstance()->setDevicePool(mDevicePool);
#endif
    return 0;
}
...
void HWComposer::terminateDevices()
{
    ...
    if (mDisplays.size() > 0) {
        DLOGW("maybe surfaceFlinger crash!!!");
        std::lock_guard<std::mutex> lock(mDisplayMutex);
        // blank all displays
...
void HWComposer::onHotplugReceived(int32_t handle, bool connected)
{
    ...
    if (connected) {
        ...
        DLOGW("display(%lld) is %p", id, display.get());
        if (id == WBID)
...
#ifdef WRITE_BACK_MODE
    WriteBackManager::getWbInstance()->setDisplays(mDisplays);
    if (id == WBID) {
        return;
    }
#endif
