...
public:
    ...
    virtual int32_t getHwIdx(int32_t logicId) = 0;
};
...
public:
   ...
   ~DeviceManager();
    ...
    void onPerValidateDisplay(int32_t id);

    class EventQueue;

private:
    void deviceChangedEventProcess(std::vector<ConnectedInfo>& cinfo);
    void populateDeviceHub(std::unique_lock<std::mutex>& lock, std::vector<ConnectedInfo>& cinfo);
    void threadLoop();
