...
void DeviceManager::deviceChangedEventProcess(std::vector<ConnectedInfo>& cinfo)
{
    ...
    WriteBackManager::getWbInstance()->setHotplugState(false);
#endif
}

void DeviceManager::populateDeviceHub(std::unique_lock<std::mutex>& lock,
        std::vector<ConnectedInfo>& cinfo)
{
        ...
        if (mDeviceHubs.count(displayId)) {
            ...
            if (hub->getDevice() != info.device) {
                DLOGW("Display.%d device changed: %s ==> %s",
                        ...
                        info.device ? info.device->name() : "null");
                hub->onDeviceChanged(info.device);
                mSwapPendingFlag = true;
            }
        }
    }

    // This populateDeviceHub is called from DeviceManager's EventThread,
    // We block here to wait the composer main thread to update the device
    if (mSwapPendingFlag) {
        mCondition.wait(lock, [this]{ return mSwapPendingFlag == false; });
    }
}

void DeviceManager::onPerValidateDisplay(int32_t id)
{
    std::unique_lock<std::mutex> lock(mDisplayLock);
    if (mSwapPendingFlag && id == HWC_DISPLAY_PRIMARY) {

        for (auto item: mDeviceHubs) {
            std::shared_ptr<DeviceHub>& hub = item.second;
            hub->populateDevice();
        }
        mSwapPendingFlag = false;
        mCondition.notify_all();
    }
}
