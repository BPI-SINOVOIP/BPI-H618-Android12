...
HWC2::Error Display::getHdrCapabilities(uint32_t *num_types, int32_t *types,
            ...
            float * /* min_luminance */) {
    if (!mActiveConfig.isSupportHdr) {
        DLOGW("%s: not hdr supported", __func__);
        *num_types = 0;
        return HWC2::Error::None;
    } else {
        if (types == nullptr) {
        ...
        types[0] = (int32_t)Hdr::HDR10;
        types[1] = (int32_t)Hdr::HLG;
        //outTypes[2] = Hdr::DOLBY_VISION;
        *num_types = 2;
        DLOGD("%s: support hdr10 and hlg", __func__);
        return HWC2::Error::None;
    }
}
    ...
    for (auto& layer : mLayerStack) {
        if (layers && fences) {
            if (num_layers < *num_elements) {
                ...
                if (mFenceUseless && fences[num_layers] > 0) {
                    close(fences[num_layers]);
                    fences[num_layers] = -1;
                }
            } else {
...
HWC2::Error Display::validateDisplay(uint32_t *num_types, uint32_t *num_requests)
{
    ...
    *num_requests = 0;
    ...
    if (mValidMode == ValidMode::BYPASS) {
        return HWC2::Error::None;
    }
...
HWC2::Error Display::presentDisplay(int32_t *retire_fence)
{
    ...
    *retire_fence = mRetireFence.release();
    ...
    ++mFrameNumber;
OUT:
    if (mFenceUseless && *retire_fence > 0) {
        close(*retire_fence);
        *retire_fence = -1;
    }
    return HWC2::Error::None;
...
CompositionContext* Display::createComposition()
{
    ...
    if (mValidMode == ValidMode::WRITEBACK) {
        ctx->InputLayers.insert(mWriteBackLyr);
    } else {
    ...
    // Check whether downstream display device needs to be update/swap
    mDeviceManager->onPerValidateDisplay(mHandle);
