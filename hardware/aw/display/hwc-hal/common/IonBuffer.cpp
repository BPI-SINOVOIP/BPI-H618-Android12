...
int IonBuffer::initWithSize(uint32_t inSize, uint64_t inUsage)
{
    ...
    if (inUsage & GRALLOC_USAGE_PROTECTED) {
        // try to allocate secure buffer on sunxi_drm_heap first!
        if (DrmHeap::isValid()) {
            buf = DrmHeap::allocate(inSize, 0);
        }
        if (buf < 0) {
            ret = ion_alloc_fd(iondev::fd(), inSize,
                    DEF_ION_BUF_ALIGN, ION_HEAP_SECURE_MASK, 0, &buf);
            if (ret != 0) {
    ...
    if (!physicalContiguous) {
        ...
        ret = ion_alloc_fd(iondev::fd(), inSize,
                DEF_ION_BUF_ALIGN, ION_HEAP_SYSTEM_MASK, 0, &buf);
    }

    if (buf == -1) {
        ret = ion_alloc_fd(iondev::fd(), inSize,
                DEF_ION_BUF_ALIGN, ION_HEAP_TYPE_DMA_MASK, 0, &buf);

        if (ret != 0) {
            ret = ion_alloc_fd(iondev::fd(), inSize,
                    DEF_ION_BUF_ALIGN, ION_HEAP_SYSTEM_CONTIG_MASK, 0, &buf);
        }
    }
    if (buf == -1) {
