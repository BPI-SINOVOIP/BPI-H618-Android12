...
/* initialize GL */
void* engine_initialize(bool isSecure, int width, int height)
{
#if 1
    ...
    /* initialize */
    EGL(eglInitialize(engineContext->eglDisplay, &majorVersion, &minorVersion));
    ALOGD("engine_initialize [%d, %d]", majorVersion, minorVersion);
    ...
    EGLint eglConfigAttribList[] = {
        ...
        EGL_NONE};
    int numConfig = 0;
    EGL(eglChooseConfig(engineContext->eglDisplay, eglConfigAttribList, &eglConfig,
                        1, &numConfig));

    ALOGD("%s, isSecure=%d", __func__, isSecure);
    /* context */
    EGLint eglContextAttribList[] = {EGL_CONTEXT_CLIENT_VERSION, 2,
        isSecure ? EGL_PROTECTED_CONTENT_EXT : EGL_NONE,
        isSecure ? EGL_TRUE : EGL_NONE,
        EGL_NONE};
    engineContext->eglContext = eglCreateContext(engineContext->eglDisplay, eglConfig,
                                                 NULL, eglContextAttribList);

    /* surface */
    EGLint eglSurfaceAttribList[] = {EGL_WIDTH, width,
        EGL_HEIGHT, height,
        isSecure ? EGL_PROTECTED_CONTENT_EXT : EGL_NONE,
        isSecure ? EGL_TRUE : EGL_NONE,
        EGL_NONE};
    engineContext->eglSurface = eglCreatePbufferSurface(engineContext->eglDisplay,
                                                        eglConfig, eglSurfaceAttribList);

    eglMakeCurrent(engineContext->eglDisplay, engineContext->eglSurface,
                   engineContext->eglSurface, engineContext->eglContext);

    ALOGD("In %s context = %p", __FUNCTION__, (void *)(engineContext->eglContext));
...
EGL_NONE };*/
    ...
    ALOGD("%s, isSecure=%d", __func__, isSecure);
    engineContext->eglDisplay = eglGetDisplay(EGL_DEFAULT_DISPLAY);
    if (engineContext->eglDisplay == EGL_NO_DISPLAY) {
...
#if 1
    EGL(eglChooseConfig(engineContext->eglDisplay, config_attribute_list, 0, 0, &numConfigs));
    if (numConfigs) {
        EGLConfig* const configs = new EGLConfig[numConfigs];
        eglChooseConfig(engineContext->eglDisplay, config_attribute_list, configs,
                        numConfigs, &n);
        myConfig = configs[0];
        delete[] configs;
    }
#else
    eglChooseConfig(engineContext->eglDisplay, s_configAttribs, &myConfig, 1,
                    &numConfigs);
    printf("numConfigs=%d\n",numConfigs);
#endif

    engineContext->eglSurface = eglCreateWindowSurface(engineContext->eglDisplay, myConfig,
                                                       (EGLNativeWindowType)&native_window,
                                                       window_attribute_list);
    if (engineContext->eglSurface == EGL_NO_SURFACE) {
        ALOGD("gelCreateWindowSurface failed.\n");
        return NULL;
    }

    engineContext->eglContext = eglCreateContext(engineContext->eglDisplay, myConfig,
                                                 EGL_NO_CONTEXT, context_attribs);
    if (engineContext->eglContext == EGL_NO_CONTEXT) {
...
{
    EngineContext* engineContext = (EngineContext*)context;
    EGL(eglMakeCurrent(engineContext->eglDisplay,
                       EGL_NO_SURFACE,
                       EGL_NO_SURFACE,
                       EGL_NO_CONTEXT));
...
unsigned int engine_load1DTexture(void *data, int sz, int format)
{
    ...
    if ((data != 0) && (sz != 0)) {
        ...
        GL(glTexImage2D(target, 0, GL_RGB10_A2, sz, 1, 0, GL_RGBA,
...
void engine_set2DOutputBuffer(int binding, unsigned int id)
{
    GL(glActiveTexture(GL_TEXTURE1 + binding));
