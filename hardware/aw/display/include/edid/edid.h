...
#define EDID_MAX_LENGTH (BLOCK_LENGTH * EDID_MAX_BLOCK_COUNT)
...
private:
    ...
    void vendorSpecificVideoDataBlock(const unsigned char *x);
    ...
    void HfEeodbDataBlock(const unsigned char *x);
    ...
    void hdr10plusVSDB(const unsigned char *x);

    const char *mPath;
    unsigned char edid[EDID_MAX_LENGTH];
    ...
    std::unordered_map<int, videoFormat *> mSupportedFormat;
};
