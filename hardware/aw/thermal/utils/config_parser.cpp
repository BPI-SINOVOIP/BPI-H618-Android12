...
std::map<std::string, CoolingType> ParseCoolingDevice(std::string config_path) {
    std::ifstream json_doc(config_path);
    ...
    if (!Json::parseFromStream(reader, json_doc, &root, &errs)) {
        LOG(ERROR) << "Failed to parse JSON config";
