...
CCameraConfig::CCameraConfig(int id)
    ...
    ,mConstructOk(false)
    ,mCurCameraId(id)
    ,mNumberOfCsiOrMipiCamera(0)
    ,mNumberOfUsbCamera(0)
    ,mCameraFacing(0)
    ,mOrientation(0)
    ,mDeviceID(0)
    ,mFastPictureMode(false)
{
    ...
    if (mhKeyFile == (void *)-1)
    {
        LOGE("open file %s failed", CAMERA_KEY_CONFIG_PATH);
        return;
    }
    else
    {
    ...
    char numberOfCsiOrMipiCamera[2];
    char numberOfUsbCamera[2];
    if(readKey(kNUMBER_OF_CSI_OR_MIPI_CAMERA, numberOfCsiOrMipiCamera))
    {
        mNumberOfCsiOrMipiCamera = atoi(numberOfCsiOrMipiCamera);
        LOGV("read csi number: %d", mNumberOfCsiOrMipiCamera);
    }
    if(readKey(kNUMBER_OF_USB_CAMERA, numberOfUsbCamera))
    {
        mNumberOfUsbCamera = atoi(numberOfUsbCamera);
        LOGV("read usb number: %d", mNumberOfUsbCamera);
    }

    if (mCurCameraId + 1 > (mNumberOfCsiOrMipiCamera + mNumberOfUsbCamera))
    {
        LOGV("should not read camera id: %d, there are %d camera(s)", mCurCameraId,
            (mNumberOfCsiOrMipiCamera + mNumberOfUsbCamera));
        return;
    }
...
bool CCameraConfig::readKey(char *key, char *value)
{
    ...
    {
        if (!strcmp(key, kNUMBER_OF_CSI_OR_MIPI_CAMERA)
            || !strcmp(key, kNUMBER_OF_USB_CAMERA)
