...
int GetV4L2Metadata(std::shared_ptr<V4L2Wrapper> device,
  ...
  if(pCameraCfg->supportFocusMode())
  {
      ...
      std::array<int32_t, 5> active_regions = {{0,0,size_width,size_height,0}};
  ...
  if(pCameraCfg->supportFlashMode() && deviceStream->GetDeviceId() == DEVICE_FACING_BACK)
  {
      components.insert(V4L2ControlOrDefault<uint8_t>(
          ControlType::kMenu,
  ...
  if(pCameraCfg->supportFlashMode() && deviceStream->GetDeviceId() == DEVICE_FACING_BACK)
  {
      ...
      components.insert(std::unique_ptr<PartialMetadataInterface>(
        new Property<int64_t>(ANDROID_FLASH_INFO_CHARGE_DURATION, flash_info_charge_duration)));*/
  ...
  components.insert(std::unique_ptr<PartialMetadataInterface>(
      new Property<int32_t>(ANDROID_JPEG_MAX_SIZE, size_width*size_height*3/2 + sizeof(camera3_jpeg_blob_t))));
  ...
  if(pCameraCfg->supportZoom()/* && deviceStream->GetDeviceId() == DEVICE_FACING_BACK*/)
  {
      ...
      HAL_LOGV("####ANDROID_SCALER_AVAILABLE_MAX_DIGITAL_ZOOM : %d",max_digital_zoom);
      if(max_digital_zoom > 0 && max_digital_zoom < 50)
      {
          components.insert(std::unique_ptr<PartialMetadataInterface>(
              new Property<float>(ANDROID_SCALER_AVAILABLE_MAX_DIGITAL_ZOOM, max_digital_zoom)));
      }
      else
      {
          HAL_LOGE("####set android scaler available max digital zoom parameters error!");
      }
  }
  else
  {
  ...
  components.insert(
      NoEffectMenuControl<int32_t>(ANDROID_SENSOR_TEST_PATTERN_MODE,
                                   ANDROID_SENSOR_AVAILABLE_TEST_PATTERN_MODES,
                                    {ANDROID_SENSOR_TEST_PATTERN_MODE_OFF, ANDROID_SENSOR_TEST_PATTERN_MODE_BLACK}));
