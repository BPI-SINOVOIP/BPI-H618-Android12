...
// NoEffectMenuControl: Some menu options, but they have no effect, select index option for default value.
// index = 1 mean the second one.
template <typename T>
std::unique_ptr<Control<T>> NoEffectMenuControlSelectIndex(
    int32_t delegate_tag,
    int32_t options_tag,
    const std::vector<T>& options,
    int32_t index,
...
std::unique_ptr<Control<T>> NoEffectMenuControlSelectIndex(
  ...
  return std::make_unique<Control<T>>(
      std::make_unique<TaggedControlDelegate<T>>(
          delegate_tag,
          std::make_unique<NoEffectControlDelegate<T>>(options[index])),
      std::make_unique<TaggedControlOptions<T>>(
          options_tag,
          std::make_unique<MenuControlOptions<T>>(options, default_values)));
}
...
std::unique_ptr<Control<T>> V4L2Control(
  ...
  std::shared_ptr<V4L2Stream> device_ = device->getStream(MAIN_STREAM);
  if (device_ == nullptr) {
  ...
  int res = device_->QueryControl(control_id, &control_query);
  if (res) {
    HAL_LOGE("Failed to query control %d.", control_id);
    return nullptr;
  }
  HAL_LOGD("Control %d (%s), type %d",
           control_id,
           control_query.name,
           control_query.type);
  ...
  switch (control_query.type) {
    case V4L2_CTRL_TYPE_BOOLEAN:
      ...
      for (int32_t i = control_min; i <= control_max; i += control_step) {
        ...
        HAL_LOGD("V4L2 value %d for control %d.",
         i,
         control_id);
        if (res == -EINVAL) {
