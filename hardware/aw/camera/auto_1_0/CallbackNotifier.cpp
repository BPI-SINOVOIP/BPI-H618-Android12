...
CallbackNotifier::~CallbackNotifier()
{
    ...
    {
        native_handle_close(mHandleBuffer);
        mHandleBuffer = NULL;
    }
}
...
void CallbackNotifier::onNextFrameAvailable(const void* frame, bool hw, int release)
{
    V4L2BUF_t *tmpBuf[1];

    tmpBuf[0] = (V4L2BUF_t*)frame;
    onNextFrameHW((const void **)(&tmpBuf), 1, release);
}

void CallbackNotifier::onNextFrameAvailable(const void** frame, int count, int release)
{
    onNextFrameHW(frame, count, release);
}
...
bool CallbackNotifier::takePicture(const void* frame, void *memOpsS,bool is_continuous)
{
    ...
    LOGD("mGpsMethod:%s,mGpsLatitude:%lf,mGpsLongitude:%lf,mGpsAltitude:%ld,mGpsTimestamp:%ld", \
        mGpsMethod,
        mGpsLatitude,
        mGpsLongitude,
        mGpsAltitude,
...
#if ENABLE_CAMERA_SOURCE
void CallbackNotifier::onNextFrameHW(const void** frame, int count, int release)
{
    int i = 0;
    V4L2BUF_t **frmArray = (V4L2BUF_t **)frame;
    VideoNativeHandleMetadata mVideoNativeMetadata;

    LOGV("%d frames need process", count);

    if (NULL != mHandleBuffer)
    {
        if (mHandleBuffer->numFds != count)
        {
            LOGV("recreate native handle from %d to %d", mHandleBuffer->numFds, count);
            native_handle_close(mHandleBuffer);
            mHandleBuffer = native_handle_create(count, NATIVE_PARAM_NUMS * count);
            if (NULL == mHandleBuffer)
            {
                LOGE("recreate native handle fail!");
                return;
            }
        }
    }
    else
    {
        ...
        if (NULL == mHandleBuffer)
        {
            LOGE("create native handle fail!");
            return;
        }
    }
    ...
    for (i = 0; i < count; i++)
    {
        V4L2BUF_t *pbuf = frmArray[i];

        LOGV("Upload a frame buffer %d,%d,%d,%d,%d,%x", pbuf->nShareBufFd, pbuf->width, pbuf->height, pbuf->index, pbuf->refCnt, release);

        mHandleBuffer->data[i] = pbuf->nShareBufFd;
        mHandleBuffer->data[count + NATIVE_PARAM_NUMS * i] = pbuf->width;
        mHandleBuffer->data[count + NATIVE_PARAM_NUMS * i + 1] = pbuf->height;
        if (NEED_RELEASE_INDEX == release)
        {
            pbuf->refMutex.lock();
            pbuf->refCnt++;
            pbuf->refMutex.unlock();
            mHandleBuffer->data[count + NATIVE_PARAM_NUMS * i + 2] = pbuf->index;
        }
        else
        {
            mHandleBuffer->data[count + NATIVE_PARAM_NUMS * i + 2] = NOT_NEED_RELEASE_INDEX;
        }
    }

    mVideoNativeMetadata.eType = kMetadataBufferTypeNativeHandleSource;
    mVideoNativeMetadata.pHandle = mHandleBuffer;
    camera_memory_t* cam_buff = mGetMemoryCB(-1, (sizeof(VideoNativeHandleMetadata)), 1, mCallbackCookie);
    if (NULL != cam_buff && NULL != cam_buff->data) {
        memcpy((void *)cam_buff->data, &mVideoNativeMetadata, sizeof(VideoNativeHandleMetadata));
        mDataCBTimestamp(0, CAMERA_MSG_VIDEO_FRAME,
    ...
    if (isMessageEnabled(CAMERA_MSG_PREVIEW_FRAME))
    {
        ...
        if (strcmp(mCallingProcessName, "com.android.facelock") == 0) {
            ...
            if (NULL != cam_buff && NULL != cam_buff->data) {
                ...
                {
                    memcpy(cam_buff->data,(void*)src_addr_vir,mCBWidth * mCBHeight * 3 / 2);
                }
                else
                {
