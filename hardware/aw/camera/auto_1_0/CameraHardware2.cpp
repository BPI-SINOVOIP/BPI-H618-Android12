...
#endif //#ifdef __OPEN_SMILEDECTION__
...
#endif
{
    ...
    priv = this;
#ifdef AWCAMRECORDER
    mCallbackNotifier.mCameraHardware = this;
#endif
...
COMMOM_PARAMS:
    ...
    if((mCameraManager != NULL) && (mCameraManager->isOviewEnable())) {
        if (mCameraManager->mCameraTotalNum == 2) {
            mCallbackNotifier.setCBSize(mCaptureWidth, mCaptureHeight*2);
            LOGD("Mic-ts,F:%s,L:%d,w:%d,h:%d",__FUNCTION__,__LINE__,mCaptureWidth,mCaptureHeight*2);
        }
        if (mCameraManager->mCameraTotalNum == 4) {
            mCallbackNotifier.setCBSize(mCaptureWidth*2, mCaptureHeight*2);
            LOGD("Mic-ts,F:%s,L:%d,w:%d,h:%d",__FUNCTION__,__LINE__,mCaptureWidth*2,mCaptureHeight*2);
        }
    } else {
        mCallbackNotifier.setCBSize(mCaptureWidth, mCaptureHeight);
        LOGD("Mic-ts,F:%s,L:%d",__FUNCTION__,__LINE__);
    }
#else
    mCallbackNotifier.setCBSize(mCaptureWidth, mCaptureHeight);
#endif
...
void CameraHardware::setCameraManager(CameraManager * manager)
{
    ...
    LOGD("CameraDebug, CameraManager F:%s,L:%d",__FUNCTION__,__LINE__);
}
#endif
...
status_t CameraHardware::awCameraReleaseByIndex(int32_t index)
{
    if (mCameraManager->isOviewEnable())
    {
        ALOGV("release camera buffer start index=%d", index);
        mCameraManager->releaseByIndex(index);
    }
    else
    {
        ALOGV("release camera buffer index=%d", index);
...
void CameraHardware::releaseRecordingFrame(const void* opaque)
{
    if (mUseHwEncoder)
    {
        ...
        if((mCameraManager != NULL) && (mCameraManager->isOviewEnable())) {
            ...
            mV4L2CameraDevice->releasePreviewFrame(((VencInputBuffer*)((unsigned char *)opaque+4))->nID);
        }
#else
        mV4L2CameraDevice->releasePreviewFrame(((VencInputBuffer*)((unsigned char *)opaque+4))->nID);
...
*/
    ...
    if (mCaptureWidth == frame_width
        ...
        && mIsImageCaptureIntent == false)
    {
        LOGD("current capture size[%dx%d] is the same as picture size", frame_width, frame_height);
        mV4L2CameraDevice->setTakePictureState(TAKE_PICTURE_FAST);
        return OK;
    }
...
status_t CameraHardware::setParameters(const char* p)
{
    ...
    if (0 < new_preview_width && 0 < new_preview_height)
    {
        ...
        #ifdef CAMERA_MANAGER_ENABLE
        if((mCameraManager != NULL) && (mCameraManager->isOviewEnable())) {
            mCallbackNotifier.setCBSize(new_preview_width, new_preview_height*2);
            LOGD("Mic-ts,F:%s,L:%d,w:%d,h:%d",__FUNCTION__,__LINE__,new_preview_width,new_preview_height);
        } else {
...
#ifdef CAMERA_MANAGER_ENABLE
    if((mCameraManager != NULL) && (mCameraManager->isOviewEnable())) {
        mCaptureWidth = mCameraManager->mFrameWidth;
        mCaptureHeight= mCameraManager->mFrameHeight;
    }
#endif
    // preview format and video format are the same
    uint32_t org_fmt = V4L2_PIX_FMT_NV21;        // android default
...
int CameraHardware::awCamera_release_byIndex(struct camera_device* dev,int32_t index)
{
    CameraHardware* ec = reinterpret_cast<CameraHardware*>(dev->priv);
    if (ec == NULL) {
        LOGE("%s: Unexpected NULL camera device", __FUNCTION__);
    ...
    return ec->awCameraReleaseByIndex(index);
}
...
camera_device_ops_t CameraHardware::mDeviceOps = {
    ...
    CameraHardware::awCamera_release_byIndex,
...
#endif
#ifdef AWCAMRECORDER
    CameraHardware::aw_cam_rec_init,
    CameraHardware::aw_cam_rec_start,
    CameraHardware::aw_cam_rec_stop,
    CameraHardware::aw_cam_rec_set_para_and_cmds,
    CameraHardware::aw_cam_rec_get_para_and_cmds,
    CameraHardware::aw_cam_rec_start_next_file,
    CameraHardware::aw_cam_rec_release,
#endif
