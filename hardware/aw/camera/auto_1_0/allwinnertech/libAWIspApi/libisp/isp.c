...
void isp_flash_update_status(struct isp_lib_context *isp_gen, struct hw_isp_device *isp)
{
	...
	switch (isp_gen->ae_settings.flash_mode) {
			...
			if(isp_gen->ae_settings.take_picture_flag == V4L2_TAKE_PICTURE_FLASH) {
				if(isp_gen->ae_settings.flash_open == 0) {
					if(1) {
						...
						ISP_PRINT("%s: TORCH_ON, ev_set.ev_idx:%d, take_pic_start_cnt:%d, flash_delay_frame:%d.",
						__FUNCTION__,
						isp_ae_cxt->ae_result.sensor_set.ev_set.ev_idx,
						isp_gen->ae_settings.take_pic_start_cnt,
						isp_gen->isp_ini_cfg.isp_tunning_settings.flash_delay_frame);
						ret = isp_flash_ctrl(isp, V4L2_FLASH_LED_MODE_TORCH);
		...
		case FLASH_MODE_AUTO: {
			if(isp_gen->ae_settings.take_picture_flag == V4L2_TAKE_PICTURE_FLASH) {
				if(isp_gen->ae_settings.flash_open == 0) {
					if(isp_ae_cxt->ae_result.sensor_set.ev_set.ev_idx > (isp_ae_cxt->ae_result.sensor_set.ev_idx_max - 40)) {
						isp_gen->ae_settings.take_pic_start_cnt = isp_gen->ae_frame_cnt;
						ISP_PRINT("%s: TORCH_ON, ev_set.ev_idx:%d, take_pic_start_cnt:%d, flash_delay_frame:%d.",
						__FUNCTION__,
						isp_ae_cxt->ae_result.sensor_set.ev_set.ev_idx,
				...
				if((isp_gen->ae_settings.flash_open == 1)
					&& (isp_gen->ae_frame_cnt == (isp_gen->ae_settings.take_pic_start_cnt +isp_gen->isp_ini_cfg.isp_tunning_settings.flash_delay_frame)))
				{
					ret = isp_flash_ctrl(isp, V4L2_FLASH_LED_MODE_NONE);

					isp_gen->ae_settings.exposure_lock = true;
					if(isp_ae_cxt->ae_result.ae_flash_ev_cumul >= 100)
					{
						isp_ae_cxt->ae_result.sensor_set.ev_idx_flash = clamp(isp_ae_cxt->ae_result.sensor_set.ev_idx_expect,1,
							isp_ae_cxt->ae_result.sensor_set.ev_idx_max);
					}
					else if(isp_ae_cxt->ae_result.ae_flash_ev_cumul < 100
						&& isp_ae_cxt->ae_result.ae_flash_ev_cumul >= isp_gen->isp_ini_cfg.isp_tunning_settings.flash_gain*100/256)
					{
						isp_ae_cxt->ae_result.sensor_set.ev_idx_flash = clamp(isp_ae_cxt->ae_result.sensor_set.ev_idx_expect,1,
							isp_ae_cxt->ae_result.sensor_set.ev_idx_max);
					}
					else if(isp_ae_cxt->ae_result.ae_flash_ev_cumul >= -25
						&& isp_ae_cxt->ae_result.ae_flash_ev_cumul < isp_gen->isp_ini_cfg.isp_tunning_settings.flash_gain*100/256)
					{
						isp_ae_cxt->ae_result.sensor_set.ev_idx_flash = clamp(isp_ae_cxt->ae_result.sensor_set.ev_idx_expect +
							isp_ae_cxt->ae_result.ae_flash_ev_cumul *isp_gen->isp_ini_cfg.isp_tunning_settings.flash_gain/256,1,
							isp_ae_cxt->ae_result.sensor_set.ev_idx_max);
					}
					else
					{
					...
					__FUNCTION__,
					isp_ae_cxt->ae_result.ae_flash_ev_cumul,
					isp_ae_cxt->ae_result.sensor_set.ev_idx_expect,
					isp_ae_cxt->ae_result.sensor_set.ev_idx_flash,
					isp_gen->isp_ini_cfg.isp_tunning_settings.flash_gain);

					isp_ae_cxt->ae_param.u.ae_pline_index = isp_ae_cxt->ae_result.sensor_set.ev_idx_flash;
					isp_ae_set_params_helper(isp_ae_cxt, ISP_AE_SET_EXP_IDX);
				}
				if((isp_gen->ae_settings.flash_open == 1)
					&& (isp_gen->ae_frame_cnt == (1 +isp_gen->ae_settings.take_pic_start_cnt +isp_gen->isp_ini_cfg.isp_tunning_settings.flash_delay_frame)))
				{
					ISP_PRINT("%s: FLASH_ON.",__FUNCTION__);
					ret = isp_flash_ctrl(isp, V4L2_FLASH_LED_MODE_FLASH);
				}
				if((isp_gen->ae_settings.flash_open == 1)
					&& (isp_gen->ae_frame_cnt >= (4 +isp_gen->ae_settings.take_pic_start_cnt +isp_gen->isp_ini_cfg.isp_tunning_settings.flash_delay_frame))
					&& (isp_gen->ae_frame_cnt <= (6 +isp_gen->ae_settings.take_pic_start_cnt +isp_gen->isp_ini_cfg.isp_tunning_settings.flash_delay_frame)))
				{
					isp_hal_interaction_params.ith_params.image_para.bits.flash_ok = 1;
				}
				if((isp_gen->ae_settings.flash_open == 1)
					&& (isp_gen->ae_frame_cnt == (7 +isp_gen->ae_settings.take_pic_start_cnt +isp_gen->isp_ini_cfg.isp_tunning_settings.flash_delay_frame)))
				{
					ISP_PRINT("%s: FLASH_OFF.",__FUNCTION__);
					ret = isp_flash_ctrl(isp, V4L2_FLASH_LED_MODE_NONE);
			...
			//isp_config_sensor_exposure(isp_ae_cxt);
			break;
		}
		case FLASH_MODE_TORCH: {
			if(isp_gen->ae_settings.take_picture_flag == V4L2_TAKE_PICTURE_FLASH) {
				...
				if((isp_gen->ae_settings.flash_open == 1)
					&& (isp_gen->ae_frame_cnt >= (1 +isp_gen->ae_settings.take_pic_start_cnt +isp_gen->isp_ini_cfg.isp_tunning_settings.flash_delay_frame))
					&& (isp_gen->ae_frame_cnt <= (3 +isp_gen->ae_settings.take_pic_start_cnt +isp_gen->isp_ini_cfg.isp_tunning_settings.flash_delay_frame)))
				{
					isp_hal_interaction_params.ith_params.image_para.bits.flash_ok = 1;
				}
				...
				ISP_PRINT("%s: TORCH_ON, FLASH_MODE TORCH_ON.",__FUNCTION__);
				ret = isp_flash_ctrl(isp, V4L2_FLASH_LED_MODE_TORCH);
			}
			break;
		}
		...
		case FLASH_MODE_OFF: {
			ISP_PRINT("%s: FLASH_OFF, FLASH_MODE FLASH_OFF.",__FUNCTION__);
			ret = isp_flash_ctrl(isp, V4L2_FLASH_LED_MODE_NONE);
			break;
		}
	}
