...
void enable_ahub_bt_out(struct mixer *mixer);
void close_ahub_hdmi_out(struct mixer *mixer);
void close_ahub_bt_out(struct mixer *mixer);
void enable_ahub_out(struct mixer *mixer);
void enable_ahub_bt_in(struct mixer *mixer);
void close_ahub_bt_in(struct mixer *mixer);
void enable_ahub_ac107_in(struct mixer *mixer);
void close_ahub_ac107_in(struct mixer *mixer);
void close_ahub_in(struct mixer *mixer);
...
static name_map audio_name_map[AUDIO_MAP_CNT] =
{
    ...
    {"snddaudio0",      AUDIO_NAME_AC107},//ac107 codec-in
};
...
static char * get_audio_devices(struct sunxi_audio_device *adev, int in_out)
{
    ...
    for(card = 0; card < MAX_AUDIO_DEVICES; card++)
    {
        if (adev->dev_manager[card].flag_exist == true
                ...
                && (strcmp(adev->dev_manager[card].name, "AUDIO_AHUB") != 0)
                && (strcmp(adev->dev_manager[card].name, "AUDIO_AC107") != 0))
        {
            if (adev->dev_manager[card].flag_in == AUDIO_IN)
            {
    ...
    for (i = 0; i < cnt; i++)
    {
        ...
        if (card == MAX_AUDIO_DEVICES)
        {
            if (in_out & AUDIO_IN)
            {
                ...
                adev->dev_manager[adev->cardAC107].flag_in_active = 1;
                ALOGE("but device %s %s will be active", adev->dev_manager[adev->cardAC107].name, (in_out & AUDIO_IN) ? "input" : "ouput");
                return 0;
            }
            else
            {
...
static int start_output_stream(struct sunxi_stream_out *out)
{
    ...
    if(adev->out_device & AUDIO_DEVICE_OUT_ALL_SCO)
    {
        ...
        out->sub_pcm[adev->cardBT] = pcm_open(adev->cardBT, 0, PCM_OUT, &out->multi_config[card]);
        if (!pcm_is_ready(out->sub_pcm[adev->cardBT])) {
            ALOGE("cannot open pcm driver: %s", pcm_get_error(out->sub_pcm[adev->cardBT]));
            pcm_close(out->sub_pcm[adev->cardBT]);
            out->sub_pcm[adev->cardBT] = NULL;
            adev->active_output = NULL;
            return -ENOMEM;
        }
        out->multi_pcm[adev->cardBT] = pcm_open(adev->cardAHUB, PORT_BT, PCM_OUT, &out->multi_config[adev->cardBT]);
    ...
    else
    {
        ...
        for (index = 0; index < MAX_AUDIO_DEVICES; index++)
        {
                ...
                && adev->dev_manager[index].flag_out_active)
            {
                ...
                set_raw_flag(adev, adev->cardAHUB, SND_AUDIO_RAW_DATA_PCM);
                if(card == adev->cardHDMI)
                {
                    if(adev->raw_flag || (out->flags & AUDIO_OUTPUT_FLAG_DIRECT))
                    {
                        property_get(PROP_RAWDATA_KEY, prop_value_card, PROP_RAWDATA_DEFAULT_VALUE);
                        if(card == adev->cardHDMI && !strcmp(prop_value_card, PROP_RAWDATA_MODE_HDMI_RAW))
                        {
                            ...
                            out->multi_config[card] = out->config;
                            set_raw_flag(adev, adev->cardAHUB, config->raw_flag);
                        }
                    }
                    out->sub_pcm[card] = pcm_open(card, port, PCM_OUT, &out->multi_config[card]);
                    ...
                    out->multi_pcm[card] = pcm_open(adev->cardAHUB, port, PCM_OUT, &out->multi_config[card]);
    ...
    enable_ahub_out(adev->audio_hub_mixer);
    return 0;
}
...
static int start_input_stream(struct sunxi_stream_in *in)
{
    ...
    {
        if (adev->dev_manager[i].flag_exist
            && (adev->dev_manager[i].flag_in == AUDIO_IN)
            && adev->dev_manager[i].flag_in_active)
        {
            card = i;
            if(i == adev->cardAC107)
                break;
    ...
    if(card == adev->cardAC107)
    {
        in->sub_pcm = pcm_open_req(card, 0, PCM_IN, &in->config, in_ajust_rate);
        ...
        in->pcm= pcm_open_req(adev->cardAHUB, 0, PCM_IN, &in->config, in_ajust_rate);
        ...
        pcm_prepare(in->sub_pcm);
        pcm_prepare(in->pcm);
        close_ahub_bt_in(adev->audio_hub_mixer);
        enable_ahub_ac107_in(adev->audio_hub_mixer);
    }
    else if(card == adev->cardBT)
    {
        ...
        close_ahub_ac107_in(adev->audio_hub_mixer);
        enable_ahub_bt_in(adev->audio_hub_mixer);
    }
    else
    {
        close_ahub_in(adev->audio_hub_mixer);
...
void enable_ahub_out(struct mixer *mixer)
{
    enable_ahub_hdmi_out(mixer);
    enable_ahub_bt_out(mixer);
}

void enable_ahub_hdmi_out(struct mixer *mixer)
{
    ALOGD("enable_ahub_hdmi_out");
    struct mixer_ctl *ctl;
...
void close_ahub_in(struct mixer *mixer)
{
    close_ahub_ac107_in(mixer);
    close_ahub_bt_in(mixer);
}
                     ...
                     hw_device_t** device)
{
    ...
    getCardNumbyName(adev, AUDIO_NAME_AHUB, &adev->cardAHUB);
    getCardNumbyName(adev, AUDIO_NAME_AC107, &adev->cardAC107);
    ...
    adev->audio_hub_mixer = mixer_open(adev->cardAHUB);
    if (!adev->audio_hub_mixer) {
        free(adev);
        ALOGE("Unable to open the audio_hub_mixer, aborting.");
        return -EINVAL;
    }
