...
#ifndef ANDROID_COMPONENTS_INCLUDE_HWC2INTERFACE_H_
#define ANDROID_COMPONENTS_INCLUDE_HWC2INTERFACE_H_
...
/**
 * Wrap a common interface object (such as Codec2Client::Interface,
 * or C2InterfaceHelper into a C2ComponentInterface.
 * \param T common interface type
 */
template <typename T>
class HwC2Interface : public C2ComponentInterface {
 public:
  HwC2Interface(const char *name, c2_node_id_t id,
                const std::shared_ptr<T> &impl)
      : mName(name), mId(id), mImpl(impl) {}

  ~HwC2Interface() override = default;
...
class HwC2Interface<void> {
 public:
  ...
  struct BaseParams : C2InterfaceHelper {
    ...
    // Marks that this component has no input latency. Otherwise,
    // component must add support for C2PortRequestedDelayTuning::input and
    // C2PortActualDelayTuning::input.
    void noInputLatency();

    // Marks that this component has no output latency. Otherwise,
    // component must add support for C2PortRequestedDelayTuning::output and
    ...
    // Marks that this component has no need for private buffers. Otherwise,
    // component must add support for C2MaxPrivateBufferCountTuning,
    // C2PrivateAllocatorsTuning and C2PrivateBlockPoolsTuning.
    void noPrivateBuffers();

    // Marks that this component holds no references to input buffers.Otherwise,
    // component must add support for C2StreamMaxReferenceAgeTuning::input and
...
std::shared_ptr<T> AllocSharedString(const Args(&... args),
  ...
  strncpy(ret->m.value, str.c_str(), str.length() + 1);
  return ret;
}
...
#endif  // COMPONENTS_INCLUDE_HWC2INTERFACE_H_
