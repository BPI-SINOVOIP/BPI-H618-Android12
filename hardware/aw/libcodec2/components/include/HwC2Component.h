...
#ifndef ANDROID_COMPONENTS_INCLUDE_HWC2COMPONENT_H_
#define ANDROID_COMPONENTS_INCLUDE_HWC2COMPONENT_H_
...
class HwC2Component : public C2Component,
                      public std::enable_shared_from_this<HwC2Component> {
 public:
  explicit HwC2Component(const std::shared_ptr<C2ComponentInterface> &intf);
  virtual ~HwC2Component();

  // C2Component
  // From C2Component
  c2_status_t setListener_vb(const std::shared_ptr<Listener> &listener,
                             c2_blocking_t mayBlock) override;
  c2_status_t queue_nb(
      std::list<std::unique_ptr<C2Work>> *const items) override;
 ...
 protected:
  ...
  void handleWorkCb(std::unique_ptr<C2Work> work);
 ...
 private:
  ...
  class WorkHandler : public AHandler {
   public:
    ...
    void setComponent(const std::shared_ptr<HwC2Component> &thiz);

   protected:
    void onMessageReceived(const sp<AMessage> &msg) override;

   private:
    std::weak_ptr<HwC2Component> mThiz;
    bool mRunning;
  };
  ...
  HwC2Component() = delete;
};
}  // namespace android

#endif  // ANDROID_COMPONENTS_INCLUDE_HWC2COMPONENT_H_
