...
//#define LOG_NDEBUG 0
#define LOG_TAG "XvidDecComponent"
#include "XvidDecComponent.h"
...
constexpr char COMPONENT_NAME[] = "c2.allwinner.xvid.decoder";
constexpr uint32_t kDefaultOutputDelay = 9;
constexpr uint32_t kMaxOutputDelay = 20;

}  // namespace

class XvidDecComponent::IntfImpl : public HwC2Interface<void>::BaseParams {
 public:
  explicit IntfImpl(const std::shared_ptr<C2ReflectorHelper> &helper)
      : HwC2Interface<void>::BaseParams(
            helper, COMPONENT_NAME, C2Component::KIND_DECODER,
            C2Component::DOMAIN_VIDEO, MEDIA_MIMETYPE_VIDEO_XVID) {
    noPrivateBuffers();
    noInputReferences();
    noOutputReferences();
    noInputLatency();
...
void XvidDecComponent::handleWorkCb2(std::unique_ptr<C2Work> work) {
  handleWorkCb(std::move(work));
}

XvidDecComponent::XvidDecComponent(const char *name, c2_node_id_t id,
  ...
  mCodec->setWorkHandlerCb(
      std::bind(&XvidDecComponent::handleWorkCb2, this, std::placeholders::_1));
  mCodec->setIntf(mIntf);
}

XvidDecComponent::~XvidDecComponent() {}

c2_status_t XvidDecComponent::onInit() { return mCodec->onInit(); }

c2_status_t XvidDecComponent::onStart() { return mCodec->onStart(); }

c2_status_t XvidDecComponent::onStop() { return mCodec->onStop(); }

void XvidDecComponent::onReset() { mCodec->onReset(); }

void XvidDecComponent::onRelease() { mCodec->onRelease(); }

c2_status_t XvidDecComponent::onFlush_sm() { return mCodec->onFlush_sm(); }

void XvidDecComponent::process(const std::unique_ptr<C2Work> &work,
                               const std::shared_ptr<C2BlockPool> &pool) {
  mCodec->process(work, pool);
}

c2_status_t XvidDecComponent::drain(uint32_t drainMode,
                                    const std::shared_ptr<C2BlockPool> &pool) {
  return mCodec->drain(drainMode, pool);
}

class XvidDecComponentFactory : public C2ComponentFactory {
 public:
  XvidDecComponentFactory()
  ...
  c2_status_t createComponent(
    ...
    *component = std::shared_ptr<C2Component>(
        new XvidDecComponent(
            COMPONENT_NAME, id,
            std::make_shared<XvidDecComponent::IntfImpl>(mHelper)),
        deleter);
    return C2_OK;
  }
  ...
  c2_status_t createInterface(
    ...
    *interface = std::shared_ptr<C2ComponentInterface>(
        new HwC2Interface<XvidDecComponent::IntfImpl>(
            COMPONENT_NAME, id,
            std::make_shared<XvidDecComponent::IntfImpl>(mHelper)),
        deleter);
    return C2_OK;
  ...
  ~XvidDecComponentFactory() override = default;
...
extern "C" ::C2ComponentFactory *CreateCodec2Factory() {
  return new ::android::XvidDecComponentFactory();
}
