...
//#define LOG_NDEBUG 0
#define LOG_TAG "HevcDecComponent"
#include "HevcDecComponent.h"
...
constexpr char COMPONENT_NAME[] = "c2.allwinner.hevc.decoder";
...
class HevcDecComponent::IntfImpl : public HwC2Interface<void>::BaseParams {
 public:
  explicit IntfImpl(const std::shared_ptr<C2ReflectorHelper> &helper)
    ...
    addParameter(
        DefineParam(mParaHelper->mSize, C2_PARAMKEY_PICTURE_SIZE)
            ...
            .withFields({
                C2F(mParaHelper->mSize, width).inRange(2, 4096, 2),
                C2F(mParaHelper->mSize, height).inRange(2, 4096, 2),
            })
...
void HevcDecComponent::handleWorkCb2(std::unique_ptr<C2Work> work) {
  handleWorkCb(std::move(work));
}

HevcDecComponent::HevcDecComponent(const char *name, c2_node_id_t id,
  ...
  mCodec->setWorkHandlerCb(
      std::bind(&HevcDecComponent::handleWorkCb2, this, std::placeholders::_1));
  mCodec->setIntf(mIntf);
}

HevcDecComponent::~HevcDecComponent() {}

c2_status_t HevcDecComponent::onInit() { return mCodec->onInit(); }

c2_status_t HevcDecComponent::onStart() { return mCodec->onStart(); }

c2_status_t HevcDecComponent::onStop() { return mCodec->onStop(); }

void HevcDecComponent::onReset() { mCodec->onReset(); }

void HevcDecComponent::onRelease() { mCodec->onRelease(); }

c2_status_t HevcDecComponent::onFlush_sm() { return mCodec->onFlush_sm(); }

void HevcDecComponent::process(const std::unique_ptr<C2Work> &work,
                               const std::shared_ptr<C2BlockPool> &pool) {
  mCodec->process(work, pool);
}

c2_status_t HevcDecComponent::drain(uint32_t drainMode,
                                    const std::shared_ptr<C2BlockPool> &pool) {
  return mCodec->drain(drainMode, pool);
}

class HevcDecComponentFactory : public C2ComponentFactory {
 public:
  HevcDecComponentFactory()
  ...
  c2_status_t createComponent(
    ...
    *component = std::shared_ptr<C2Component>(
        new HevcDecComponent(
            COMPONENT_NAME, id,
            std::make_shared<HevcDecComponent::IntfImpl>(mHelper)),
        deleter);
    return C2_OK;
  }
  ...
  c2_status_t createInterface(
    ...
    *interface = std::shared_ptr<C2ComponentInterface>(
        new HwC2Interface<HevcDecComponent::IntfImpl>(
            COMPONENT_NAME, id,
            std::make_shared<HevcDecComponent::IntfImpl>(mHelper)),
        deleter);
    return C2_OK;
  ...
  virtual ~HevcDecComponentFactory() override = default;
...
extern "C" ::C2ComponentFactory *CreateCodec2Factory() {
  return new ::android::HevcDecComponentFactory();
}
