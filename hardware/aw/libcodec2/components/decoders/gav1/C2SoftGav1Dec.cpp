...
static void copyOutputBufferToYV12Frame(
    uint8_t *dstY, uint8_t *dstU, uint8_t *dstV, const uint8_t *srcY,
    const uint8_t *srcU, const uint8_t *srcV, size_t srcYStride,
    size_t srcUStride, size_t srcVStride, size_t dstYStride, size_t dstUVStride,
    uint32_t width, uint32_t height, bool isMonochrome) {
...
static void convertYUV420Planar16ToYUV420Planar(
    uint8_t *dstY, uint8_t *dstU, uint8_t *dstV, const uint16_t *srcY,
...
bool C2SoftGav1Dec::outputBuffer(const std::shared_ptr<C2BlockPool> &pool,
  ...
  if (buffer->bitdepth == 10) {
    ...
    copyOutputBufferToYV12Frame(dstY, dstU, dstV, srcY, srcU, srcV, srcYStride,
                                srcUStride, srcVStride, dstYStride, dstUVStride,
...
__attribute__((cfi_canonical_jump_table)) extern "C" ::C2ComponentFactory *
CreateCodec2Factory() {
  ALOGV("in %s", __func__);
