...
//#define LOG_NDEBUG 0
#define LOG_TAG "Vp8DecComponent"
#include "Vp8DecComponent.h"
...
constexpr char COMPONENT_NAME[] = "c2.allwinner.vp8.decoder";
constexpr uint32_t kDefaultOutputDelay = 9;
constexpr uint32_t kMaxOutputDelay = 20;

}  // namespace

class Vp8DecComponent::IntfImpl : public HwC2Interface<void>::BaseParams {
 public:
  explicit IntfImpl(const std::shared_ptr<C2ReflectorHelper> &helper)
      : HwC2Interface<void>::BaseParams(
            helper, COMPONENT_NAME, C2Component::KIND_DECODER,
            C2Component::DOMAIN_VIDEO, MEDIA_MIMETYPE_VIDEO_VP8) {
    noPrivateBuffers();
    noInputReferences();
    noOutputReferences();
    noInputLatency();
...
void Vp8DecComponent::handleWorkCb2(std::unique_ptr<C2Work> work) {
  handleWorkCb(std::move(work));
}

Vp8DecComponent::Vp8DecComponent(const char *name, c2_node_id_t id,
  ...
  mCodec->setWorkHandlerCb(
      std::bind(&Vp8DecComponent::handleWorkCb2, this, std::placeholders::_1));
  mCodec->setIntf(mIntf);
}

Vp8DecComponent::~Vp8DecComponent() {}

c2_status_t Vp8DecComponent::onInit() { return mCodec->onInit(); }

c2_status_t Vp8DecComponent::onStart() { return mCodec->onStart(); }

c2_status_t Vp8DecComponent::onStop() { return mCodec->onStop(); }

void Vp8DecComponent::onReset() { mCodec->onReset(); }

void Vp8DecComponent::onRelease() { mCodec->onRelease(); }

c2_status_t Vp8DecComponent::onFlush_sm() { return mCodec->onFlush_sm(); }

void Vp8DecComponent::process(const std::unique_ptr<C2Work> &work,
                              const std::shared_ptr<C2BlockPool> &pool) {
  mCodec->process(work, pool);
}

c2_status_t Vp8DecComponent::drain(uint32_t drainMode,
                                   const std::shared_ptr<C2BlockPool> &pool) {
  return mCodec->drain(drainMode, pool);
}

class Vp8DecComponentFactory : public C2ComponentFactory {
 public:
  Vp8DecComponentFactory()
  ...
  c2_status_t createComponent(
    ...
    *component = std::shared_ptr<C2Component>(
        new Vp8DecComponent(
            COMPONENT_NAME, id,
            std::make_shared<Vp8DecComponent::IntfImpl>(mHelper)),
        deleter);
    return C2_OK;
  }
  ...
  c2_status_t createInterface(
    ...
    *interface = std::shared_ptr<C2ComponentInterface>(
        new HwC2Interface<Vp8DecComponent::IntfImpl>(
            COMPONENT_NAME, id,
            std::make_shared<Vp8DecComponent::IntfImpl>(mHelper)),
        deleter);
    return C2_OK;
  ...
  ~Vp8DecComponentFactory() override = default;
...
extern "C" ::C2ComponentFactory *CreateCodec2Factory() {
  return new ::android::Vp8DecComponentFactory();
}
