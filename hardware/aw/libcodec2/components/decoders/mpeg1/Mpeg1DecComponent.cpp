...
//#define LOG_NDEBUG 0
#define LOG_TAG "Mpeg1DecComponent"
#include "Mpeg1DecComponent.h"
...
constexpr char COMPONENT_NAME[] = "c2.allwinner.mpeg1.decoder";
constexpr uint32_t kDefaultOutputDelay = 9;
constexpr uint32_t kMaxOutputDelay = 20;

}  // namespace

class Mpeg1DecComponent::IntfImpl : public HwC2Interface<void>::BaseParams {
 public:
  explicit IntfImpl(const std::shared_ptr<C2ReflectorHelper> &helper)
      : HwC2Interface<void>::BaseParams(
            helper, COMPONENT_NAME, C2Component::KIND_DECODER,
            C2Component::DOMAIN_VIDEO, MEDIA_MIMETYPE_VIDEO_MPEG1) {
    noPrivateBuffers();
    noInputReferences();
    noOutputReferences();
    noInputLatency();
...
void Mpeg1DecComponent::handleWorkCb2(std::unique_ptr<C2Work> work) {
  handleWorkCb(std::move(work));
}

Mpeg1DecComponent::Mpeg1DecComponent(const char *name, c2_node_id_t id,
  ...
  mCodec->setWorkHandlerCb(std::bind(&Mpeg1DecComponent::handleWorkCb2, this,
                                     std::placeholders::_1));
  mCodec->setIntf(mIntf);
}

Mpeg1DecComponent::~Mpeg1DecComponent() {}

c2_status_t Mpeg1DecComponent::onInit() { return mCodec->onInit(); }

c2_status_t Mpeg1DecComponent::onStart() { return mCodec->onStart(); }

c2_status_t Mpeg1DecComponent::onStop() { return mCodec->onStop(); }

void Mpeg1DecComponent::onReset() { mCodec->onReset(); }

void Mpeg1DecComponent::onRelease() { mCodec->onRelease(); }

c2_status_t Mpeg1DecComponent::onFlush_sm() { return mCodec->onFlush_sm(); }

void Mpeg1DecComponent::process(const std::unique_ptr<C2Work> &work,
                                const std::shared_ptr<C2BlockPool> &pool) {
  mCodec->process(work, pool);
}

c2_status_t Mpeg1DecComponent::drain(uint32_t drainMode,
                                     const std::shared_ptr<C2BlockPool> &pool) {
  return mCodec->drain(drainMode, pool);
}

class Mpeg1DecComponentFactory : public C2ComponentFactory {
 public:
  Mpeg1DecComponentFactory()
  ...
  c2_status_t createComponent(
    ...
    *component = std::shared_ptr<C2Component>(
        new Mpeg1DecComponent(
            COMPONENT_NAME, id,
            std::make_shared<Mpeg1DecComponent::IntfImpl>(mHelper)),
        deleter);
    return C2_OK;
  }
  ...
  c2_status_t createInterface(
    ...
    *interface = std::shared_ptr<C2ComponentInterface>(
        new HwC2Interface<Mpeg1DecComponent::IntfImpl>(
            COMPONENT_NAME, id,
            std::make_shared<Mpeg1DecComponent::IntfImpl>(mHelper)),
        deleter);
    return C2_OK;
  ...
  ~Mpeg1DecComponentFactory() override = default;
...
extern "C" ::C2ComponentFactory *CreateCodec2Factory() {
  return new ::android::Mpeg1DecComponentFactory();
}
