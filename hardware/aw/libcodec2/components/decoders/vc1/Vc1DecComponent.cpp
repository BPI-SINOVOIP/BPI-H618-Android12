...
//#define LOG_NDEBUG 0
#define LOG_TAG "Vc1DecComponent"
#include "Vc1DecComponent.h"
...
constexpr char COMPONENT_NAME[] = "c2.allwinner.vc1.decoder";

}  // namespace

class Vc1DecComponent::IntfImpl : public HwC2Interface<void>::BaseParams {
 public:
  explicit IntfImpl(const std::shared_ptr<C2ReflectorHelper> &helper)
      : HwC2Interface<void>::BaseParams(
            helper, COMPONENT_NAME, C2Component::KIND_DECODER,
            C2Component::DOMAIN_VIDEO, MEDIA_MIMETYPE_VIDEO_VC1) {
    noPrivateBuffers();
    noInputReferences();
    noOutputReferences();
    noInputLatency();
...
void Vc1DecComponent::handleWorkCb2(std::unique_ptr<C2Work> work) {
  handleWorkCb(std::move(work));
}

Vc1DecComponent::Vc1DecComponent(const char *name, c2_node_id_t id,
  ...
  mCodec->setWorkHandlerCb(
      std::bind(&Vc1DecComponent::handleWorkCb2, this, std::placeholders::_1));
}

Vc1DecComponent::~Vc1DecComponent() {}

c2_status_t Vc1DecComponent::onInit() { return mCodec->onInit(); }

c2_status_t Vc1DecComponent::onStart() { return mCodec->onStart(); }

c2_status_t Vc1DecComponent::onStop() { return mCodec->onStop(); }

void Vc1DecComponent::onReset() { mCodec->onReset(); }

void Vc1DecComponent::onRelease() { mCodec->onRelease(); }

c2_status_t Vc1DecComponent::onFlush_sm() { return mCodec->onFlush_sm(); }

void Vc1DecComponent::process(const std::unique_ptr<C2Work> &work,
                              const std::shared_ptr<C2BlockPool> &pool) {
  mCodec->process(work, pool);
}

c2_status_t Vc1DecComponent::drain(uint32_t drainMode,
                                   const std::shared_ptr<C2BlockPool> &pool) {
  return mCodec->drain(drainMode, pool);
}

class Vc1DecComponentFactory : public C2ComponentFactory {
 public:
  Vc1DecComponentFactory()
  ...
  c2_status_t createComponent(
    ...
    *component = std::shared_ptr<C2Component>(
        new Vc1DecComponent(
            COMPONENT_NAME, id,
            std::make_shared<Vc1DecComponent::IntfImpl>(mHelper)),
        deleter);
    return C2_OK;
  }
  ...
  c2_status_t createInterface(
    ...
    *interface = std::shared_ptr<C2ComponentInterface>(
        new HwC2Interface<Vc1DecComponent::IntfImpl>(
            COMPONENT_NAME, id,
            std::make_shared<Vc1DecComponent::IntfImpl>(mHelper)),
        deleter);
    return C2_OK;
  ...
  ~Vc1DecComponentFactory() override = default;
...
extern "C" ::C2ComponentFactory *CreateCodec2Factory() {
  return new ::android::Vc1DecComponentFactory();
}
