...
//#define LOG_NDEBUG 0
#define LOG_TAG "Mpeg2DecComponent"
#include "Mpeg2DecComponent.h"
...
constexpr char COMPONENT_NAME[] = "c2.allwinner.mpeg2.decoder";
constexpr uint32_t kDefaultOutputDelay = 9;
constexpr uint32_t kMaxOutputDelay = 20;
}  // namespace

class Mpeg2DecComponent::IntfImpl : public HwC2Interface<void>::BaseParams {
 public:
  explicit IntfImpl(const std::shared_ptr<C2ReflectorHelper> &helper)
      : HwC2Interface<void>::BaseParams(
            helper, COMPONENT_NAME, C2Component::KIND_DECODER,
            C2Component::DOMAIN_VIDEO, MEDIA_MIMETYPE_VIDEO_MPEG2) {
    noPrivateBuffers();
    noInputReferences();
    noOutputReferences();
    noInputLatency();
    ...
    addParameter(
        DefineParam(mParaHelper->mSize, C2_PARAMKEY_PICTURE_SIZE)
            ...
            .withFields({
                C2F(mParaHelper->mSize, width).inRange(176, 1920, 2),
                C2F(mParaHelper->mSize, height).inRange(144, 1088, 2),
            })
    ...
    addParameter(
        DefineParam(mParaHelper->mProfileLevel, C2_PARAMKEY_PROFILE_LEVEL)
            .withDefault(new C2StreamProfileLevelInfo::input(
                0u, C2Config::PROFILE_MP2V_SIMPLE, C2Config::LEVEL_MP2V_HIGH))
            .withFields({C2F(mParaHelper->mProfileLevel, profile)
...
void Mpeg2DecComponent::handleWorkCb2(std::unique_ptr<C2Work> work) {
  handleWorkCb(std::move(work));
}

Mpeg2DecComponent::Mpeg2DecComponent(const char *name, c2_node_id_t id,
  ...
  mCodec->setWorkHandlerCb(std::bind(&Mpeg2DecComponent::handleWorkCb2, this,
                                     std::placeholders::_1));
  mCodec->setIntf(mIntf);
}

Mpeg2DecComponent::~Mpeg2DecComponent() {}

c2_status_t Mpeg2DecComponent::onInit() { return mCodec->onInit(); }

c2_status_t Mpeg2DecComponent::onStart() { return mCodec->onStart(); }

c2_status_t Mpeg2DecComponent::onStop() { return mCodec->onStop(); }

void Mpeg2DecComponent::onReset() { mCodec->onReset(); }

void Mpeg2DecComponent::onRelease() { mCodec->onRelease(); }

c2_status_t Mpeg2DecComponent::onFlush_sm() { return mCodec->onFlush_sm(); }

void Mpeg2DecComponent::process(const std::unique_ptr<C2Work> &work,
                                const std::shared_ptr<C2BlockPool> &pool) {
  mCodec->process(work, pool);
}

c2_status_t Mpeg2DecComponent::drain(uint32_t drainMode,
                                     const std::shared_ptr<C2BlockPool> &pool) {
  return mCodec->drain(drainMode, pool);
}

class Mpeg2DecComponentFactory : public C2ComponentFactory {
 public:
  Mpeg2DecComponentFactory()
  ...
  c2_status_t createComponent(
    ...
    *component = std::shared_ptr<C2Component>(
        new Mpeg2DecComponent(
            COMPONENT_NAME, id,
            std::make_shared<Mpeg2DecComponent::IntfImpl>(mHelper)),
        deleter);
    return C2_OK;
  }
  ...
  c2_status_t createInterface(
    ...
    *interface = std::shared_ptr<C2ComponentInterface>(
        new HwC2Interface<Mpeg2DecComponent::IntfImpl>(
            COMPONENT_NAME, id,
            std::make_shared<Mpeg2DecComponent::IntfImpl>(mHelper)),
        deleter);
    return C2_OK;
  ...
  ~Mpeg2DecComponentFactory() override = default;
...
extern "C" ::C2ComponentFactory *CreateCodec2Factory() {
  return new ::android::Mpeg2DecComponentFactory();
}
