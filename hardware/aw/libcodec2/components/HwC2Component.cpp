...
std::unique_ptr<C2Work> HwC2Component::WorkQueue::pop_front() {
  std::unique_ptr<C2Work> work = std::move(mQueue.front().work);
  mQueue.pop_front();
  return work;
}

void HwC2Component::WorkQueue::push_back(std::unique_ptr<C2Work> work) {
  mQueue.push_back({std::move(work), NO_DRAIN});
}

bool HwC2Component::WorkQueue::empty() const { return mQueue.empty(); }

void HwC2Component::WorkQueue::clear() { mQueue.clear(); }

uint32_t HwC2Component::WorkQueue::drainMode() const {
  return mQueue.front().drainMode;
}

void HwC2Component::WorkQueue::markDrain(uint32_t drainMode) {
  mQueue.push_back({nullptr, drainMode});
}

HwC2Component::WorkHandler::WorkHandler() : mRunning(false) {}

void HwC2Component::WorkHandler::setComponent(
    const std::shared_ptr<HwC2Component> &thiz) {
  mThiz = thiz;
}
...
void HwC2Component::WorkHandler::onMessageReceived(const sp<AMessage> &msg) {
  std::shared_ptr<HwC2Component> thiz = mThiz.lock();
  if (!thiz) {
...
class HwC2Component::BlockingBlockPool : public C2BlockPool {
 public:
  explicit BlockingBlockPool(const std::shared_ptr<C2BlockPool> &base)
      : mBase{base} {}

  local_id_t getLocalId() const override { return mBase->getLocalId(); }

  C2Allocator::id_t getAllocatorId() const override {
    return mBase->getAllocatorId();
  }

  virtual c2_status_t fetchLinearBlock(uint32_t capacity, C2MemoryUsage usage,
  ...
  virtual c2_status_t fetchGraphicBlock(
    ...
    c2_status_t status;
    do {
      status = mBase->fetchGraphicBlock(width, height, format, usage, block);
    } while (status == C2_BLOCKING);
    return status;
  }
...
void HwC2Component::handleWorkCb(std::unique_ptr<C2Work> work) {
  std::shared_ptr<C2Component::Listener> listener =
      mExecState.lock()->mListener;
  listener->onWorkDone_nb(shared_from_this(), vec(std::move(work)));
  ALOGV("call back work by handleWorkCb");
}

HwC2Component::HwC2Component(const std::shared_ptr<C2ComponentInterface> &intf)
    : mIntf(intf), mLooper(new ALooper), mHandler(new WorkHandler) {
  mLooper->setName(intf->getName().c_str());
  ALOGD("make instance of codec component %s.", intf->getName().c_str());
  mLooper->registerHandler(mHandler);
  mLooper->start(false, false, ANDROID_PRIORITY_VIDEO);
}

HwC2Component::~HwC2Component() {
  mLooper->unregisterHandler(mHandler->id());
  mLooper->stop();
}

c2_status_t HwC2Component::setListener_vb(
...
c2_status_t HwC2Component::queue_nb(
...
c2_status_t HwC2Component::announce_nb(
    const std::vector<C2WorkOutline> & /*items*/) {
  return C2_OMITTED;
}

c2_status_t HwC2Component::flush_sm(
...
c2_status_t HwC2Component::drain_nb(drain_mode_t drainMode) {
...
c2_status_t HwC2Component::start() {
  Mutexed<ExecState>::Locked state(mExecState);
  if (state->mState == RUNNING) {
    return C2_BAD_STATE;
  }
...
c2_status_t HwC2Component::stop() {
  ALOGV("stop");
  {
    Mutexed<ExecState>::Locked state(mExecState);
    if (state->mState != RUNNING) {
...
c2_status_t HwC2Component::reset() {
  ALOGV("reset");
  {
    Mutexed<ExecState>::Locked state(mExecState);
...
c2_status_t HwC2Component::release() {
  ALOGV("release");
  sp<AMessage> reply;
...
std::shared_ptr<C2ComponentInterface> HwC2Component::intf() { return mIntf; }

bool HwC2Component::processQueue() {
  ...
  if (!mOutputBlockPool) {
    c2_status_t err = [this] {
      ...
      ALOGD("Using output block pool with poolID %llu => got %llu - %d",
            (uint64_t)poolId,
            (uint64_t)(blockPool ? blockPool->getLocalId() : 111000111), err);
      if (err == C2_OK) {
  ...
  ALOGV("start processing frame #%" PRIu64,
        work->input.ordinal.frameIndex.peeku());
  // If input buffer list is not empty, it means we have some input to process
  // on. However, input could be a nullptr buffer. In such case, clear the
  // buffer list before making call to process().
  if (!work->input.buffers.empty() && !work->input.buffers[0]) {
    ALOGD("Encountered nullptr input buffer. Clearing the input buffer");
    work->input.buffers.clear();
  }

  // process work, the work while processing is const
  ...
  // To call back work which bas been processed.
  {
    queue.unlock();
    Mutexed<ExecState>::Locked state(mExecState);
