...
struct sensor_extend_t gsensorList[] = {
    {
        ...
        {
            "sc7a20", 32.0f,    //for max range is 4G
        },  {
            "lis7a20 detect Accelerometer",
            "SL",
            1, 0,
            SENSOR_TYPE_ACCELEROMETER,
            (GRAVITY_EARTH * 4.0f),
...
int getDevice(struct sensor_extend_t list[], struct sensor_info *info,
        char buf[], char classPath[], int number, int ID)
{
    int ret = 0;
    ...
    while (ret < number) {
        if (!strncmp(buf, list[ret].sensors.name, strlen(buf)) && list[ret].sList.handle == ID) {
...
void statusInit(void)
{
    ...
    seStatus[ID_AU].isUsed  = true;
    seStatus[ID_AU].isFound = false;
}

int setUncalDevice(int ID, int number, int ID_UNCAL, int sensor_type, const char* sensor_string_type)
{
    int id_number = seSensorIndex[ID];
    const char* id_name = sSensorList[id_number].name;
    int len = strlen(id_name);
    char* uncal_name = new char[len + 16];
    memset(uncal_name, 0, len + 16);
    memcpy((char* )uncal_name, id_name, len);
    memcpy((char* )uncal_name + len," Uncalibrated", strlen(" Uncalibrated"));
    sSensorList[number].name = (const char* )uncal_name;
    sSensorList[number].vendor = sSensorList[id_number].vendor;
    sSensorList[number].version = sSensorList[id_number].version;
    sSensorList[number].handle = ID_UNCAL;
    sSensorList[number].type = sensor_type;
    sSensorList[number].maxRange = sSensorList[id_number].maxRange;
    sSensorList[number].resolution = sSensorList[id_number].resolution;
    sSensorList[number].power = sSensorList[id_number].power;
    sSensorList[number].minDelay = sSensorList[id_number].minDelay;
    sSensorList[number].stringType = sensor_string_type;
    sSensorList[number].requiredPermission = sSensorList[id_number].requiredPermission;
    sSensorList[number].maxDelay = sSensorList[id_number].maxDelay;
    sSensorList[number].flags = sSensorList[id_number].flags;
    memcpy(sSensorList[number].reserved, sSensorList[id_number].reserved,
            sizeof(sSensorList[number].reserved));
    return 1;
}
...
int searchDevice(char buf[], char classPath[])
{
    ...
    if ((seStatus[ID_A].isUsed == true) && (seStatus[ID_A].isFound == false)) {
        ...
        ret = getDevice(gsensorList, &gsensorInfo, buf, classPath,
                ARRAY_SIZE(gsensorList),ID_A);

        if (ret == 1) {
            seStatus[ID_A].isFound = true;
            seSensorIndex[ID_A] = sNumber;
            sNumber++;
            ret = setUncalDevice(ID_A, sNumber, ID_AU, SENSOR_TYPE_ACCELEROMETER_UNCALIBRATED,
					SENSOR_STRING_TYPE_ACCELEROMETER_UNCALIBRATED);
            if(ret == 1){
                seStatus[ID_AU].isFound = true;
                seSensorIndex[ID_AU] = sNumber;
                sNumber++;
                return 2;
            } else {
                return 1;
    ...
    if ((seStatus[ID_GY].isUsed == true) && (seStatus[ID_GY].isFound == false)) {
        ret = getDevice(gyrSensorList, &gyrSensorInfo, buf, classPath,
                ARRAY_SIZE(gyrSensorList),ID_GY);
    ...
    if ((seStatus[ID_PX].isUsed == true) && (seStatus[ID_PX].isFound == false)) {
        ret = getDevice(proSensorList, &proSensorInfo, buf, classPath,
                ARRAY_SIZE(proSensorList),ID_PX);
    ...
    if ((seStatus[ID_T].isUsed == true) && (seStatus[ID_T].isFound == false)) {
        ret = getDevice(tempSensorList, &tempSensorInfo, buf, classPath,
                ARRAY_SIZE(tempSensorList),ID_T);
