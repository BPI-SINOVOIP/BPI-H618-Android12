    ...
    mPendingUncal(0)
{
    ...
    memset(&mPendingUncalEvent, 0, sizeof(mPendingUncalEvent));
    memset(&mAccData, 0, sizeof(mAccData));

    mPendingUncalEvent.version = sizeof(sensors_event_t);
    mPendingUncalEvent.sensor = ID_AU;
    mPendingUncalEvent.type = SENSOR_TYPE_ACCELEROMETER_UNCALIBRATED;
    mPendingUncalEvent.acceleration.status = SENSOR_STATUS_ACCURACY_HIGH;
    mUser = 0;

    init_xyz_bias();

    mPendingUncalEvent.uncalibrated_accelerometer.x_bias = x_init_bias;
    mPendingUncalEvent.uncalibrated_accelerometer.y_bias = y_init_bias;
    mPendingUncalEvent.uncalibrated_accelerometer.z_bias = z_init_bias;
    ...
    if (!strncmp(ACC_DATA_NAME, "sc7", 3)) {
        UseRevise = true;
    } else {
        UseRevise = false;
    }
...
int AccelSensor::gsensor_cfg()
{
    ...
    int32_t sf_rotation = property_get_int32("ro.vendor.sf.rotation", 0);
    rotation = (sf_rotation - rotation + 360) % 360;
    switch (rotation) {
    case 90:
        direct_y = (-1) * direct_y;
...
int AccelSensor::setEnable(int32_t handle, int en) {
    ...
    if (handle != ID_A && handle != ID_O && handle != ID_M && handle != ID_AU)
        return -1;
    ...
    //update_delay();
#ifdef DEBUG_SENSOR
    ALOGD("AccelSensor enable %d ,usercount %d, handle %d ,mEnabled %d, err %d ,acclenable= %d,uncalaccel= %d",
        en, mUser, handle, mEnabled, err, mAccelEnabled, mUncalAccelEnabled);
#endif
    return 0;
}
...
bool AccelSensor::ReviseEvent() {
    ...
    mPendingUncalEvent.uncalibrated_accelerometer.x_uncalib = mPendingEvent.acceleration.x;
    mPendingUncalEvent.uncalibrated_accelerometer.y_uncalib = mPendingEvent.acceleration.y;
    mPendingUncalEvent.uncalibrated_accelerometer.z_uncalib = mPendingEvent.acceleration.z;
    return true;
}
    ...
    while (count && dataAvailable) {
        ...
        if ((type == EV_ABS) || (type == EV_REL) || (type == EV_KEY)) {
            ...
            if (mPendingMask) {
                ...
                mPendingUncalEvent.timestamp = time;
