...
public:
    ...
    int createRingRequest(WifiRequest& request) {
        ...
        if (result != WIFI_SUCCESS) {
            ALOGV("Failed to create start ring logger request; result = %d", result);
            return result;
        }
    ...
    int createRequest(WifiRequest &request) {
        ...
        switch (mType) {
            case GET_FW_VER:
            {
                ...
                if (result != WIFI_SUCCESS) {
                    ALOGV("Failed to create get fw version request; result = %d", result);
                    return result;
                }
                ...
                if (result != WIFI_SUCCESS) {
                    ALOGV("Failed to put get fw version request; result = %d", result);
                    return result;
                }
                request.attr_end(data);
                break;
            ...
            case GET_DRV_VER:
            {
                result = request.create(GOOGLE_OUI, LOGGER_GET_VER);
                if (result != WIFI_SUCCESS) {
                    ALOGV("Failed to create get drv version request; result = %d", result);
                    return result;
                }
                ...
                if (result != WIFI_SUCCESS) {
                    ALOGV("Failed to put get drv version request; result = %d", result);
                    return result;
                }
                request.attr_end(data);
                break;
            ...
            case GET_RING_DATA:
            {
                ...
                if (result != WIFI_SUCCESS) {
                    ALOGV("Failed to create get ring data request; result = %d", result);
                    return result;
                }
                ...
                result = request.put_string(LOGGER_ATTRIBUTE_RING_NAME, mRingName);
                if (result != WIFI_SUCCESS) {
                    ALOGV("Failed to put ring data request; result = %d", result);
                    return result;
                }
                request.attr_end(data);
                break;
            ...
            case GET_RING_STATUS:
            {
                ...
                if (result != WIFI_SUCCESS) {
                    ALOGV("Failed to create get ring status request; result = %d", result);
                    return result;
                }
                break;
            }
            ...
            case GET_FEATURE:
            {
                ...
                if (result != WIFI_SUCCESS) {
                    ALOGV("Failed to create get feature request; result = %d", result);
                    return result;
                }
                break;
            }
    ...
    int start() {
        ...
        if (result != WIFI_SUCCESS) {
            ALOGV("Failed to create debug request; result = %d", result);
            return result;
        }

        result = requestResponse(request);
        if (result != WIFI_SUCCESS) {
            ALOGV("Failed to register debug response; result = %d", result);
        }
        return result;
    }
            ...
            case GET_RING_STATUS:
            {
                ...
                if (it.get_type() == LOGGER_ATTRIBUTE_RING_NUM) {
                    ...
                    if (*mNumRings < num_rings) {
                        ALOGV("Not enough status buffers provided, available: %d required: %d",
                                *mNumRings, num_rings);
                    } else {
                        *mNumRings = num_rings;
                    }
                } else {
                    ALOGV("Unknown attribute: %d expecting %d",
        ...
        int buffer_size)
{
    ...
    if (buffer && (buffer_size > 0)) {
        ...
        ALOGV("FW version buffer NULL");
        return  WIFI_ERROR_INVALID_ARGS;
    }
#endif
	std::string pFirmwareVer = "XRADIO_FIRMWARE";
...
wifi_error wifi_get_driver_version(wifi_interface_handle iface, char *buffer, int buffer_size)
{
    ...
    if (buffer && (buffer_size > 0)) {
        ...
        ALOGV("Driver version buffer NULL");
        return  WIFI_ERROR_INVALID_ARGS;
    }
#endif
	std::string pDriverVer = "XRADIO_DRIVER";
        ...
        u32 *num_rings, wifi_ring_buffer_status *status)
{
    ...
    if (status && num_rings) {
        ...
        ALOGV("Ring status buffer NULL");
        return  WIFI_ERROR_INVALID_ARGS;
    }
#endif
	wifi_ring_buffer_status* pLocalstatus = NULL;
	std::string from = "XRADIO_RING";
...
public:
    ...
    virtual int cancel() {
        ...
        if (result != WIFI_SUCCESS) {
            ALOGV("failed to create reset request; result = %d", result);
            return result;
        }

        result = requestResponse(request);
        if (result != WIFI_SUCCESS) {
            ALOGV("failed to request reset; result = %d", result);
            return result;
        }
...
public:
    ...
    virtual int handleResponse(WifiEvent& reply) {
        ...
        ALOGD("len = %d", len);
        if (vendor_data == NULL || len == 0) {
            ALOGV("no vendor data in memory dump response; ignoring it");
            return NL_SKIP;
        }
        ...
        if (event_id == GOOGLE_DEBUG_MEM_DUMP_EVENT) {
            ...
            */
            ...
            if (mBuffSize) {
                ...
                if (!mBuff) {
                    ALOGV("Buffer allocation failed");
                    return NL_SKIP;
                }
                ...
                if (result != WIFI_SUCCESS) {
                    ALOGV("Failed to create get memory dump request; result = %d", result);
                    free(mBuff);
                    return NL_SKIP;
                }
                nlattr *data = request.attr_start(NL80211_ATTR_VENDOR_DATA);
                ...
                if (result != WIFI_SUCCESS) {
                    ALOGV("Failed to put get memory dump request; result = %d", result);
                    return result;
                }
                ...
                if (result != WIFI_SUCCESS) {
                    ALOGV("Failed to register get momory dump response; result = %d", result);
                }
            } else {
                ALOGV("dump event missing dump length attribute");
                return NL_SKIP;
            }
        }
        return NL_OK;
...
public:
    ...
    int start() {
        ...
        if (result != WIFI_SUCCESS) {
            ALOGV("Failed to create trigger fw memory dump request; result = %d", result);
            return result;
        }

        result = requestResponse(request);
        if (result != WIFI_SUCCESS) {
            ALOGV("Failed to register trigger memory dump response; result = %d", result);
        }
        return result;
    }
...
public:
    ...
    int createRequest(WifiRequest& request) {
        if (mReqType == TX_PACKET_FATE) {
            ...
            ALOGV("%s Unknown packet fate request\n", __FUNCTION__);
    ...
    int start() {
        ...
        int result = createRequest(request);
        if (result < 0) {
            ALOGV("Failed to create get pkt fate request; result = %d\n", result);
            return result;
        }

        result = requestResponse(request);
        if (result != WIFI_SUCCESS) {
            ALOGV("Failed to register get pkt fate response; result = %d\n", result);
        }
        return result;
    }
    ...
    int handleResponse(WifiEvent& reply) {
        ...
        if (mReqType == TX_PACKET_FATE) {
            ...
            ALOGV("Response recieved for unknown pkt fate command\n");
            return NL_SKIP;
        }

        if (vendor_data == NULL || len == 0) {
            ALOGV("no vendor data in GetPktFateCommand response; ignoring it\n");
            return NL_SKIP;
        }
