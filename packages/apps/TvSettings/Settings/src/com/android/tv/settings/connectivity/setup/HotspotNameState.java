...
/**
 * State responsible for modify hotspot name.
 */
public class HotspotNameState implements State {
    ...
    public HotspotNameState(FragmentActivity activity) {
        mActivity = activity;
    }

    @Override
    public void processForward() {
        mFragment = new HotspotNameFragment();
     ...
     */
    public static class HotspotNameFragment extends WifiConnectivityGuidedStepFragment {
        ...
        public GuidanceStylist.Guidance onCreateGuidance(Bundle savedInstanceState) {
            return new GuidanceStylist.Guidance(
                    getString(R.string.hotspot_name_title),
                    null,
                    null,
                    null);
        }
        ...
        public void onCreateActions(List<GuidedAction> actions, Bundle savedInstanceState) {
            ...
            if (config == null) {
                ...
                if (WifiHotspotParameter.hotspotname != "") {
                    title = WifiHotspotParameter.hotspotname;
                } else {
                    title = config.SSID;
                }
            }
        ...
        public long onGuidedActionEditedAndProceed(GuidedAction action) {
            if (action.getId() == GuidedAction.ACTION_ID_CONTINUE) {
                mUserChoiceInfo.put(UserChoiceInfo.HOTSPOTNAME,
                        action.getTitle().toString());
                int setNameResult = WifiHotspotParameter.setHotspotName(getActivity());
                if (setNameResult == 0) {
                    mStateMachine.getListener().onComplete(StateMachine.CONTINUE);
                } else {
                    mStateMachine.getListener().onComplete(StateMachine.SOFTAP_NAME_INVALID);
                }
            }
            return action.getId();
        }
