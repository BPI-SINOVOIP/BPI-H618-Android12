        ...
        case kWhatMediaSenderNotify:
        {
            ...
            if (what == MediaSender::kWhatInitDone) {
				...
				for (size_t i = 0; i < mTracks.size(); ++i) {
		                ...
		                converter->setVideoBitrate(encoderBitrate);
		            if(i == 0) {
...
status_t WifiDisplaySource::PlaybackSession::addAudioSource(bool usePCMAudio) {
   const audio_attributes_t remote_submix = {AUDIO_CONTENT_TYPE_MOVIE,
                                             AUDIO_USAGE_MEDIA,
    ...
    ALOGW("Unable to instantiate audio source, usePCMAudio=%d", usePCMAudio);
