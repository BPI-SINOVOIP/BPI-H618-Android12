...
status_t Converter::initEncoder() {
    ...
    int32_t videoBitrate = GetInt32Property("media.wfd.video-bitrate", 2000000);
    ...
    while (!mInputBufferQueue.empty()
        ...
        if (buffer != NULL) {
            ...
            if (mediaBuffer != NULL) {
                ...
                mEncoderInputBuffers.itemAt(bufferIndex)->meta()->setObject(
                        "mediaBufferHolder", holder);

                buffer->meta()->setObject(
                        "mediaBufferHolder", sp<MediaBufferHolder>(nullptr));
            }
        } else {
