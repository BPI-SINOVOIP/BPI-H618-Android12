...
static OMX_ERRORTYPE omxSetRole(AwOmxVdec* impl, const OMX_U8* srcRole)
{
    ...
    else if(!strncmp((char*)impl->m_cName, "OMX.allwinner.video.decoder.vp9.secure",
         OMX_MAX_STRINGNAME_SIZE))
    {
        if(srcRole != NULL &&
        strncmp((const char*)srcRole,"video_decoder.vp9", OMX_MAX_STRINGNAME_SIZE))
             ...
             OMX_MAX_STRINGNAME_SIZE))
    {
        ...
        impl->bIsSecureVideoFlag = 1;
        #else
        omx_loge("This platform does not support secure video playback");
        err = OMX_ErrorInvalidComponentName;
        #endif
    }
...
{
    if(port != NULL && getPortAllocSize(port) > 0)
    {
        OMX_U32       nIndex;
...
static inline void controlFlush(AwOmxVdec* impl, OMX_U32 portIdx)
{
    ...
    impl->m_InputNum  = 0;
    impl->m_OutputNum = 0;
    OmxVdecoderFlush(impl->pDec, portIdx);
    if (portIdx == kInputPortIndex || portIdx == OMX_ALL)
    {
        doFlushPortBuffer(impl->m_InPort);
        omx_logv("OMX_EventCmdComplete for input port OMX_CommandFlush");
        impl->m_Callbacks.EventHandler(&impl->base.mOmxComp, impl->m_pAppData,
    ...
    if (portIdx == kOutputPortIndex || portIdx == OMX_ALL)
    {
        ...
        /*
               *fix cts test: android.media.cts.AdaptivePlaybackTest#testMpeg2_eosFlushSeek,etc.
               *we should clean notify eos flag after flush completed.
               *
               *   *PAY ATTENTION TO cts fail, it is means bug*
               */
        if(impl->bVdrvNotifyEosFlag)
        {
            impl->bVdrvNotifyEosFlag = OMX_FALSE;
            omx_logi("control flush, clean eos flag...");
        }

        omx_logv("OMX_EventCmdComplete for output port OMX_CommandFlush");
        impl->m_Callbacks.EventHandler(&impl->base.mOmxComp, impl->m_pAppData,
                                        OMX_EventCmdComplete, OMX_CommandFlush,
...
static inline void controlStopPort(AwOmxVdec* impl, OMX_U32 portIdx)
{
    ...
    omx_logd("****stop port****, portIdx:%lu", portIdx);
    if (portIdx == kInputPortIndex || portIdx == OMX_ALL)
    {
        doFlushPortBuffer(impl->m_InPort);
...
static void* onMessageReceived(void* pThreadData)
{
    ...
    while(1)
    {
        if(CdcMessageQueueTryGetMessage(impl->mqMsgHandle, &msg, 5) == 0)
        {
            ...
            omx_logv("onMessageReceived, cmd:%s", cmdToString(cmd));
            switch(cmd){
...
static int decoderCallbackProcess(void* pSelf, int eMessageId, void* param)
{
    ...
    switch(eMessageId)
    {
        ...
        case AW_OMX_CB_PORT_CHANGE:
        {
            ...
            impl->m_Callbacks.EventHandler(&impl->base.mOmxComp, impl->m_pAppData,
                                            OMX_EventPortSettingsChanged, kOutputPortIndex, 0, NULL);
            break;
        }
        case AW_OMX_CB_DATASPACE_CHANGE:
        {
            impl->m_Callbacks.EventHandler(&impl->base.mOmxComp, impl->m_pAppData,
                                            OMX_EventPortSettingsChanged, kOutputPortIndex,
                                            AWOMX_IndexParamVideoDescribeColorAspects, NULL);
            break;
        }
...
static void doSubmit(AwOmxVdec *impl)
{
    ...
    impl->m_InputNum++;
    omx_logv("*****input num:%ld", impl->m_InputNum);
    impl->m_Callbacks.EmptyBufferDone(&impl->base.mOmxComp, impl->m_pAppData, pInBufHdr);
}
...
static int doDrain(AwOmxVdec *impl)
{
    ...
    omx_logv("****FillBufferDone is called, pOutBufHdr[%p],"
    ...
    impl->m_OutputNum++;
    omx_logv("*****output num:%ld", impl->m_OutputNum);
                                      ...
                                      OMX_IN OMX_PTR         pCmdData)
{
    ...
    switch (eCmd)
    {
        omx_logv(" COMPONENT_SEND_COMMAND: %d", eCmd);
                                                        ...
                                                        OMX_INOUT OMX_PTR     pParamData)
{
    ...
    omx_logv("eParamIndex = 0x%x", eParamIndex);
    if(impl->m_state == OMX_StateInvalid)
    {
    ...
    switch(eParamIndex)
    {
        ...
        {
            omx_logv(" COMPONENT_GET_PARAMETER: OMX_IndexParamVideoInit");
        ...
        case OMX_IndexParamPortDefinition:
        {
            ...
            {
                omx_loge("AwOmxVdecPortGetDefinitioin got OMX_ErrorHardware");
                impl->m_Callbacks.EventHandler(&impl->base.mOmxComp, impl->m_pAppData,
                                            OMX_EventError, OMX_ErrorHardware, 0, NULL);
            }
            break;
        ...
        {
            omx_logv(" COMPONENT_GET_PARAMETER: OMX_IndexParamStandardComponentRole");
        ...
        {
            omx_logv(" COMPONENT_GET_PARAMETER: OMX_IndexParamPriorityMgmt");
        ...
        {
            omx_logv(" COMPONENT_GET_PARAMETER: OMX_IndexParamCompBufferSupplier");
        ...
        {
            omx_logv(" COMPONENT_GET_PARAMETER: %d", eParamIndex);
        ...
        {
            omx_logv(" COMPONENT_GET_PARAMETER: OMX_IndexParamVideoProfileLevelQuerySupported");
                                                        ...
                                                        OMX_IN OMX_PTR pParamData)
{
    ...
    switch(eParamIndex)
    {
        ...
        case OMX_IndexParamPriorityMgmt:
        {
            omx_logv(" COMPONENT_SET_PARAMETER: OMX_IndexParamPriorityMgmt");
            if(impl->m_state != OMX_StateLoaded)
            {
        ...
        case OMX_IndexParamCompBufferSupplier:
        {
            omx_logv(" COMPONENT_SET_PARAMETER: OMX_IndexParamCompBufferSupplier");
            OMX_PARAM_BUFFERSUPPLIERTYPE* value  = (OMX_PARAM_BUFFERSUPPLIERTYPE*)pParamData;
            AwOmxVdecPort* port = getPort(impl, value->nPortIndex);
            if(port == NULL)
            {
                                                   ...
                                                   OMX_IN OMX_PTR pConfigData)
{
    ...
    if (impl->m_state == OMX_StateExecuting)
    {
        omx_logv("set_config: Ignore in Executing state\n");
        return OMX_ErrorNone;
    }
    ...
    switch(eConfigIndex)
    {
        default:
        {
            switch((OMX_INDEXEXTTYPE)eConfigIndex)
            {
                case OMX_IndexConfigLowLatency:
                {
                    //TODO
                    omx_loge("TODO:set OMX_IndexConfigLowLatency !");
                    break;
                }
                default:
                {
...
{
    omx_logv("param name = %s", pParamName);
    unsigned int  nIndex;
    ...
    if(impl->m_state == OMX_StateInvalid)
    {
        omx_logw("Get Extension Index in Invalid State\n");
        return OMX_ErrorInvalidState;
    }
    ...
    if(eError != OMX_ErrorNone)
        omx_logw("GetExtensionIndex %s, err [0x%x].", pParamName, eError);
    return eError;
}
                                                  ...
                                                  OMX_OUT OMX_STATETYPE* pState)
{
    ...
    *pState = impl->m_state;
    omx_logv("COMPONENT_GET_STATE, state: %s", OmxState2String(impl->m_state));
    return OMX_ErrorNone;
}
                                          ...
                                          OMX_IN    OMX_U8*                 pBuffer)
{
    ...
    omx_logv("PortIndex[%d], nSizeBytes[%d], pBuffer[%p]",
    ...
    omx_logv("pPortDef[%d]->bEnabled=%d, m_state=%s, can allocate_buffer.",
         (int)nPortIndex, def->bEnabled, OmxState2String(impl->m_state));
                                                ...
                                                OMX_IN OMX_BUFFERHEADERTYPE* pBufferHdr)
{
    ...
    omx_logv("***emptyThisBuffer: pts = %lld , videoFormat = %d, pBufferHdr = %p",
