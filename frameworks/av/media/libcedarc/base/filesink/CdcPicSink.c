...
static int InterUpdateMd5Data(CdcPicSink* pContext, CdcSinkPicture* pPic, u8 mMd5Ret[16])
{
    ...
    int mStride       = pPic->nLineStride;
    u8* pSrc = (u8*)pPic->pData[0] + (mStride * mTopOffset + mLeftOffset);
    int i = 0;
    SinkMD5* pMd5Ctx = &(pContext->mMd5Ctx);

    logd("md5 cul top:%d bot:%d left:%d right:%d",
            mTopOffset,mBottomOffset,mLeftOffset,mRightOffset);
    SinkMd5Init(pMd5Ctx);
    //data[0], for all
...
int PicSinkWritePic(CdcSinkInterface* pSelf, CdcSinkPicture* pPic)
{
    ...
    {
        loge("pointer error pic:%p file:%p", pPic,pContext->pFile);
        return -1;
    }
    ...
    nLen = pPic->nLineStride*pPic->nHeight;
    ...
    switch(pPic->nFormat)
    {
        ...
        case CDC_SINK_PIC_FORMAT_NV21:
            if(pPic->nBpp == 10)
            {
                logw("allwinner 10bit format, we just save 8bit data");
            }
...
EXIT:
    logd("save pic width:%d height:%d format:%d bpp:%d", pPic->nWidth, pPic->nHeight, pPic->nFormat, pPic->nBpp);
    pContext->mPicCount++;
...
int PicSinkSetParam(CdcSinkInterface* pSelf, CdcSinkParam* pParam)
{
    ...
    logd("start num:%d total num:%d filename:%s",
            pContext->mStartNum,pContext->mTotalNum,pContext->mFileName);
...
CdcSinkInterface* CdcPicSinkCreate(void)
{
    ...
    return (CdcSinkInterface*)(pContext);
}
