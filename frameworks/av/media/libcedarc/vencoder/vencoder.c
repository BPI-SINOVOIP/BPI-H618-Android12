...
int AddOneInputBuffer(VideoEncoder* pEncoder, VencInputBuffer* pBuffer)
{
    ...
    if(CdcIonGetMemType() == MEMORY_IOMMU && pBuffer->nShareBufFd > 0)
    {
        ...
        logv(" GetVeIommuAddr,  addrY %p. addrC %p. stride %d. height %d. aligned height %d.",
            ...
            venc_ctx->baseConfig.nStride, venc_ctx->baseConfig.nInputHeight);
    }
    else
    {
...
int AlreadyUsedInputBuffer(VideoEncoder* pEncoder, VencInputBuffer* pBuffer)
{
    ...
    if (pBuffer != NULL)
    {
        struct user_iommu_param iommu_buffer;
        iommu_buffer.fd = pBuffer->nShareBufFd;
        iommu_buffer.iommu_addr = (unsigned int)pBuffer->pAddrPhyY;
        VideoEncoderFreeVeIommuAddr(pEncoder, &iommu_buffer);
    }
    return result;
}
...
int AWJpecEnc(JpegEncInfo* pJpegInfo, EXIFInfo* pExifInfo, void* pOutBuffer, int* pOutBufferSize)
{
    ...
    if(pJpegInfo->bNoUseAddrPhy)
    {
        ...
        memcpy(inputBuffer.pAddrVirY, pJpegInfo->pAddrVirY, bufferParam.nSizeY);
        memcpy(inputBuffer.pAddrVirC, pJpegInfo->pAddrVirC, bufferParam.nSizeC);
...
//get iommu addr
        ...
        handle_ion = 0;
        iommu_buffer.fd = pJpegInfo->nShareBufFd;
...
{
    int ret = 0;
    if (crop_data->pCropMemOps != NULL &&
        crop_data->pCropIsp    != NULL &&
        crop_data->pCropVeOpsS != NULL &&
        crop_data->pCropVeOpsSelf != NULL)
    {
        logd("AWCrop has inited");
        return ret;
    }
    crop_data->pCropMemOps = MemAdapterGetOpsS();
    if (crop_data->pCropMemOps == NULL)
    {
        loge("MemAdapterGetOpsS failed");
        ret = -1;
    ...
    CdcMemOpen(crop_data->pCropMemOps);
    //* get ve ops
    int type = VE_OPS_TYPE_NORMAL;
    crop_data->pCropVeOpsS = GetVeOpsS(type);
    if (crop_data->pCropVeOpsS == NULL)
    {
        loge("get ve ops failed , type = %d", type);
        ret = -1;
        goto EXIT;
    ...
    mVeConfig.nWidth = 0;
    mVeConfig.nResetVeMode = 0;
    mVeConfig.memops = crop_data->pCropMemOps;
    crop_data->pCropVeOpsSelf = CdcVeInit(crop_data->pCropVeOpsS, &mVeConfig);
    if (crop_data->pCropVeOpsSelf == NULL)
    {
        loge("init ve ops failed");
        ret = -1;
        goto EXIT;
    }
    crop_data->pCropIsp = VideoEncIspCreate();
    if (crop_data->pCropIsp == NULL)
    {
...
void AWCropExit(aw_crop_data_t* crop_data)
{
    if (crop_data->pCropMemOps    == NULL &&
        ...
        crop_data->pCropVeOpsSelf == NULL)
    {
        logd("AWCropExit has exited");
        return;
    }
    if (crop_data->pCropVeOpsS)
    {
      CdcVeRelease(crop_data->pCropVeOpsS, crop_data->pCropVeOpsSelf);
    }
    if (crop_data->pCropMemOps)
    {
        CdcMemClose(crop_data->pCropMemOps);
    }

    if (crop_data->pCropIsp)
    {
        VideoEncIspDestroy(crop_data->pCropIsp);
    }
}
              ...
              int                nIsAlign)
{
    ...
    if ((unsigned long)pInBuffer->pAddrVirY < 0x400 ||
        (unsigned long)pOutBuffer->pAddrVirY < 0x400)
    {
    ...
    pInBuffer_tmp.pAddrVirY = CdcMemPalloc(crop_data->pCropMemOps,
                                           nInBufferSize,
                                           crop_data->pCropVeOpsS,
                                           crop_data->pCropVeOpsSelf);
    ...
    pInBuffer_tmp.pAddrVirC   =
        pInBuffer_tmp.pAddrVirY + pInBuffer_tmp.nWidth*pInBuffer_tmp.nHeight;
    pInBuffer_tmp.pAddrPhyY   =
        CdcMemGetPhysicAddress(crop_data->pCropMemOps, pInBuffer_tmp.pAddrVirY);
    pInBuffer_tmp.pAddrPhyC0 =
        pInBuffer_tmp.pAddrPhyY + pInBuffer_tmp.nWidth*pInBuffer_tmp.nHeight;
    pInBuffer_tmp.pAddrPhyC1 =
        pInBuffer_tmp.pAddrPhyC0 + pInBuffer_tmp.nWidth*pInBuffer_tmp.nHeight/4;

    int nOutBufferSize = pOutBuffer->nWidth*pOutBuffer->nHeight*3/2;
    int nOutBufferSize_tmp =
        ALIGN_XXB(16, pOutBuffer->nWidth)*ALIGN_XXB(16, pOutBuffer->nHeight)*3/2;
    pOutBuffer_tmp.pAddrVirY = CdcMemPalloc(crop_data->pCropMemOps,
                                            nOutBufferSize_tmp,
                                            crop_data->pCropVeOpsS,
                                            crop_data->pCropVeOpsSelf);
    ...
    pOutBuffer_tmp.nWidth      = ALIGN_XXB(16, pOutBuffer->nWidth);
    pOutBuffer_tmp.nHeight     = ALIGN_XXB(16, pOutBuffer->nHeight);
    pOutBuffer_tmp.colorFormat = pOutBuffer->colorFormat;
    pOutBuffer_tmp.pAddrVirC   =
        pOutBuffer_tmp.pAddrVirY + pOutBuffer_tmp.nWidth*pOutBuffer_tmp.nHeight;
    pOutBuffer_tmp.pAddrPhyY   =
        CdcMemGetPhysicAddress(crop_data->pCropMemOps, pOutBuffer_tmp.pAddrVirY);
    pOutBuffer_tmp.pAddrPhyC0  =
        pOutBuffer_tmp.pAddrPhyY + pOutBuffer_tmp.nWidth*pOutBuffer_tmp.nHeight;
    pOutBuffer_tmp.pAddrPhyC1 =
        pOutBuffer_tmp.pAddrPhyC0 + pOutBuffer_tmp.nWidth*pOutBuffer_tmp.nHeight/4;
    ...
    logv("Input:nWidth=%d, nHeight=%d, colorFormat=%d, Y=%p, C0=%p, C1=%p, "
    ...
    CdcMemFlushCache(crop_data->pCropMemOps, pInBuffer_tmp.pAddrVirY, nInBufferSize);

    VideoEncIspFunction(crop_data->pCropIsp, &pInBuffer_tmp, &pOutBuffer_tmp, &mIspFunction);

    CdcMemFlushCache(crop_data->pCropMemOps, pOutBuffer_tmp.pAddrVirY, nOutBufferSize);
    if (nIsAlign)
    {
        memcpy(pOutBuffer->pAddrVirY, pOutBuffer_tmp.pAddrVirY, nOutBufferSize_tmp);
        CdcMemFlushCache(crop_data->pCropMemOps, pOutBuffer->pAddrVirY, nOutBufferSize_tmp);
    }
    else
    {
        memcpy(pOutBuffer->pAddrVirY, pOutBuffer_tmp.pAddrVirY, nOutBufferSize*2/3);
        memcpy(pOutBuffer->pAddrVirY + nOutBufferSize*2/3,
               pOutBuffer_tmp.pAddrVirC,
               nOutBufferSize/6);
        memcpy(pOutBuffer->pAddrVirY + nOutBufferSize*5/6,
               pOutBuffer_tmp.pAddrVirC + nOutBufferSize_tmp/6,
               nOutBufferSize/6);
        CdcMemFlushCache(crop_data->pCropMemOps, pOutBuffer->pAddrVirY, nOutBufferSize_tmp);
    }
...
EXIT:
    if (crop_data->pCropVeOpsS != NULL)
    {
        ...
        {
            if (pInBuffer_tmp.pAddrVirY != NULL)
                CdcMemPfree(crop_data->pCropMemOps,
                            pInBuffer_tmp.pAddrVirY,
                            crop_data->pCropVeOpsS,
                            crop_data->pCropVeOpsSelf);
            if (pOutBuffer_tmp.pAddrVirY != NULL)
                CdcMemPfree(crop_data->pCropMemOps,
                            pOutBuffer_tmp.pAddrVirY,
                            crop_data->pCropVeOpsS,
                            crop_data->pCropVeOpsSelf);
        }
    }
