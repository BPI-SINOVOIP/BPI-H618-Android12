...
#define BIT_AWSP_FILE "/data/camera/bitstream.awsp"
...
static void printfHdr10PlusInfo(struct sunxi_metadata* pSunxi_metadata)
{
    struct hdr_10Plus_metadata *hdr_10Plus_info = &pSunxi_metadata->hdr10_plus_smetada;
    logd("-------------++metadata-hdr-10plus:flag=%d, divLut:%d, %d, %d, afbc_data:%d, %d, %d, %d, ncomponents=%d\
		targeted_system_display_maximum_luminance=%d, \
		application_version=%d, num_distributions=%d, \
		maxscl[0]=%d, maxscl[1]=%d, maxscl[2]=%d,\
		average_maxrgb=%d, \
		distribution_values[0]=%d, distribution_values[1]=%d, distribution_values[2]=%d, \
		distribution_values[3]=%d, distribution_values[4]=%d, distribution_values[5]=%d, \
		distribution_values[6]=%d, distribution_values[7]=%d, distribution_values[8]=%d, \
		distribution_values[9]=%d, knee_point_x=%d, knee_point_y=%d, \
		num_bezier_curve_anchors=%d, bezier_curve_anchors[0]=%d, \
		bezier_curve_anchors[1]=%d, bezier_curve_anchors[2]=%d, bezier_curve_anchors[3]=%d, \
		bezier_curve_anchors[4]=%d, bezier_curve_anchors[5]=%d, bezier_curve_anchors[6]=%d, \
		bezier_curve_anchors[7]=%d, bezier_curve_anchors[8]=%d", \
		pSunxi_metadata->flag,\
		hdr_10Plus_info->divLut[0][0],\
		hdr_10Plus_info->divLut[0][1],\
		hdr_10Plus_info->divLut[1][0],\
		...
		hdr_10Plus_info->targeted_system_display_maximum_luminance,\
		hdr_10Plus_info->application_version,\
		hdr_10Plus_info->num_distributions, \
		hdr_10Plus_info->maxscl[0],hdr_10Plus_info->maxscl[1], hdr_10Plus_info->maxscl[2], \
		hdr_10Plus_info->average_maxrgb, \
		hdr_10Plus_info->distribution_values[0], hdr_10Plus_info->distribution_values[1], \
		hdr_10Plus_info->distribution_values[2], hdr_10Plus_info->distribution_values[3], \
		hdr_10Plus_info->distribution_values[4], hdr_10Plus_info->distribution_values[5], \
		hdr_10Plus_info->distribution_values[6], hdr_10Plus_info->distribution_values[7], \
		hdr_10Plus_info->distribution_values[8], hdr_10Plus_info->distribution_values[9], \
		hdr_10Plus_info->knee_point_x, hdr_10Plus_info->knee_point_y, \
		hdr_10Plus_info->num_bezier_curve_anchors, \
		hdr_10Plus_info->bezier_curve_anchors[0], hdr_10Plus_info->bezier_curve_anchors[1] , \
		hdr_10Plus_info->bezier_curve_anchors[2], hdr_10Plus_info->bezier_curve_anchors[3] , \
		hdr_10Plus_info->bezier_curve_anchors[4], hdr_10Plus_info->bezier_curve_anchors[5] , \
		hdr_10Plus_info->bezier_curve_anchors[6], hdr_10Plus_info->bezier_curve_anchors[7] , \
		hdr_10Plus_info->bezier_curve_anchors[8]);
}
#endif
...
static int DebugCheckConfig(VideoDecoderContext* p)
{
    ...
    if(bSaveAwspFlag == 0)
    {
        bSaveAwspFlag = CdcGetConfigParamterInt("vdecoder_save_special_bitstream", 0);
    }

    if(bSaveStreamFlag == 1)
    {
        nSinkParm.pFileName = CdcGetConfigParamterString("vdecoder_save_bitstream_path",
                                                         BIT_STREAM_FILE);
    ...
    if(bSaveAwspFlag == 1)
    {
        nSinkParm.pFileName = CdcGetConfigParamterString("vdecoder_save_special_bitstream_path",
                                                        BIT_AWSP_FILE);
        p->pAWSPSink = CdcSinkIFCreate(CDC_SINK_TYPE_BS);
        if(p->pAWSPSink)
        {
            nSinkParm.nLen = strlen(nSinkParm.pFileName) + 1;
            CdcSinkSetParam(p->pAWSPSink, &nSinkParm);
        }
    }
    ...
    if(bSavePicFlag == 1)
    {
        ...
        nSinkParm.bMd5Open = CdcGetConfigParamterInt("vdecoder_save_md5", 0);
...
void DestroyVideoDecoder(VideoDecoder* pDecoder)
{
    ...
    if(p->pAWSPSink)
        CdcSinkIFDestory(p->pAWSPSink, CDC_SINK_TYPE_BS);
...
int InitializeVideoDecoder(VideoDecoder* pDecoder, VideoStreamInfo* pVideoInfo,  VConfig* pVconfig)
{
...
#else
    pVconfig->nVeFreq = 696;
    logd("set ve freq %d  for 8k video playback !", pVconfig->nVeFreq);
#endif
    ...
    if(p->pBSSink != NULL || p->pAWSPSink != NULL)
    {
        ...
        if(p->pAWSPSink != NULL)
        {
            ...
            CdcSinkWriteBSData(p->pAWSPSink, &nBitSteram);
        }
    }
    return 0;
}
...
static void DebugSaveBitStream(VideoDecoderContext* p, CdcSinkInterface* pSink,
        int nIndex, int nLen, u64 nPts, int nAddHead)
{
    ...
    logd("%p, %p, %p, %d",pWriteAddr, pSbmBufferStartAddr,
                          pSbmBufferEndAddr, nSbmBufferSize);

    nBitSteram.nAddHead = nAddHead;
    nBitSteram.mHead.mMask[0] = 'a';
    nBitSteram.mHead.mMask[1] = 'w';
    nBitSteram.mHead.mMask[2] = 's';
    nBitSteram.mHead.mMask[3] = 'p';
    ...
    if(pWriteAddr+nLen <= pSbmBufferEndAddr)
    {
        ...
        CdcSinkWriteBSData(pSink, &nBitSteram);
    }
    else
    {
        nBitSteram.pData = (void*)pWriteAddr;
        ...
        //second part we should not set add head
                          ...
                          int                  nStreamBufIndex)
{
    ...
    if(pDataInfo->bIsFirstPart)
    {
        if(pPartialStreamDataInfo->nLength != 0)    //* last frame is not complete yet.
        {
            ...
            if(p->pBSSink != NULL)
            {
                DebugSaveBitStream(p, p->pBSSink,nStreamBufIndex, pPartialStreamDataInfo->nLength,
                        pPartialStreamDataInfo->nPts, 0);
            }
            if(p->pAWSPSink != NULL)
            {
                DebugSaveBitStream(p, p->pAWSPSink, nStreamBufIndex, pPartialStreamDataInfo->nLength,
    ...
    if(p->pPicSink != NULL)
    {
        ...
        switch(pPicture->ePixelFormat)
        {
            ...
            case PIXEL_FORMAT_YUV_PLANER_420:
                ...
                mPic.pData[1] = pPicture->pData1;
                mPic.pData[2] = pPicture->pData2;
                break;
            ...
            case PIXEL_FORMAT_P010_VU:
                mPic.nFormat = CDC_SINK_PIC_FORMAT_P010;
                mPic.pData[0] = pPicture->pData0;
                mPic.pData[1] = pPicture->pData1;
                break;
            default:
        ...
        mPic.nLineStride  = pPicture->nLineStride;
        mPic.nBpp    = (pPicture->b10BitPicFlag == 1) ? 10 : 8;
...
VideoPicture* RequestPicture(VideoDecoder* pDecoder, int nStreamIndex)
{
    ...
    if(pPicture != NULL)
    {
        ...
        #if(CHECK_METADATA_INFO)
        printfHdr10PlusInfo((struct sunxi_metadata* )pPicture->pMetaData);
...
//    logv("ValidPictureNum, nStreamIndex=%d", nStreamIndex);
    ...
    if(pFbm == NULL)
    {
        ...
        logv("Fbm module of video stream %d not create yet, ValidPictureNum() fail.", nStreamIndex);
        if(pFbm == NULL)
        {
            return 0;
        }
...
int EmptyPictureNum(VideoDecoder* pDecoder, int nStreamIndex) {
    ...
    if(pFbm == NULL)
    {
        ...
        logv("Fbm module of video stream %d not create yet, %s fail.", nStreamIndex, __func__);
        if(pFbm == NULL)
        {
            return 0;
        }
    }
    return FbmEmptyPictureNum(pFbm);
}
int ReleasePictureNum(VideoDecoder* pDecoder, int nStreamIndex) {
    ...
    return FbmReleasePictureNum(pFbm);
}
...
VideoPicture* RequestReleasePicture(VideoDecoder* pDecoder)
{
    ...
    if(p->pVideoEngine != NULL)
    {
        ...
        if((CdcIonGetMemType()==MEMORY_IOMMU)&&(pVpicture!=NULL))
        {
            ...
            sUserIommuBuf.iommu_addr = pVpicture->phyYBufAddr;
...
int SetReleasePicture(VideoDecoder* pDecoder, VideoPicture* pPicture)
{
    ...
    logv("%s, pPicture=%p", __func__, pPicture);
    ...
    ret = FbmSetReleasePicture(pFbm, pPicture);
    if(ret != 0)
    {
        logw("FbmSetReleasePicture return fail,\
                it means the picture being returned it not one of this FBM.");
    }
    return ret;
}
