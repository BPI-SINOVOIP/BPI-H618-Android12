...
nWidth=%d, nHeight=%d nAlignStride = %d",
        ...
        (pFbmCreateInfo->nBufferType!=BUF_TYPE_ONLY_REFERENCE))
    {
        ...
        else
        {
            ...
            /*
            there is no need to increase buffer size in decoder,
            when use gpu to support multiple out.
        */
#if 0
    ...
    else
    {
        ...
        for(i=0; i<pFbm->nMaxFrameNum; i++)
        {
            if(CdcIonGetMemType() == MEMORY_IOMMU)
            {
                ...
                sUserIommuBuf.iommu_addr = pFbm->pFrames[i].vpicture.phyYBufAddr;
...
int FbmEmptyPictureNum(Fbm* pFbm)
{
    VideoFbmInfo*  pFbmInfo = NULL;
    Fbm* ppFbm = NULL;
    int nEmptyBufferNum = 0;
...
int FbmReleasePictureNum(Fbm* pFbm)
{
    VideoFbmInfo*  pFbmInfo = NULL;
    Fbm* ppFbm = NULL;
    int nReleaseBufferNum = 0;
    if(pFbm == NULL)
        return 0;
    pFbmInfo = (VideoFbmInfo*)pFbm->pFbmInfo;
    if(pFbmInfo == NULL)
    ...
    nReleaseBufferNum = ppFbm->nReleaseBufferNum;
    return nReleaseBufferNum;
}
            ...
            VideoPicture* pPicture, int bForbidUseFlag)
{
    ...
    if(pFbmInfo->pFbmBufInfo.bIsSoftDecoderFlag == 0)
    {
    ...
    pFrameNode->vpicture.nColorPrimary = 0xffffffff;
#ifdef CONF_VE_MULTIPLE_OUTPUT
    if(pFbmInfo->pFbmBufInfo.bAfbcModeFlag==1)
    {
        int mFbmAfbcBufferSize = FbmComputeAfbcBufferSize(&(pFrameNode->vpicture),
            pFrameNode->vpicture.b10BitPicFlag);
        pFrameNode->vpicture.pData2 = (char*)(pPicture->phyYBufAddr+ mFbmAfbcBufferSize);
        if(pFrameNode->vpicture.nAfbcSize != mFbmAfbcBufferSize)
        {
            pFrameNode->vpicture.nAfbcSize = mFbmAfbcBufferSize;
        }
    }
    else
    {
...
int FbmNewDispRelease(Fbm* pFbm)
{
    ...
    while(1)
    {
        ...
        if(pFrameNode == NULL)
        {
            logd("FbmDequeue from pEmptyBufferQueue is null!!!!!");
            break;
        }
        else
        {
...
int FbmSetReleasePicture(Fbm* pFbm, VideoPicture* pVPicture)
{
    int        index;
    FrameNode* pFrameNode;
    VideoFbmInfo*  pFbmInfo = NULL;
    ...
    dy_log("****%s pVPicture=%p, nId = %d", __func__, pVPicture, pVPicture->nID);
    if(pFbmInfo->bTwoStreamShareOneFbm == 1)
    {
        if(pFbm == pFbmInfo->pFbmFirst)
        {
    ...
    if(i == pFbm->nMaxFrameNum)
    {
        loge("%s: cannot find this picture", __func__);
        return -1;
    }
    index = pFbm->pFrames[i].vpicture.nID;
    ...
    if(index < 0 || index >= pFbm->nMaxFrameNum)
    {
        ...
        logw("%s: the picture id is invalid, pVPicture=%p, pVPicture->nID=%d",\
                __func__, pVPicture, pVPicture->nID);
        return -1;
    }
