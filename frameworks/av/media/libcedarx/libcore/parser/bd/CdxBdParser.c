...
next_child:
    if(m == 3)
    {
        CDX_LOGV("bdParser->status = PSR_EOS");
    ...
    {
        CDX_LOGV("child[%d]== NULL", bdParser->prefetchType);
        m++;
...
_prefetch:
    ...
    if(ret < 0)
    {
        ...
        if(mErrno == PSR_EOS)
        {
            CDX_LOGV("child EOS, bdParser->prefetchType = %d", bdParser->prefetchType);
            ...
            else if(bdParser->prefetchType == 0)
            {
                ...
                CDX_LOGV("curSeqNum = %d", bdParser->curPlayItemIndex);
                ...
                CDX_LOGV("child[0](%s)", bdParser->fileUrl);
            ...
            else
            {
                ...
                CDX_LOGV("curSeqNum = %d", bdParser->curSubPlayItemIndex);
                ...
                int fd;
                sprintf(bdParser->fileUrl, "STREAM/%s.m2ts", clip->ClipName);
                CDX_LOGV("child[1](%s)", bdParser->fileUrl);
                fd = ExtOpen(bdParser->cdxStream, bdParser->fileUrl);
                if (fd == -1)
                {
                    CDX_LOGE("ExtOpen failure");
...
//    CDX_LOGD("bdParser->prefetchType = %d", bdParser->prefetchType);
    ...
    //CDX_LOGW("pts1:%lld, pts0:%lld!!, bdParser->prefetchType:%d",
    //        bdParser->streamPts[1], bdParser->streamPts[0], bdParser->prefetchType);

    // maybe stream0 has jumping backward pts, but stream1 has right pts,
    // then it will judge the prefetchType=0 next BdParserPrefetch()
    // which is wrong prefetchType
    if(bdParser->child[1]
    ...
    // maybe stream0 has jumping forward pts, but stream1 has rigth pts,
    // then it will judge the prefetchType=1 next BdParserPrefetch()
    // which is wrong prefetchType  --fix by gqy
    // it will work for jumping backward pts of stream0 too.
    if(bdParser->prefetchType == 1 && bdParser->child[1]
        && bdParser->streamPts[1] + 2000000LL < bdParser->streamPts[0])
    {
        CDX_LOGW("maybe the stream0 has something wrong, pts1:%lld, pts0:%lld!!",
            bdParser->streamPts[1], bdParser->streamPts[0]);
        bdParser->streamPts[0] = 0;
    }
...
cdx_int32 BdParserSeekTo(CdxParserT *parser, cdx_int64  timeUs, SeekModeType seekModeType)
{
    ...
    CDX_LOGV("BdParserSeekTo start, timeUs = %lld", timeUs);
    CdxBdParser *bdParser = (CdxBdParser *)parser;
    ...
    CDX_LOGV("timeUs=%lld, mDurationUs=%lld, diff=%lld", timeUs, mDurationUs, diff);
...
//    bdParser->streamCtl.param = &bdParser->streamSeekPos;
    ...
    if(bdParser->child[1])
    {
        ...
        {
            stream = clip->ps->streams + i;
            CDX_LOGV("clip=%p, clip->ps=%p, clip->ps->streams=%p, stream = %p",
...
int SelectPlaylists(CdxBdParser *bdParser)
{
    ...
    if(maxPlayTime > 30*60*1000*1000)
    {
        CdxListForEachEntrySafe(playlist, playlist1, &bdParser->playlists, node)
        {
            ...
            CDX_LOGV("SelectPlaylists playlist->durationUs = %lld", playlist->durationUs);
    ...
    for(i = 0; i < clip->number_of_program_sequences; i++)
    {
        ...
        for(j = 0; j < ps->number_of_streams_in_ps; j++)
        {
               ...
               stream->coding_type == 0x20)//从流 MVC
            {
                ...
                {
                    CDX_LOGV("MVC Dependent view video(%d)",
...
int processMpls(CdxBdParser *bdParser, Playlist *playlist)
{
    ...
    else
    {
        ...
        CDX_LOGV("bdParser->fileUrl(%s)", bdParser->fileUrl);
        ret = OpenFile(bdParser, bdParser->fileUrl);
        if(ret < 0)
        {
            CDX_LOGW("open file fail:%s", bdParser->fileUrl);
...
//    char *tmp = strstr(bdParser->originalUrl, "BDMV/");
    ...
    CdxListForEachEntrySafe(playlist, playlist1, &bdParser->playlists, node)
    {
        CDX_LOGV("playlist->durationUs = %lld", playlist->durationUs);
    ...
    CDX_LOGV("curPlaylist=%s", playlist->mplsFileName);
...
//    bdParser->cdxDataSource.extraDataType = EXTRA_DATA_APPOINTED_TS;
    ...
    if(playlist->subPath)
    {
        ...
        sprintf(bdParser->fileUrl, "STREAM/%s.m2ts", clip->ClipName);
        CDX_LOGV("child[2](%s)", bdParser->fileUrl);
        fd = ExtOpen(bdParser->cdxStream, bdParser->fileUrl);
        if (fd == -1)
        {
            CDX_LOGE("ExtOpen failure");
