...
cdx_bool read_frame_header_(cdx_uint8 *src,FlacParserImplS *impl)
{
    ...
    {
        CDX_LOGV("orgSampsPerFrm : %d, min : %d, max : %d",
...
static int FlacInit(CdxParserT *flac_impl)
{
    ...
    {
        CDX_LOGV("After seek is not right header,we need resync!");
        while(1)
        {
            buf_ptr ++;
            syncoff ++;
...
SUCESS:
    ...
    CDX_LOGV("impl->extradata_size : %d, impl->extradata : %p, impl->file_offset :%lld",
