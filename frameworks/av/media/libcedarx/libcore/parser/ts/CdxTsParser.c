    ...
    //类似于new_pes_av_stream
{
    ...
    {
        Program *program = mStream->mProgram;
        logi("program->format: %d", program->format);
        if(program->format != 1) //NOT HDMV
        {
    ...
    */
    switch(descriptor_tag)
    {
        ...
        case 0xA0:
            ...
            CDX_LOGV("dra_version_flag = %u", dra_version_flag);
...
{
    CdxMediaInfoT *mediainfo = NULL;
    CdxMediaInfoT *tmp = NULL;
    cdx_int32 ret = 0;
                ...
                || mTSParser->needUpdateProgram))
    {
        ...
        if(mTSParser->needSelectProgram && mTSParser->needUpdateProgram)
        {
            ...
            if(programHasVideo || allProgramHasStream)
            {
                ...
                mediainfo = (CdxMediaInfoT *)CdxMalloc(sizeof(CdxMediaInfoT));
                if(!mediainfo)
                {
                ...
                {
                     tmp = (CdxMediaInfoT *)CdxMalloc(sizeof(CdxMediaInfoT));
                     if (tmp == NULL)
                     {
                        CDX_LOGE("malloc failed!");
                        ret = -1;
...
cdx_int32 SeekToTime(TSParser *mTSParser, cdx_int64 timeUs, cdx_uint32 mode)
{
    ...
    if(mTSParser->seekMethod == 0)
    {
        ...
        do
        {
            ...
            cnt++;
            seekto_next_pos = file_pos + tmp;
            if(CdxStreamSeek(cdxStream, seekto_next_pos, SEEK_SET) < 0)
            {
                CDX_LOGE("CdxStreamSeek fail");
                break;
            }
