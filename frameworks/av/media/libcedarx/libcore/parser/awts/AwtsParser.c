...
static int AwtsReadStreamInfo(AwtsParser *awtsParser)
{
    ...
    CDX_LOGV("Length = 0x%x %x %x %x %x %x %x",
             tmp[0], tmp[1], tmp[2], tmp[3], tmp[4], tmp[5], tmp[6]);
    Length  = GetBE32Bits(data);
    CDX_LOGV("Length = 0x%x", Length);
    Version = Get8Bits(data);
    Flags   = Get8Bits(data);
    TrackCount = Get8Bits(data);
    CDX_LOGV("TrackCount = 0x%x", TrackCount);
    while(TrackCount--)
    {
...
_sync:
    ...
    {
        CDX_LOGV("Maybe Probe data is not enough. len(%u)", probeData->len);
        return 0;
    }
    sp += 9;
    ...
    if(packetFlags & (1<<7))
    {
        ...
        {
            CDX_LOGV("AwtsParserProbe = 1");
            return (probeData->len - sp)*100/probeData->len;
        }
        else
        {
            CDX_LOGV("it is not awts.");
            return 0;
        }
    }
    else
