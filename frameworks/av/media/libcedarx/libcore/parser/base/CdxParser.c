...
/*************** for debug start ***********************/
#ifdef __ANDROID__
//#define ENABLE_RAW_STREAM_PARSER
...
extern CdxParserCreatorT wvParserCtor;
static struct ParserUriKeyInfoS wvKeyInfo =
{
    "wv",
    {NULL},  /*scheme*/
    {".wv"}, /*suffix*/
...
extern CdxParserCreatorT mmfParserCtor;
static struct ParserUriKeyInfoS mmfKeyInfo =
{
    "mmf",
    {NULL}, /*scheme*/
    {".mmf"}, /*suffix*/
...
#ifdef ENABLE_RAW_STREAM_PARSER
...
void AwParserInit(void)
{
    ...
    AwParserRegister(&mmfParserCtor, CDX_PARSER_MMF, &mmfKeyInfo);
    AwParserRegister(&wvParserCtor, CDX_PARSER_WV, &wvKeyInfo);
    ...
    CDX_LOGV("aw parser size:%d",parserList.size);
    return ;
}
...
out:
    ...
    if (retPsrNode)
    {
        CDX_LOGV("I think it's something about '%s', type id(%d)",
            retPsrNode->keyInfo->comment ? retPsrNode->keyInfo->comment : "unknow",
            retPsrNode->type);
    }
    else
    {
        CDX_LOGV("Sorry, I don't know what it is!");
    }
    return retPsrNode;
}
...
probe:
    ...
    if (psrNode)
    {
        ...
        else
        {
            ...
            {
                CDX_LOGV("parser node maybe id3v2.");
                parserNode = malloc(sizeof(*parserNode));
                if(!parserNode){
                    CDX_LOGV("malloc id3v2 parser node failed.");
                    return NULL;
                }
...
found:
    ...
    CDX_LOGV("parser type(%d)", parser->type);
    return parser;
}
