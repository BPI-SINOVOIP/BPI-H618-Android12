                                  ...
                                  cdx_int32 isWide, cdx_int32 *frameSize)
{
    ...
    if (ret < 1) {
        CDX_LOGW("Sync head data no enough! offset : %lld", offset);
...
static int AmrInit(CdxParserT* parameter)
{
    ...
    if(ret!=6){
        CDX_LOGW("First 6 byte no enough!");
        goto OPENFAILURE;
    }
    impl->file_offset+=6;
    offset+=6;
    if(memcmp(d,AMR_header,6))
    {
        ...
        if(ret!=3){
            CDX_LOGW("Extern 3 byte no enough!");
            goto OPENFAILURE;
        }
        impl->file_offset+=3;
        offset+=3;
...
static cdx_int32 __AmrParserPrefetch(CdxParserT *parser, CdxPacketT *pkt)
{
    ...
    if(rc == AMR_LACK_OF_DATA){
        CDX_LOGW("AMR_LACK_OF_DATA Prefetch while lack of data...");
    }
...
static cdx_int32 __AmrParserSeekTo(CdxParserT *parser, cdx_int64 timeUs, SeekModeType seekModeType)
{
    ...
    CDX_LOGV("offset : %lld,  fileSize : %lld", offset, impl->fileSize);
    for (i = 0; i< frames - index * 50; i++) {
        rc = AmrGetFrameSizeByOffset(impl, offset,impl->is_wide, &framesize);
        if (rc == AMR_LACK_OF_DATA) {
            CDX_LOGW("AMR_LACK_OF_DATA seek frame but lack of data!!");
            break;
        }
        else if(rc == AMR_UNEXPECTED_ERROR) {
...
static cdx_int32 AmrProbe(CdxStreamProbeDataT *p)
{
    ...
    if(memcmp(d,AMR_header,6))
    {
        if(p->len<9)
        {
            CDX_LOGW("AMRWB_header data is not enough.");
            return CDX_FALSE;
        }
        if(memcmp(d,AMRWB_header,9))
            return CDX_FALSE;
...
static cdx_uint32 __AmrParserProbe(CdxStreamProbeDataT *probeData)
{
    ...
    if(probeData->len < 6)
    {
        CDX_LOGV("Probe AMR_header data is not enough.");
        return 0;
    }

    if(!AmrProbe(probeData))
    {
        CDX_LOGV("amr probe failed.");
        return 0;
    }
    return 100;
}
