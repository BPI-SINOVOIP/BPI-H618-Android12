...
int VideoDecCompSetVideoStreamInfo(VideoDecComp* p, VideoStreamInfo* pStreamInfo, VConfig* pVconfig)
{
    ...
    if(pStreamInfo->eCodecFormat == VIDEO_CODEC_FORMAT_H264
        &&(pStreamInfo->nWidth >= 1920 ||pStreamInfo->nHeight >= 1080))
    {
        pVconfig->bAdapteDropFrame = 1;
        logv("the video format is H264, and nWidth = %d,  nHeight = %d, bAdapteDropFrame = %d",
             pStreamInfo->nWidth, pStreamInfo->nHeight,pVconfig->bAdapteDropFrame);
    }
