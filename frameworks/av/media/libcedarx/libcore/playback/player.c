...
#define FAST_MODE_DROP_TIME (50*1000)
...
Player* PlayerCreate(void)
{
    ...
    p->nPlayRate.mStretchMode = XAUDIO_TIMESTRETCH_STRETCH_DEFAULT;
    p->nPlayRate.mFallbackMode = XAUDIO_TIMESTRETCH_FALLBACK_DEFAULT;
...
#if CONFIG_DISABLE_VIDEO
    int param[2];
    param[0] = PLAYER_MEDIA_INFO_DISABLE_VIDEO;
    p->callback(p->pUserData, PLAYBACK_DISABLE_MEDIA, (void*)param);
    return 0;
#else
        ...
        int nStreamNum, int nDefaultStream)
{
    ...
    CEDARX_UNUSE(nDefaultStream);
    int param[2];
    param[0] = PLAYER_MEDIA_INFO_DISABLE_AUDIO;
    p->callback(p->pUserData, PLAYBACK_DISABLE_MEDIA, (void*)param);
    return 0;
#else
    int i = 0;
...
{
    logv("PlayerSetPlayBackSettings");
...
int PlayerStart(Player* pl,int bSeekClosest,int nSeekTimeMs)
{
    ...
    else
    {
        ...
        if(p->pLayerCtrl)
        {
            LayerControl(p->pLayerCtrl, CDX_LAYER_CMD_SET_HOLD_BY_GPU_BUFFER_NUM,
...
{
    PlayerContext* p = (PlayerContext*)pl;
    p->mFlag |= flag;
}
...
#if CONFIG_DISABLE_SUBTITLE
    int param[2];
    param[0] = PLAYER_MEDIA_INFO_DISABLE_SUBTITLE;
    p->callback(p->pUserData, PLAYBACK_DISABLE_MEDIA, (void*)param);
    return 0;
#else
    if(p->pSubtitleStreamInfo != NULL && p->nSubtitleStreamNum > 0)
...
static int CallbackProcess(void* pSelf, int eMessageId, void* param)
{
    ...
    switch(eMessageId)
    {
        ...
        case PLAYER_VIDEO_RENDER_NOTIFY_ERROR:
        {
            p->callback(p->pUserData, PLAYBACK_NOTIFY_VIDEO_UNSUPPORTED, NULL);
            loge("==== video render internal error === ");
            return 0;
        }
        ...
        case PLAYER_AUDIO_RENDER_NOTIFY_FIRST_FRAME:
        {
            ...
            if(p->bInFastMode)
            {
                ...
                if(p->nPreVideoPts == -1 || nAudioPts < (p->nPreVideoPts - FAST_MODE_DROP_TIME))
                {
                    logd("TIMER_DROP_AUDIO_DATA, nAudioPts=%lld ms, p->nPreVideoPts=%lld ms, diff=%lld ms",
                        nAudioPts/1000, p->nPreVideoPts/1000, (p->nPreVideoPts-nAudioPts)/1000);
                    p->nFirstAudioRenderPts = -1;
                    return TIMER_DROP_AUDIO_DATA;
                }
                else
                    ...
                    if(p->nFirstVideoRenderPts != -1)
                    {
                        ...
                        else    //* p->nFirstVideoRenderPts > p->nFirstAudioRenderPts
                        {
                            ...
                            else    //* time difference is more than threshold to judge pts jump.
                            {
                                ...
                                logw("nFirstAudioRenderPts %" PRId64 ", nFirstVideoRenderPts %"
                                    PRId64 ", diff time (%lld),""difference too big.",
                                    p->nFirstAudioRenderPts,p->nFirstVideoRenderPts,
                                    llabs(p->nFirstAudioRenderPts - p->nFirstVideoRenderPts));
        ...
        case PLAYER_VIDEO_RENDER_NOTIFY_PICTURE_PTS:
        {
            ...
            if(!IS_DTMB_STREAM || (PlayerHasAudio((Player*)p) && (p->bAudioCrash == 0)))
            {
                if(CdxAbs(nTimeDiff) <= CONFIG_THREASHOLD_OF_PTS_DIFFERENCE_TO_JUDGE_PTS_jUMP ||
                   fabs(p->nPlayRate.mSpeed - 1.0) > 0.000001)
                {
        ...
        case PLAYER_AUDIO_RENDER_NOTIFY_PTS_AND_CACHETIME:
        {
            ...
            if(p->bInFastMode)
            {
                ...
                //* in fast mode, video is showed without any synchronization,
                //* audio data is discard if overtime.
                if(p->nPreAudioPts < (p->nPreVideoPts - FAST_MODE_DROP_TIME))
                {
                    logd("TIMER_DROP_AUDIO_DATA, nAudioPts=%lld ms, p->nPreVideoPts=%lld ms, diff=%lld ms",
                        nAudioPts/1000, p->nPreVideoPts/1000, (p->nPreVideoPts-nAudioPts)/1000);
                    p->nFirstAudioRenderPts = -1;
            ...
            else
            {
                   ...
                   (p->nLastTimeTimerAdjusted > nAudioPts))
                {
                    ...
                    if (fabs(p->nPlayRate.mSpeed - 1.0) > 0.000001)// !=1.0
                    {
                        if(nTimeDiff > 50000)
...
static int PlayerInitialVideo(PlayerContext* p)
{
...
#if defined(CONF_DI_300_SUPPORT) //tnr needed
    vd_outfmt = "yv12";
#endif
    ...
    p->sVideoConfig.bSecureosEn = p->pVideoStreamInfo->bSecureStreamFlag;
    if(p->pVideoStreamInfo->nWidth <= 2048 && p->pVideoStreamInfo->nHeight <= 1088 &&
        p->pVideoStreamInfo->nFrameRate <= 30000)
    {
        p->sVideoConfig.nDisplayHoldingFrameBufferNum = GetConfigParamterInt("pic_4list_num", 3);
    }
    else
    {
        p->sVideoConfig.nDisplayHoldingFrameBufferNum = 3;
    }
    ...
    VideoDecCompSetFlag(p->pVideoDecComp, p->mFlag);
...
void PlayerSetDiDemo(Player* pl, int enable, int left, int top, int right, int bottom)
{
    PlayerContext* p;
    p = (PlayerContext*)pl;
    ...
    if(p->pVideoRender != NULL)
    {
        return VideoRenderCompSetDiDemo(p->pVideoRender, enable, left, top, right, bottom);
    }
    return;
}

void PlayerSetDiTnr(Player* pl, int enable, int mode, int strength)
{
    PlayerContext* p;
    p = (PlayerContext*)pl;
    ...
    if(p->pVideoRender != NULL)
    {
        return VideoRenderCompSetDiTnr(p->pVideoRender, enable, mode, strength);
    }
    return;
}

void PlayerSetDiFmd(Player* pl, int enable)
{
    PlayerContext* p;
    p = (PlayerContext*)pl;
    ...
    if(p->pVideoRender != NULL)
    {
        return VideoRenderCompSetDiFmd(p->pVideoRender, enable);
    }
    return;
}
#endif
