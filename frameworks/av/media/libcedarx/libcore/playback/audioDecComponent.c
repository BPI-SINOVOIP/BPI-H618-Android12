...
/*
  value=80 is an Empirical value for StreamManagerFlushSteamWithHoldFrames()
  if the value is too big, it will play rewind when starting
  if the value is too little, it will be a bad effect when fast switching stream
*/
#define MAX_AUDIO_STREAM_HOLDING_SUBMITTED_FRAMES (80)
...
AudioDecComp* AudioDecCompCreate(void)
{
    ...
    logv("libadecoder.so dlopen success, to load internal symbols...");
    ...
    while(idx < ALIB_FUNC_NUM)
    {
        ...
        logv("func(%s) link success...", ALIB_FUNCS[idx].name);
    ...
    if (pFrame != NULL)
    {
        ...
        if ((nBufSize0+nBufSize1) >= pFrame->nLength)
        {
            ...
            p->AlibItf.ParserUpdateBsBuffer(p->AlibItf.libadecoder,
                                 ...
                                 p->nOffset);
            //StreamManagerFlushStream(pSm, pFrame);
            StreamManagerFlushStreamWithHoldFrames(pSm, MAX_AUDIO_STREAM_HOLDING_SUBMITTED_FRAMES);
