...
int DlOpenPlugin(CdxPluginNodeS *plugin)
{
    ...
    if(plugin->comment != NULL)
        logv("plugin %s comment is \"%s\"",plugin->id,plugin->comment);
    logv("plugin open lib: %s",plugin->lib);
    ...
    if (plugin->init != NULL && strcasecmp(plugin->init, "") != 0)
    {
        ...
        logv("plugin init : %s", plugin->init);
...
CdxPluginNodeS* ReadPluginEntry(const char *entry)
{
    ...
    if (ret == 1)
    {
        logv("read plugin entry %s ok.",entry);
    ...
    {
        loge("read plugin entry %s fail!",entry);
    }
...
void CdxPluginLoadList(CdxPluginListS *loadlist,const char *key)
{
    ...
    while (1)
    {
        ...
        if (pluginNode)
        {
            ...
            if(pluginNode->exit)
            {
                if (strncmp(pluginNode->exit,"exit",sizeof("exit")))
                    break;
            }
            loadlist->size++;
            index++;
        ...
        {
            logv("pluginNode = NULL,the loading is complete or an exception occurs");
            break;
        }
    }

    logv("have config %d entry",loadlist->size);
    logv("start to open %s lib",key);
    ...
    CdxListForEachEntry(pluginNode, &loadlist->list, node)
    {
        if (pluginNode->isload != 1)
        {
            ...
            if (ret != 1)
            {
                loge("load %s id %s fail!",key,pluginNode->id);
            }
        }
    }
}
