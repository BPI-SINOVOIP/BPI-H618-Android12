...
//static struct timeval mTimeStart;
//static struct timeval mTimeEnd;
...
static int ParserCallbackProcess(void* pUserData, int eMessageId, void* param)
{
    ...
    switch(eMessageId)
    {
        ...
        case STREAM_EVT_DOWNLOAD_ERROR:
        {
            ...
            demux->callback(demux->pUserData, nMsg, (void*)&cur);
            logv("demux: STREAM_EVT_DOWNLOAD_ERROR, statusCode(%d)", cur.statusCode);
            break;
        }
        ...
        case STREAM_EVT_NET_DISCONNECT:
        {
            ...
            {
                logd("STREAM_EVT_NET_DISCONNECT, param = %d", flag);
                demux->bDisconnet = 1;
            }
...
int DemuxCompStart(DemuxComp* d)
{
    ...
    if(pthread_equal(pthread_self(), demux->threadId)
        || (demux->cacheThreadId && pthread_equal(pthread_self(), demux->cacheThreadId)))
    {
...
int DemuxCompSeekTo(DemuxComp* d, int nSeekTimeMs, SeekModeType nSeekModeType)
{
    ...
    demux->bCancelSeek = 0;
    demux->bSeeking = 1;
    if(demux->pParser != NULL && (demux->eStatus == DEMUX_STATUS_STARTED || demux->eStatus == DEMUX_STATUS_PREPARED))
...
//
            ...
            logv("--- demux->shiftedTimeUrl = %s", demux->shiftedTimeUrl);
...
_endTask:
                ...
                if(demux->cacheThreadId != 0)
                {
                    ...
                    if(nSeekTimeFinal < 0)
                    {
                        ...
                        while(demux->nCacheReply)
                        {
                            ...
                            {
                                logv("send buffer Start, in demux thread");
                                unsigned char url[1024];
                                if(demux->pParser)
                                {
        ...
        else if(msg.messageId == DEMUX_COMMAND_RESET_URL)
        {
            ...
            logv("ShiftedTimeUrl = %s", demux->shiftedTimeUrl);
        ...
        else if(msg.messageId == DEMUX_COMMAND_READ)
        {
            ...
            else
            {
                ...
                while(1)
                {
                    ...
                    {
                        //* no buffer, try to wait sometime.
                        const char* strMediaType = (ePlayerMediaType == MEDIA_TYPE_VIDEO ? "video":
                                (ePlayerMediaType == MEDIA_TYPE_AUDIO ? "audio" : "other(subtitle)"));
                        logi("waiting for %s stream buffer.", strMediaType);
                        ret = AwMessageQueueTryGetMessage(demux->mq, &msg, 200);
                        if(ret == 0)    //* new message come, quit loop to process.
                            goto process_message;
                    }
...
reopen:
            ...
            do {
                ...
                if(ret < 0)
                {
                    ...
                    logv("get status code: %zu", statusCode);
            ...
            else
            {
                ...
                if(ret == 0)
                {
                    ...
                    if(packet.type == CDX_MEDIA_VIDEO)
                    {
                        ...
                        {
                            logv("cache -- video stream info change: "
                    ...
                    else if(packet.type == CDX_MEDIA_AUDIO)
                    {
                        ...
                        {
                            logv("cache -- audio stream info change: "
                                    "preInfoVersion = %d, curInfoVersion = %d",
                                    nAudioInfoVersion, packet.infoVersion);
