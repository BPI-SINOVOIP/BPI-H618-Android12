...
#if (CONF_ANDROID_MAJOR_VER >= 8 && CONF_ANDROID_MAJOR_VER <= 10)
...
static const char *GOOGLE_PLAY_STORE_APP_NAME = "com.android.vending";
...
AwPlayer::AwPlayer()
{
    ...
    if(mPriData->mXPlayer != NULL)
    {
        ...
        if (!strcmp(mPriData->strApkName, GOOGLE_PLAY_STORE_APP_NAME))
            XPlayerSetFlag(mPriData->mXPlayer, AW_FLAG_NOT_USE_AFBC);
...
#if (CONF_ANDROID_MAJOR_VER == 8 || CONF_ANDROID_MAJOR_VER ==9)
    ...
    mPriData->mAnalyticsItem = MediaAnalyticsItem::create(kKeyPlayer);
    mPriData->mAnalyticsItem->generateSessionID();
    mPriData->pBuffering = NULL;
#endif
#if (CONF_ANDROID_MAJOR_VER >= 11)
    mPriData->mAnalyticsItem = mediametrics::Item::create(kKeyPlayer);
    //mediametrics_setUid(mPriData->mAnalyticsItem, mMetrics.GetAppUid());
    mPriData->pBuffering = NULL;
#endif
}
...
status_t AwPlayer::setDataSource(int fd, int64_t offset, int64_t length)
{
    ...
    else if (!strcmp(mPriData->strApkName, TVD_FILE_MANAGER_APP_NAME))
    {
        ...
        XPlayerSetScaleDown(mPriData->mXPlayer, 1);
    }
#endif
    else
        mPriData->mConfigInfo.appType = APP_DEFAULT;
...
status_t AwPlayer::prepareAsync()
{
    ...
    {
        logv("++++++++ IPTV_PLAYER_CONTENTMODE_LETTERBOX");
        displayRatio = 0;
    }
    else
    {
        logv( "+++++++ IPTV_PLAYER_CONTENTMODE_FULL");
        displayRatio = 1;
    }
...
#endif
{
   ...
   if(mPriData->mLogRecorder != NULL)
    ...
    switch(mode)
    {
        case MediaPlayerSeekMode::SEEK_PREVIOUS_SYNC:
            nSeekModeType = AW_SEEK_PREVIOUS_SYNC;
            break;
        case MediaPlayerSeekMode::SEEK_NEXT_SYNC:
            nSeekModeType = AW_SEEK_NEXT_SYNC;
            break;
        case MediaPlayerSeekMode::SEEK_CLOSEST_SYNC:
            nSeekModeType = AW_SEEK_CLOSEST_SYNC;
            break;
        case MediaPlayerSeekMode::SEEK_CLOSEST:
            nSeekModeType = AW_SEEK_CLOSEST;
            break;
        default:
            loge("unknow media player seek mode!");
    ...
    if (mPriData->mConfigInfo.livemode == 0)
    {
        ...
        {
            logv("change seek position from %d to %d", nSeekTimeMs, n);
            nSeekTimeMs = n;
        }
    }
...
status_t AwPlayer::invoke(const Parcel &request, Parcel *reply)
{
    ...
    switch(nMethodId)
    {
        ...
        {
            logv("=== INVOKE_ID_ADD_EXTERNAL_SOURCE");
            int                 fd;
            int64_t             nOffset;
            int64_t             nLength;
            int                 fdSub;
        ...
        case INVOKE_ID_UNSELECT_TRACK:
        {
            ...
            logv("invode::INVOKE_ID_SELECT_TRACK, stream index = %d", nStreamIndex);
        ...
        case INVOKE_ID_SET_SCALE_DOWN:
        {
            ...
            logv("invoke set scale down. bSetScaleDown:%d",bSetScaleDown);
        ...
        case INVOKE_ID_GET_SELECTED_TRACK:
        {
            ...
            logv("==== invoke: INVOKE_ID_GET_SELECTED_TRACK, trackType(%d)", trackType);
            logv("=== mCurrentSelectSubTrackIndex: %d", mPriData->mCurrentSelectSubTrackIndex);
        ...
        {
            logv("===invoke playready===");
            PlayReady_Drm_Invoke(request, reply);
            break;
        }
#endif
#if defined(CONF_DI_300_SUPPORT)
        case INVOKE_ID_SET_DI_DEMO://public void setDiDemo(boolean enable, int left, int top, int right, int bottom) in MediaPlayer.java
        {
            int enable = request.readInt32();
            int left   = request.readInt32();
            int top    = request.readInt32();
            int right  = request.readInt32();
            int bottom = request.readInt32();
            XPlayerSetDiDemo(mPriData->mXPlayer, enable, left, top, right, bottom);
            break;
        }
        case INVOKE_ID_SET_DI_TNR: //public void setTnr(boolean enable, int mode, int strength) in MediaPlayer.java
        {
            int enable = request.readInt32();
            int mode   = request.readInt32();
            int strength = request.readInt32();
            XPlayerSetDiTnr(mPriData->mXPlayer, enable, mode, strength);
            break;
        }
        case INVOKE_ID_SET_DI_FMD: //public void setFmd(boolean enable) in MediaPlayer.java
        {
            int enable = request.readInt32();
            XPlayerSetDiFmd(mPriData->mXPlayer, enable);
            break;
        }
#endif
        default:
...
status_t AwPlayer::setBufferingSettings(const BufferingSettings& buffering)
{
    ...
    logv("InitMode is %d, ReMode is %d, InitKB is %d, InitMs is %d.,\
...
status_t AwPlayer::getBufferingSettings(BufferingSettings* pBuffering)
{
    ...
    {
        logw("buffering is null. please check it.");
        return -1;
    }
...
status_t AwPlayer::callbackProcess(int messageId, int ext1, void* param)
{
    switch(messageId)
    {
        ...
        case SUBCTRL_SUBTITLE_AVAILABLE:
        {
            ...
            logv("subtitle available. id = %d, pSubtitleItem = %p",nSubtitleId,pSubtitleItem);
            ...
            if(mPriData->mIsSubtitleDisable == 0)
            {
                ...
                if(i!=64)
                {
                    ...
                    if(mPriData->mIsSubtitleInTextFormat == 1)
                    {
                        ...
                        logv("notify text expired message.");
                        sendEvent(MEDIA_TIMED_TEXT, 0, 0, &parcel);
                    }
                    else
                    {
                        ...
                        logv("notify subtitle expired message.");
        ...
        case AWPLAYER_MEDIA_SET_VIDEO_SIZE:
        {
            ...
            logv("set videosize (%d, %d)", nWidth, nHeight);
        ...
        case AWPLAYER_MEDIA_DISABLE_MEDIA:
        {
            int DisParam  = ((int*)param)[0];
            switch(DisParam)
            {
                case PLAYER_MEDIA_INFO_DISABLE_VIDEO:
                {
                    mPriData->nDisableVideo = 1;
                    break;
                }
                case PLAYER_MEDIA_INFO_DISABLE_AUDIO:
                {
                    mPriData->nDisableAudio = 1;
                    break;
                }
                case PLAYER_MEDIA_INFO_DISABLE_SUBTITLE:
                {
                    mPriData->nDisableSubtitle = 1;
                    break;
                }
                default:
                {
                    logw("unKnown parameter");
                    break;
                }
            }
            break;
...
void enableMediaBoost(MediaInfo* mi)
{
    ...
    if(mi->pVideoStreamInfo->bSecureStreamFlag == 1)
    {
      ...
      && total < 190000 )
    {
        ...
        logv("setprop media.boost.pref %s", cmd);
