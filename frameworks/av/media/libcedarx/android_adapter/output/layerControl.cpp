...
/* grallocs.h, graphics-vendor.h and sunxi_metadata_def.h have been combiled to graphics-sunxi.h on 2019.09.27,
 * location: android/hardware/aw/gpu/include/hardware
 */
//#if ((CONF_ANDROID_MAJOR_VER >= 10))
//#include <hardware/graphics-vendor.h>
//#include <hardware/grallocs.h>
//#endif
...
#if (CONF_ANDROID_MAJOR_VER >= 4 && CONF_ANDROID_MAJOR_VER <=9)
#include <hal_public.h>
#elif (CONF_ANDROID_MAJOR_VER >= 10)
...
#if ((defined(CONF_HIGH_DYNAMIC_RANGE_ENABLE) || defined(CONF_AFBC_ENABLE)) && CONF_ANDROID_MAJOR_VER < 10)
                                ...
                                void** pViraddress)
{
    ...
    int ret = 0;
#if defined(CONF_GPU_MALI) || defined(CONF_GPU_IMG_USE_COMMON_STRUCT)
    ...
    {
        logw("the hnd is wrong : hnd = %p", hnd);
        return -1;
    }
...
// Get the bufferQueue whether queue to SurfaceFlinger. if not, the AVsync should do in player.
static int getWhetherQueueToSurfaceFlinger(LayerCtrlContext* lc,int* nQueuesToSurfaceFlinger)
{
    int nErr = -1;
    *nQueuesToSurfaceFlinger = 1;//default video bufferQueue queue to SurfaceFlinger.
    nErr = lc->pNativeWindow->query(
           lc->pNativeWindow, NATIVE_WINDOW_QUEUES_TO_WINDOW_COMPOSER,
           nQueuesToSurfaceFlinger);
    if (nErr != 0) {
       loge("error authenticating native window: %d", nErr);
    }
    logd("Video Buffer QueueToSurfaceFlinger = %d [1 AVsyncInDisplay, 0 AVsyncInPlayer]",
         *nQueuesToSurfaceFlinger);

    return *nQueuesToSurfaceFlinger;
}
...
#if (defined(CONF_HIGH_DYNAMIC_RANGE_ENABLE) || defined(CONF_AFBC_ENABLE))
    #ifdef CONF_SIMPLE_ION
    CdcIonMunmap(virsize, viraddress);
    #else
    if (viraddress != 0) {
...
{
    logv("pushBlankBuffersToNativeWindow: pNativeWindow = %p",lc->pNativeWindow);
...
static int setLayerParam(LayerCtrlContext* lc)
{
    ...
    logd("setLayerParam: dispW(%d), dispH(%d), buffercount(%d), bProtectFlag(%d),\
          bIsSoftDecoderFlag(%d), bAfbcModeFlag=%d",
          lc->nDisplayWidth,lc->nDisplayHeight,lc->nGpuBufferCount,
          lc->bProtectFlag,lc->bIsSoftDecoderFlag, lc->bAfbcModeFlag);
    ...
    if(lc->bProtectFlag == 1)
    {
        ...
        logv("set usage to GRALLOC_USAGE_PROTECTED");
...
#if (CONF_ANDROID_MAJOR_VER >= 10)
    lc->nUsage |= GRALLOC1_PRODUCER_USAGE_VIDEO_DECODER;
#endif
    ...
    logv("crop (l,t,r,b); %d %d %d %d", crop.left, crop.top, crop.right, crop.bottom);
    ...
    native_window_set_usage(lc->pNativeWindow,lc->nUsage | AHARDWAREBUFFER_USAGE_VIDEO_ENCODE);
...
//    ATRACE_CALL();
    ...
    crop.right  = 64;
    crop.bottom = 64;
    native_window_set_usage(lc->pNativeWindow, (lc->nUsage & (~GRALLOC_USAGE_AFBC_MODE))
            | AHARDWAREBUFFER_USAGE_VIDEO_ENCODE);
    native_window_set_buffers_dimensions(lc->pNativeWindow, 64, 64);
    lc->pNativeWindow->perform(lc->pNativeWindow, NATIVE_WINDOW_SET_CROP, &crop);

    for(i = 0; i < lc->nGpuBufferCount; i++)
    {
        //ATRACE_NAME("dequeue min buffer");
        do
        {
            err = lc->pNativeWindow->dequeueBuffer_DEPRECATED(lc->pNativeWindow, &pWindowBuf);
            if(err)
            {
                logw("fail to dequeue buffer, err = %d.", err);
                return -1;
            }
            for(j=0; j<i; j++)
            {
    ...
    // wait here for all queued buffers have been replace
    int slot = -1;
    int lastSlot = lc->nGpuBufferCount - 1;
    do {
        //ATRACE_NAME("wait min buffer replace");
        err = lc->pNativeWindow->dequeueBuffer_DEPRECATED(lc->pNativeWindow, &pWindowBuf);
        if (err) {
            logw("---> fail to dequeue buffer, err = %d.", err);
            return -1;
        ...
        for (int s = 0; s <= lastSlot; s++) {
            if (pWindowBuf == bufferV[s].pWindowBuf) {
                slot = s;
                break;
            }
        }
    ...
    native_window_set_buffers_dimensions(lc->pNativeWindow, lc->nWidth, lc->nHeight);

    logv("=== diff: %lld", GetNowUs()-start);
    return 0;
}
...
#error invalid GPU type config
    ...
    //if(lc->bIsSoftDecoderFlag == 0)
    {
        ...
        {
            *pBufFd = hnd->share_fd;
            CdcMemGetPhyByFd(lc->memops, hnd->share_fd, (void*)pPhyaddress);
            *pPhyaddress -= CONF_VE_PHY_OFFSET;
            return 0;
        }
        else
        {
            logw("tmemops is NULL");
            return -1;
        }
        #else
        if(lc->ionFd >= 0)
        ...
        {
            logw("the ion fd is wrong : fd = %d",lc->ionFd);
            return -1;
        }
        #endif
...
dequeue_buffer:
    ...
    private_handle_t *hnd = (private_handle_t*)pWindowBuf->handle;
    logv("==== init: buf(fd: %d), nUnFreeBufferCount: %d, aw_buf_id: %lld, private_handle_t: %p", nBufFd, lc->nUnFreeBufferCount, hnd->aw_buf_id, hnd);
...
static void setDisplay3DMode(LayerCtrlContext* lc)
{
    ...
    logv("lc->nDisplayHeight=%d, nHeight=%d", lc->nDisplayHeight, lc->nHeight);
...
static void __LayerRelease(LayerCtrl* l)
{
    ...
    for(i = 0; i < GPU_BUFFER_NUM; i++)
    {
        ...
        {
            CdcIonMunmap(lc->mGpuBufferInfo[i].nMetaDataVirAddrSize, (unsigned char *)lc->mGpuBufferInfo[i].pMetaDataVirAddr);
        }
        if(lc->mGpuBufferInfo[i].nBufFd >= 0)
        {
            CdcMemFreePhyByFd(lc->memops, lc->mGpuBufferInfo[i].nBufFd,(unsigned long)lc->mGpuBufferInfo[i].pBufPhyAddr);
        }
#else
        if(lc->mGpuBufferInfo[i].nBufFd >= 0)
        {

            logv("close: ion_buf_fd[%d] = %d", i, lc->mGpuBufferInfo[i].nBufFd);
        ...
        {
            //lc->nUnFreeBufferCount--;
            logv("ion_free: handle_ion[%d] = %d, nUnFreeBufferCount: %d",
...
#if (defined(CONF_PRODUCT_STB) && (CONF_ANDROID_MAJOR_VER < 10))
    if(lc->pNativeWindow != NULL)
...
static int __LayerGetBufferNumHoldByGpu(LayerCtrl* l)
{
    ...
    if(lc->nBufferNumHoldByGpu)
    {
        return lc->nBufferNumHoldByGpu;
    }
    else
    {
...
{
    logv("debugxx __LayerDequeueBuffer, *ppVideoPicture(%p),bInitFlag(%d)",
...
{
    logv("debugxx LayerQueueBuffer: pInPicture = %p, pData0 = %p",
...
static int __LayerReleaseBuffer(LayerCtrl* l,VideoPicture* pPicture)
{
    ...
    for(i = 0; i < GPU_BUFFER_NUM; i++)
    {
        if(lc->mGpuBufferInfo[i].nBufFd == pPicture->nBufFd)
        {
            ...
            lc->mGpuBufferInfo[i].mStatus = UNRECOGNIZED;
            lc->nUnFreeBufferCount--;
            logv("release this buffer(fd: %d), nUnFreeBufferCount: %d", pPicture->nBufFd, lc->nUnFreeBufferCount);
            break;
        }
    }
...
static int __LayerReset(LayerCtrl* l)
{
    ...
    for(i = 0; i < lc->nGpuBufferCount; i++)
    {
        ...
        if(lc->mGpuBufferInfo[i].handle_ion != ION_NULL_VALUE)
        {
            logv("ion_free: handle_ion[%d] = %d",i, (int)lc->mGpuBufferInfo[i].handle_ion);
            ion_free(lc->ionFd,lc->mGpuBufferInfo[i].handle_ion);
        }
        #endif
...
static int __LayerControl(LayerCtrl* l, int cmd, void *para)
{
    ...
    switch(cmd)
    {
        ...
        case CDX_LAYER_CMD_GET_WHETHER_QUEUETOSF:
        {
            getWhetherQueueToSurfaceFlinger(lc,(int*)para);
            break;
        }
        case CDX_LAYER_CMD_SET_HOLD_BY_GPU_BUFFER_NUM:
        {
            lc->nBufferNumHoldByGpu = *((int*)para);
            logd("layer set holding by gpu buffer num is  %d !",lc->nBufferNumHoldByGpu);
            break;
        }
        default:
            break;
...
LayerCtrl* LayerCreate()
{
    ...
    int ret = CdcMemOpen(lc->memops);

    if(ret != 0)
    {
        loge("memops open fail ! ");
        return NULL;
    }
#else
    lc->ionFd = -1;
