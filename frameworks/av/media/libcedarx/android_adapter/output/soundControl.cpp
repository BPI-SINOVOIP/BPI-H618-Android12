...
static int __SdControl(SoundCtrl* s, cdx_int32 cmd, void* para)
{
    ...
    switch(cmd)
    {
        ...
        case SOUND_CONTROLL_SET_CLBK_EOS:
            ...
            logv("__SdControll : set clbk eos");
            ret = 0;
            break;
#endif
        case SOUND_CONTROLL_QUERY_IF_GAPLESS_PLAY:
            {
                ...
                ret = answer;
                logv("__SdControll : query if gapless play (%s)", answer?"yes":"no");
                break;
            }
        default:
        ...
        if(sc->needDirectOutPut)
        {
            ...
            switch(sc->data_type)
            {
                case AUDIO_RAW_DATA_AC3:
                    logv("cedarx passthrough data ac3");
                    format = AUDIO_FORMAT_AC3;
                    break;
                case AUDIO_RAW_DATA_DTS:
                    logv("cedarx passthrough data dts");
                    format = AUDIO_FORMAT_DTS;
                    break;
                case AUDIO_RAW_DATA_DOLBY_DIGITAL_PLUS:
                    logv("cedarx passthrough data ddp");
                    format = AUDIO_FORMAT_E_AC3;
                    break;
                case AUDIO_RAW_DATA_DTS_HD:
                    logv("cedarx passthrough data dts-hd");
...
*/
                ...
                case AUDIO_RAW_DATA_MAT:
                    logv("cedarx passthrough data mat/truehd");
        ...
        if(sc->pAudioSink != NULL && !sc->needDirectOutPut)
        {
            ...
            */
            ...
            if(!sc->pAudioSink->ready()){
                logv("Cedarx use AudioSink open spr : %d, ch : %d, fmt : %u",
                ...
                uint32_t latency;
                AudioSystem::getOutputFrameCount(&afFrameCount, AUDIO_STREAM_DEFAULT);
                AudioSystem::getOutputSamplingRate(&samplingRate, AUDIO_STREAM_DEFAULT);
                AudioSystem::getOutputLatency(&latency, AUDIO_STREAM_DEFAULT);
                const size_t minFrameCount = AudioSystem::calculateMinFrameCount(latency,
                    afFrameCount, samplingRate, sc->nSampleRate, 2.0);
                const size_t framesPerBuffer = (uint64_t)sc->nSampleRate * afFrameCount / samplingRate;
                reqBufferCount = (minFrameCount / framesPerBuffer)
                    + (minFrameCount % framesPerBuffer > 0 ? 1 : 0);
                ...
                if(err != OK)
                {
                    loge("failed to open AudioSink with err %d.", err);
                    pthread_mutex_unlock(&sc->mutex);
                    return -1;
                }
            }
            else{
                logv("Audiosink has opened");
            }
        ...
        {
            do{
                logv("CedarX request Direct output to transfer iec61937 stream(or muti ch pcm)");
                if(sc->pAudioTrack == NULL)
                {
                    if(sc->directSink.pAudioTrackHandle != NULL)
                    {
                        ...
                        {
                            logv("Direct out put reuse audiotrack from recycle");
                    ...
                    logv("Direct out put new a Audiotrack");
                ...
                {
                    logv("Direct out put reuse audiotrack from self");
                }
            }
            while(0);
            ...
            logv("framesize infomation : nFrameSize : %d", sc->nFrameSize);
...
static int __SdWrite(SoundCtrl* s, void* pData, int nDataSize)
{
    ...
    if(sc->pAudioSink != NULL && !sc->needDirectOutPut)
    {
        ...
        nWritten = sc->pAudioSink->write(pData, nDataSize, false);
...
int _SetPlaybackRate(SoundCtrl* s,const XAudioPlaybackRate *rate)
{
    ...
    logv("rate.mSpeed          %f",sc->nPlayRate.mSpeed);
    logv("rate.mPitch          %f",sc->nPlayRate.mPitch);
    logv("rate.mFallbackMode   %d",sc->nPlayRate.mFallbackMode);
    logv("rate.mStretchMode    %d",sc->nPlayRate.mStretchMode);
