...
static inline void aw_host_nfc_set_user_data_len(struct nfc_reg *nfc, uint32_t user_data_len)
{
	...
	uint8_t user_data_len_reg_cnt = ecc_block_cnt / 8;
	if (ecc_block_cnt % 8)
...
static inline int aw_host_nfc_get_dummy_byte(int real_page_size, int ecc_mode, int ecc_block_cnt, int user_data_len)
{
