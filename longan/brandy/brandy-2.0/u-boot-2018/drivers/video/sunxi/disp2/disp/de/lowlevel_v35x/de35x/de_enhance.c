...
static s32 de_enhance_set_size(u32 disp, u32 chn)
{
	...
	if (g_cfg->dci_exist[disp][chn]) {
		...
		de_dci_set_window(disp, chn, demo_enable, tmp_win2);
	}

	/* fcm */
	if (g_cfg->fcm_exist[disp][chn]) {
		de_fcm_set_size(disp, chn, size.width, size.height);
		de_fcm_set_window(disp, chn, demo_enable, tmp_win2);
	}
	...
	if (g_cfg->sharp_exist[disp][chn]) {
		...
		de_sharp_set_window(disp, chn, demo_enable, tmp_win2);
	}

	/* process dep */
	if (chn == 0) {
		/* gamma */
		de_gamma_set_size(disp, size.width, size.height);
		de_gamma_set_window(disp, demo_enable, tmp_win);

		/* dither */
		de_dither_set_size(disp, size.width, size.height);

		/* deband */
		de_deband_set_size(disp, size.width, size.height);
		de_deband_set_window(disp, demo_enable, tmp_win);
	}
...
s32 de_enhance_layer_apply(u32 disp, struct disp_enhance_chn_info *info)
{
	...
	for (chn = 0; chn < chno; chn++) {
		...
		g_para[disp][chn]->flags = (disp_manager_dirty_flags)flags;
	}
