...
{
#if defined(SUPPORT_DSI) && defined(DSI_VERSION_40)
	if (panel->lcd_if == LCD_IF_DSI && panel->lcd_tcon_mode != DISP_TCON_DUAL_DSI)
...
int disp_al_lcd_get_start_delay(u32 screen_id, disp_panel_para *panel)
{
	...
	if (panel && panel->lcd_if == LCD_IF_DSI && panel->lcd_tcon_mode != DISP_TCON_DUAL_DSI) {
		lcd_start_delay =
...
int disp_al_edp_cfg(u32 screen_id, u32 fps, u32 edp_index, struct disp_video_timings *video_info)
{
	al_priv.tcon_type[screen_id] = 1;
	tcon_init(screen_id);
...
int disp_al_edp_enable(u32 screen_id)
{
	tcon1_edp_clk_enable(screen_id, 1);
	tcon1_open(screen_id);
...
int disp_al_edp_disable(u32 screen_id)
{
	tcon1_close(screen_id);
	tcon_exit(screen_id);
	tcon1_edp_clk_enable(screen_id, 0);
...
/* #endif */
	tcon1_set_timming(screen_id, video_info);
#if defined(CONFIG_MACH_SUN50IW9) || defined(CONFIG_MACH_SUN55IW3)
...
int disp_al_lcd_get_status(u32 screen_id, disp_panel_para *panel)
{
	int ret = 0;
