...
static u8 get_panel_id(u32 sel, u32 cmd)
{
	u8 result[16] = {0};
	int count = 5;
	u32 num = 0;

	sunxi_lcd_dsi_set_max_ret_size(sel, 1);

	while (count--) {
		sunxi_lcd_delay_ms(5);
		if (sunxi_lcd_dsi_dcs_read(sel, cmd, result, &num) == 0) {
			printf("read suc : num %d %d %x %x %x\n", num, count, result[0], result[1], result[2]);
		break;
		}
	}
...
static void LCD_panel_try_switch(u32 sel)
{
	...
	sunxi_lcd_delay_ms(100);
	result[0] = get_panel_id(sel, 0xDB);
