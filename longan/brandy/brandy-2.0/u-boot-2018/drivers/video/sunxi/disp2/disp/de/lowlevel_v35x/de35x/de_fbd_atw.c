...
static s32 de_fbd_get_info(struct afbc_header *hd, struct de_fbd_info *info)
{
	u32 inputbits;

	inputbits = hd->inputbits[0] | (hd->inputbits[1] << 8) |
		    (hd->inputbits[2] << 16) | (hd->inputbits[3] << 24);

	info->block_layout = hd->block_layout;

	DE_WRN("hd->block_layout=%d\n", hd->block_layout);
