...
__weak void ss_ctrl_start(u8 channel_id, u8 alg_type)
{
	u32 val = 0;
	while ((readl(SS_TLR) & (0x1 << (28 + channel_id))) == 1) {
	};
	val = readl(SS_TLR);
	val |= (0x1 << alg_type);
	writel(val, SS_TLR);
...
__weak int ss_wait_finish(u32 task_id)
{
	uint int_en;
	int_en = readl(SS_ICR) & 0xf;
	int_en = int_en & (0x01 << task_id);
	uint reg_value = -1;
	volatile unsigned long long time_start  = 0;
	volatile unsigned long long time_delta  = 0;
	volatile unsigned long long time_expire = CE_TIMEOUT_1S;

	if (int_en != 0) {
		time_start =  get_ticks();
		do {
			reg_value = readl(SS_ISR);
			reg_value = (reg_value >> (task_id * 2)) & 0x3;
			if (reg_value == 0x1) {
				return CE_SUCESS;
			} else if (reg_value == 0x2) {
				return CE_FAIL;
			}
			time_delta = get_ticks() - time_start;
		} while (time_delta < time_expire);
	} else {
		return CE_OTHER;
	}
	return CE_TIMEOUT;
...
__weak void ss_open(void)
{
	...
	/*set CE src clock*/
	reg_val &= ~(CE_CLK_REG_CLK_SRC_SEL_CLEAR_MASK);
	udelay(10);
#ifdef FPGA_PLATFORM
	/* OSC24M */
