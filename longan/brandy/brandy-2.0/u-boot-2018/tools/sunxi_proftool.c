...
static int read_trace_config(FILE *fin)
{
	...
	while (fgets(buff, sizeof(buff), fin)) {
		...
		if (err) {
			int r = regex_report_error(&line->regex, err, "compile",
						   tok);
			free(line);
			return r;
		}
...
static void out_func(ulong func_offset, int is_caller, const char *suffix)
{
	...
	func = (is_caller ? find_caller_by_offset :
				  find_func_by_offset)(func_offset);
...
static int make_ftrace(void)
{
	...
	int i;
	int j, depth = 8;
	uint64_t entry_time[64];
	int64_t duration = 0;
	int last_exit_time = 0;
	int last_dir = FUNCF_ENTRY;
	int last_stamp;
	int caller_exist;
	struct {
		int threshold;
	...
	for (i = 0, call = call_list; i < call_count; i++, call++) {
		...
		duration     = -1;
		caller_exist = 1;
		if (TRACE_CALL_TYPE(call) == FUNCF_EXIT) {
			if (entry_time[depth] != UINT64_MAX)
				duration = time - entry_time[depth];
			else
				duration = time - last_exit_time;
			last_exit_time = time;
			depth -= 1;
		} else {
			if (last_dir == FUNCF_EXIT) {
				if (time - last_stamp > 1000)
					duration = time - last_stamp;
			}
		}
		...
		printf("%16s-%-5d [01] %lu.%06lu: ", "uboot", 1, time / 1000000,
		       time % 1000000);

		if (caller_exist)
			out_func(call->caller, 1,
				 TRACE_CALL_TYPE(call) == FUNCF_ENTRY ? " -> " :
									      " <- ");
		for (j = 0; j < 4; j++) {
			if (duration / 1000 >= prompts[j].threshold)
				prompt = prompts[j].str;
		}
		if (duration >= 0) {
			out_func(call->func, 0, "");
			printf("\t:%s takes %lu ms\n", prompt, duration / 1000);
		} else
			out_func(call->func, 0, "\n");
		if (TRACE_CALL_TYPE(call) == FUNCF_ENTRY) {
			depth += 1;
			entry_time[depth] = time;
		}
		last_dir   = TRACE_CALL_TYPE(call);
		last_stamp = time;
	}
	info("ftrace: %d functions not found, %d excluded\n", missing_count,
	     skip_count);
...
static int prof_tool(int argc, char *const argv[], const char *prof_fname,
