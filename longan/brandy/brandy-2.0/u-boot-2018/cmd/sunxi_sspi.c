...
static int do_sunxi_spi_xfer(int bus, int cs)
{
	struct sunxi_spi_slave *spi_slave;
	int ret = 0;

	spi_slave = get_sspi(bus);
	if (spi_slave != NULL) {
		ret = spi_claim_bus(&spi_slave->slave);
		if (ret) {
			printf("Failed to claim SPI bus: %d\n", ret);
		}
		ret = spi_xfer(&spi_slave->slave, bitlen, dout, din, SPI_XFER_END);
		if (ret) {
...
int do_sunxi_sspi (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	char  *cp = 0;
	uchar tmp;
	int   j;
