...
//note:  this file is copy form /andriod12/bootable/libbootloader
#include "libxbc.h"
...
#define BOOTCONFIG_TRAILER_SIZE                                                \
	BOOTCONFIG_MAGIC_SIZE + BOOTCONFIG_SIZE_SIZE + BOOTCONFIG_CHECKSUM_SIZE
/*
...
{
	return !strncmp(
		(const char *)(bootconfig_end_addr - BOOTCONFIG_MAGIC_SIZE),
...
int32_t addBootConfigParameters(char *params, uint32_t params_size,
				unsigned long bootconfig_start_addr,
				uint32_t bootconfig_size)
{
	...
	int32_t new_size      = 0;
	unsigned long end     = bootconfig_start_addr + bootconfig_size;
 ...
 * note:  this file is copy form /andriod12/bootable/libbootloader
 */
int32_t addBootConfigTrailer(unsigned long bootconfig_start_addr,
			     unsigned long bootconfig_size)
{
	...
	// checksum
	uint32_t sum = checksum((unsigned char *)bootconfig_start_addr,
				bootconfig_size);
