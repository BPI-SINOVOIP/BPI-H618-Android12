...
int update_for_uboot(char *uboot_name)
{
	...
	struct spare_boot_data_head *uboot_data_head;
	char *uboot_buf = NULL;
	int   length = 0;
	int   i;
	int   ret = -1;
	...
	if (uboot_version_newer(uboot_head->boot_head.version, "4.0.0")) {
		uboot_data_head = (struct spare_boot_data_head
					  *)((uint8_t *)uboot_head +
					     sizeof(struct spare_boot_ctrl_head));
		if (!script_parser_fetch("platform", "debug_mode",
					 value)) {
			uboot_data_head->debug_mode = value[0];
		} else {
			uboot_data_head->debug_mode = 8;
		}
		printf("debug_mode =%d\n", uboot_data_head->debug_mode);
		if (!script_parser_fetch("target", "burn_secure_mode",
					 value)) {
			uboot_data_head->secure_mode = value[0];
		}
		printf("secure_mode =%d\n", uboot_data_head->secure_mode);
	}

	if (!script_parser_fetch("target", "force_rotpk", value)) {
		if (value[0]) {
			uboot_head->boot_data.func_mask |=
				UBOOT_FUNC_MASK_BIT_FORCE_ROTPK;
		}
		printf("force_rotpk = %d",
		       (uboot_head->boot_data.func_mask &
			UBOOT_FUNC_MASK_BIT_FORCE_ROTPK) != 0);
	}
