...
int update_for_fes1(char *fes1_name, int storage_type)
{
	...
	if (value[0]) {
		...
		if (!script_parser_mainkey_get_gpio_cfg("twi_para", gpio_set, 32)) {
			for (i = 0; i < 32; i++) {
				...
				fes1_head->i2c_gpio[i].port		= gpio_set[i].port;
				fes1_head->i2c_gpio[i].port_num	= gpio_set[i].port_num;
				fes1_head->i2c_gpio[i].mul_sel		= gpio_set[i].mul_sel;
				fes1_head->i2c_gpio[i].pull		= gpio_set[i].pull;
				fes1_head->i2c_gpio[i].drv_level	= gpio_set[i].drv_level;
				fes1_head->i2c_gpio[i].data		= gpio_set[i].data;
				fes1_head->i2c_gpio[i].reserved[0]	= value[1];
			}
		}
	...
	printf("extd_head.select_mode:%d\n", fes1_head->extd_head.select_mode);
	printf("magic:%s\n", fes1_head->extd_head.magic);
	if (!strncmp((char *)fes1_head->extd_head.magic, "DRAM.ext", strlen("DRAM.ext"))) {
		if (!strncmp((char *)fes1_head->extd_head.version, "dram2.0",
			     strlen("dram2.0"))) {
			max_groupdram = 32;
		} else {
			max_groupdram = 15;
		}
		printf("version:%s\n", fes1_head->extd_head.version);
		printf("max_groupdram:%d\n", max_groupdram);
		if (!script_parser_fetch("dram_select_para", "select_mode", value)) {
			fes1_head->extd_head.select_mode = value[0];
		}
		if (fes1_head->extd_head.select_mode != 0) {
				char dram_para_name[16];
				for (i = 1; i < max_groupdram; i++) {
					sprintf(dram_para_name, "dram_para%d", i);
				if (script_parser_sunkey_all(dram_para_name, (void *)fes1_head->extd_head.dram_para[i-1])) {
						printf("script fetch dram para%d failed\n", i);
					}
				}
				if (!script_parser_fetch("dram_select_para", "gpadc_channel", value)) {
					fes1_head->extd_head.gpadc_channel = value[0];
				}
				for (i = 0; i < 4; i++) {
					if (!script_parser_mainkey_get_gpio_cfg("dram_select_para", gpio_set, 32)) {
						fes1_head->extd_head.dram_select_gpio[i].port          = gpio_set[i].port;
						fes1_head->extd_head.dram_select_gpio[i].port_num      = gpio_set[i].port_num;
						fes1_head->extd_head.dram_select_gpio[i].mul_sel       = gpio_set[i].mul_sel;
						fes1_head->extd_head.dram_select_gpio[i].pull          = gpio_set[i].pull;
						fes1_head->extd_head.dram_select_gpio[i].drv_level     = gpio_set[i].drv_level;
						fes1_head->extd_head.dram_select_gpio[i].data          = gpio_set[i].data;
					}
				}
		}
	}
