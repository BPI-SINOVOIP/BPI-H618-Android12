...
unsigned int logic_offset = (20 * 1024 * 1024 / 512);
int media_type = 0;
...
//		GetFullPath(mbr_name,   argv[2]);
	...
	else if (argc == 5)	//update_mbr    sys_partition.bin  n mbr_file dlinfo_file
	{
		...
		/*update_mbr    sys_partition.bin  n mbr_file dlinfo_file flash_size logic_offset media_type*/
		if ((argv[1] == NULL) || (argv[3] == NULL) || (argv[4] == NULL)) {
			ERR("update mbr error: one of the  file names is empty\n");
			return __LINE__;
		}
		...
		media_type = atoi(argv[7]);
		/*unit sectors*/
		if (media_type == STORAGE_SDMMC) {
			total_sectors = atoi(argv[5]);
			logic_offset = temp;
		} else {
			total_sectors = atoi(argv[5]) - temp;
			logic_offset = 0;
		}

		DBG("mbr count = %d total_sectors = %d logic_offset = %d media = %d\n",
				mbr_count, total_sectors, logic_offset, media_type);
	}
	else
	{
