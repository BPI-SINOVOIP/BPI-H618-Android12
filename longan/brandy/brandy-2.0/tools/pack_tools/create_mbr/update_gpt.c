...
extern unsigned int logic_offset;
...
int create_standard_gpt(sunxi_mbr_t *sunxi_mbr_buf, char* gpt_buf)
{
	...
	for(i=0;i<sunxi_mbr->PartCount;i++) {
		...
		printf("MBR addr = 0x%llx,logic_offset = 0x%x ",
			((u64)sunxi_mbr->array[i].addrhi<<32) + sunxi_mbr->array[i].addrlo, logic_offset);
		...
		if(i == sunxi_mbr->PartCount-1) {
			...
			disk_size += ((u64)sunxi_mbr->array[i].lenhi<<32) + sunxi_mbr->array[i].lenlo-1 + logic_offset;
		}
		...
		//ASCII to unicode
		part_size = PARTNAME_SZ *sizeof(efi_char16_t);
		memset(pgpt_entry->partition_name, 0, part_size);
