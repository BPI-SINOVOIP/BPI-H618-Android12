...
//	p_toc0_cert += sizeof(TOC0_COMST_MAIN_SEQUENCE);*/
	...
	/*
	 * add dummy padder, so non hashed part in main_part is
	 * nothing but dummy data. BROM parser code can not be
	 * modified, so fix the hash range to cover whole main_part
	 * is not an option.
	 * dummy padder len should be sizeof(TOC0_CONST_MAIN_SEQUENCE),
	 * which is included in hash range but excluded form hash
	 * range len calculating.
	 *
	 * before
	 * data range    | main seq          |    main_part   |
	 * hash range    | hashed (main_part_len) |
	 *
	 * after
	 * data range    | main seq          |    main_part   | main_part_dummy |
	 * hash range    | hashed (main_part_len)             |
	 */
	p_toc0_cert += sizeof(TOC0_CONST_MAIN_SEQUENCE);
