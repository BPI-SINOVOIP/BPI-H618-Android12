...
static int	__create_cert_for_toc1(char *cnf_name, char *keyname, char *cert_name)
{
	...
	sprintf(cmdline, "openssl req -new -x509 -sha256 -batch "
							"-key %s.pem "
							"-out %s.crt "
							"-config %s "
							"-extensions usr_cert",
	...
	memset(cmdline, 0, 1024);
	sprintf(cmdline, "openssl x509 -in "
						"%s.crt "
						"-inform PEM "
						"-out %s.der "
						"-outform DER",
...
************************************************************************************************************
*/
int create_cert_for_toc1(char *lpCfg, char *section_name, toc_descriptor_t *toc1, char *keypath, char *cnfpath_base)
{
	char all_toc1[1024];
	int  i;
	...
	char tmp_cert_dir[64], tmp_cnf_dir[64];
	int gen_rootkey = 0;
	if (!strcmp(section_name, "toc1"))
		gen_rootkey = 1;
	sprintf(tmp_cert_dir, "%s/%s", section_name, "cert");
	sprintf(tmp_cnf_dir, "%s/%s", section_name, "cnf");

	memset(all_toc1, 0, 1024);
	memset(all_toc1_line, 0, 16 * sizeof(char *));

	if (GetPrivateProfileSection(section_name, all_toc1, 1024, lpCfg)) {
	...
	for(i=0;i<16;i++)
	{
		if(all_toc1_line[i])
		{
			...
			GetFullPath(current_path, tmp_cnf_dir);
			...
			//保存证书名称
			memset(current_path , 0, MAX_PATH);
			GetFullPath(current_path, tmp_cert_dir);
...
//			printf("toc1[i].cert=%s\n", toc1[i].cert);
			...
			else
			{
				...
				if (gen_rootkey) {
					ret = WritePrivateProfileSection(
						"usr_cert", toc1[i].item,
						key_value, toc1[0].cnf);
					if (ret < 0) {
