...
int splittoc1(char *toc1 )
{
	...
	for(; cnt >0 ;item_head ++, cnt -- )
		if( (!strcmp(item_head->name, "u-boot") )&&
				(item_head->type == ITEM_TYPE_BINFILE))
			break ;
...
/*split all item image from toc1*/
int splittoc1_item(char *toc1, char *toc1_item_name, char *out_item_file)
{
	char full_toc1[MAX_PATH], full_item[MAX_PATH] ;
	FILE *ftoc1 = NULL, *fitem = NULL;
	int ret = -1, totallen, actlen, cnt;
	char *body = NULL;
	char *toc1_item_file = out_item_file;

	unsigned int offset;

	if (toc1_item_name == NULL) {
		goto splittoc1_item_out;
	}
	printf("toc1_item_name = %s\n", toc1_item_name);
	sbrom_toc1_head_info_t  *toc1_head;
	sbrom_toc1_item_info_t  *item_head ;

	GetFullPath(full_toc1, toc1);
	GetFullPath(full_item, toc1_item_file);
	ftoc1 = fopen(full_toc1, "r+");
	if (ftoc1 == NULL) {
		printf("Open toc1 file %s fail ---%s\n", full_toc1,
				strerror(errno));
		goto splittoc1_item_out;
	...
	offset = (sizeof(sbrom_toc1_head_info_t) + cnt* sizeof(sbrom_toc1_item_info_t) + 1023) & (~1023);
	for (; cnt > 0; item_head++, cnt--)
		if ((!strcmp(item_head->name, toc1_item_name)) &&
				(item_head->type != ITEM_TYPE_BINKEY))
			break ;
#if 0

	printf("split %s\n", toc1);
	printf("out_item_file = %s\n", out_item_file);
	for (; cnt > 0; item_head++, cnt--) {
		printf("======================\n");
		printf("item_head->name=%s\n", item_head->name);
		printf("item_head->type=%d\n", item_head->type);
	}

	printf("item_head->name:%s,item_head->type = %d\n", item_head->name, item_head->type);
#endif
