...
#include "include.h"
#include <errno.h>
#define TOC_ALIGN_SIZE		(4 * 1024)

extern int32_t toc0_item_name;
extern char *toc0_item_cfg;
...
int createtoc0(toc_descriptor_t *toc0, char *toc0_name)
{
	...
	offset = (offset + file_len + 15) & (~15);
	printf("offset=%d\n", offset);
	offset_align = (offset + TOC_ALIGN_SIZE - 1) & (~(TOC_ALIGN_SIZE - 1));
...
{
	char full_toc0[MAX_PATH], full_item[MAX_PATH];
	FILE *ftoc0 = NULL, *fitem = NULL;
	int ret = -1, totallen, actlen;
	char *body ;
	char *item_file = out_item_file;
	unsigned int offset;

	sbrom_toc0_item_info_t  *item_head ;

	GetFullPath(full_toc0, toc0);
	GetFullPath(full_item, item_file);
	ftoc0 = fopen(full_toc0, "r+");
	if (ftoc0 == NULL) {
		printf("Open toc0 file %s fail ---%s\n", full_toc0,
				strerror(errno));
		goto splittoc0_item_out;
	}

	totallen = getfile_size(ftoc0);
	body = malloc(totallen);
	if (body == NULL) {
		printf("malloc toc0 body fail\n");
		goto splittoc0_item_out;
	}
	...
	item_head = (sbrom_toc0_item_info_t *)(sizeof(sbrom_toc0_head_info_t) + body);
	for (; item_head->name; item_head++) {

		printf("item_head->name = 0x%x\n", item_head->name);
		if (item_head == NULL) {
			printf("cant find enum 0x%x --- %s\n", toc0_item_name,
				strerror(errno));
			goto splittoc0_item_out;
		}
		if (item_head->name == toc0_item_name) {
			printf("find enum 0x%x\n", toc0_item_name);
			break;
		}
	}
	...
	offset = item_head->data_offset;
	actlen = item_head->data_len;
	fitem = fopen(full_item, "w+");
	if (fitem == NULL) {
		printf("Open item file %s fail ---%s\n", full_item,
				strerror(errno));
		goto splittoc0_item_out;
	}

	if (fwrite(body+offset, actlen, 1, fitem) != 1) {
		printf("write item file fail --- %s\n", strerror(errno));
		goto splittoc0_item_out;
	}
	ret = 0;
	printf("split item from toc0 done\n");
splittoc0_item_out:
	if (body)
		free(body);
	if (fitem)
