...
void str_to_OID(char *name_in_oid, char *name_in_str)
{
	int i;
	char tmp[256];
	i = 0;
	while (name_in_str[i] != 0) {
		memset(tmp, 0, 256);
		if (i == 0) {
			sprintf(tmp, "%d", name_in_str[i] / 40);
			strcpy(name_in_oid, tmp);
			sprintf(tmp, ".%d", name_in_str[i] % 40);
		} else {
			sprintf(tmp, ".%d", name_in_str[i]);
		}
		strcat(name_in_oid, tmp);
		i++;
	}
}
	...
	do
	{
		...
		if(*p_line_buff == '[')	//main key
		{
			...
			else if(pre_mode == 1)
			{
				...
				char subkey_in_oid[1024];
				str_to_OID(subkey_in_oid, subkey);

				memset(tmp_line_buff, 0, 1024);
				sprintf(tmp_line_buff, "%s=ASN1:UTF8String:\"%s\"\n", subkey_in_oid, value);
...
int get_section_data(char *mainkey, char *dst_buf, char *lpcfg)
{
	/*
	 * Only check whether the input parameter and format are
	 * correct, return 0 regardless of whether the obtained
	 * data is empty or not
	 * */
	if (GetPrivateProfileSection(mainkey, dst_buf, sizeof(dst_buf), lpcfg)) {
		printf("get %s err in GetPrivateProfileSection\n", mainkey);
		return -1;
	}
	/* No session or no data under session*/
	if (dst_buf[0] == '\0') {
		printf("dragoncreate_toc1_certif err no content match %s\n", mainkey);
		return -1;
	}
	return 0;
}
