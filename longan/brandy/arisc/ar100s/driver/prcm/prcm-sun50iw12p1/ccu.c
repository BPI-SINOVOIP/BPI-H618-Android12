...
s32 ccu_init(void)
{
	...
	/* setup apbs1 post div source to 200M */
	writel((readl(APBS1_CFG_REG) & (~APBS1_FACTOR_M_MASK)) | APBS1_FACTOR_M(2), APBS1_CFG_REG);
	writel((readl(APBS1_CFG_REG) & (~APBS1_CLK_DIV_N_MASK)) | APBS1_CLK_DIV_N(0), APBS1_CFG_REG);

	/* set apbs1 clock source to pll_peri0 */
	writel((readl(APBS1_CFG_REG) & (~APBS1_CLK_SRC_SEL_MASK)) | APBS1_CLK_SRC_SEL(3), APBS1_CFG_REG);
#endif
...
{
	writel(value, RTC_FAKE_POWER_OFF_REG);
}
u32 read_fake_poweroff_flag(void)
{
	return readl(RTC_FAKE_POWER_OFF_REG);
}
