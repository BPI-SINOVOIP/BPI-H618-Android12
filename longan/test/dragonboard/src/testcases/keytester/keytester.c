...
static int getNumberSlots(void)
{
    FILE *fp = NULL;
    unsigned int size = 0, len = 0;
    char data[128] = {'0'}, *p = data;

    fp = popen("bootctl get-number-slots", "r");
    if (fp == NULL) {
        printf("popen error!\n");
        return -1;
    }

    while ((size = fread(p, 1, 128, fp)) > 0) {
        p += size;
        len += size;
    }
...
int main(int argc, char *argv[])
{
    ...
    char volume_key[16];
    int active_num = 0, cur_key = -1, old_key = -1;
    int key_num = 0, key_numfind = 0, keytestcode = 0;
    struct key_test need_testkey[32];
    bool keytest_pass = true;
    char key_code[64];
    unsigned int slot_num = getNumberSlots();
    ...
    if (script_fetch("key", "key_num", &key_num, 1) == 0) {
        db_debug("key needs test num is %d\n", key_num);
        for (i = 1; i <= key_num; i++) {
            memset(key_code, 0, sizeof(key_code));
            snprintf(key_code, sizeof(key_code), "key_code%d", i);
            if (script_fetch("key", key_code, &keytestcode, 0) == 0 && keytestcode > 0) {
                need_testkey[key_numfind].testkey_code = keytestcode;
                need_testkey[key_numfind].is_test = false;
                key_numfind++;
            }
        }
    }
            ...
            if (evt[i].type == EV_SYN) {
                ...
                if (slot_num > 1) {
                    if (evt[i].value == 0) { // key up
                        ...
                        if (active_num >= 6) {
                            db_debug("bootctl set-active-boot-slot 0, then reboot\n");
                            system("bootctl set-active-boot-slot 0");
                            system("reboot");
                        }
                    }
                }
                ...
                if (key_numfind == 0) {
                    ...
                    snprintf(pre_key, sizeof(pre_key), "%s", buf);
                    SEND_CMD_PIPE_OK_EX(buf);
                } else if (key_numfind > 0 ) {
                    keytest_pass = true;
                    for (j = 0; j < key_numfind; j++) {
                        if (evt[i].code == need_testkey[j].testkey_code) {
                            if (!need_testkey[j].is_test) {
                                need_testkey[j].is_test = true;
                                if (pre_key[0]) {
                                    snprintf(buf, sizeof(buf), "%s, %s",
                                            names[evt[i].type] ? (names[evt[i].type][evt[i].code] ? names[evt[i].type][evt[i].code] : "?") : "?",
                                            pre_key);
                        ...
                        keytest_pass &= need_testkey[j].is_test;
                    }
                    snprintf(pre_key, sizeof(pre_key), "%s", buf);
                    if (keytest_pass) {
                        SEND_CMD_PIPE_OK_EX(buf);
                    } else {
                        SEND_CMD_PIPE_EX(buf);
                    }
                }
