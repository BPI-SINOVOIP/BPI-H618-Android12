...
if [ "$bt_vnd" == "common" ]; then
	bt_sup="$(wl_info -s | awk -F= '/btsup=/{print $NF}' | sed $'s,\x1b\\[[0-9;]*[a-zA-Z],,g')"
	bt_chip="$(wl_info -m | awk -F= '/module=/{print $NF}'| sed $'s,\x1b\\[[0-9;]*[a-zA-Z],,g')"
	if [ "$bt_chip" == "xr819" ]; then
		insmod /vendor/modules/xr819.ko 2>&1
		hw_ver=$(dmesg | grep -o "XRADIO_HW_REV\s\+\S\+" | awk '{print $2}')
		[ "$hw_ver" == "1.1" ] && bt_sup=1
	fi

	if [ "$bt_sup" == "0" ]; then
		SEND_CMD_PIPE_OK_EX $3 "Bluetooth not support for $bt_chip"
		LOGW "Bluetooth not support for $bt_chip"
		exit 0
	fi
	bt_vnd="$(wl_info -v | awk -F= '/vendor=/{print $NF}' | sed $'s,\x1b\\[[0-9;]*[a-zA-Z],,g')"
	[ "$bt_vnd" == "sprd" ] && device_node="ttyBT0"
fi
...
insmod /vendor/modules/sunxi_rfkill.ko 2>/dev/null
...
if [ "X_$bt_vnd" == "X_realtek" ]; then
	...
	insmod /vendor/modules/aic8800_btlpm.ko 2>/dev/null
...
scan_cmd=$(hcitool scan 2>&1 | grep "Operation not supported")
[ -n "$scan_cmd" ] && scan_cmd=lescan || scan_cmd=scan

for((i=1;i<=${loop_time} ;i++));  do
	hcitool $scan_cmd > /tmp/devlist 2>&1 &
	pid=$!
	sleep 3
	kill -2 $pid
	devlist=`grep "${destination_bt}" /tmp/devlist`
	if [ ! -z "$devlist" ]; then
		...
		SEND_CMD_PIPE_OK_EX $3 "[$bdaddr] List: $devlist"
		hciconfig hci0 down
		exit 1
	fi
done
