...
unsigned char Si2151_pollForResponse (L1_Si2151_Context *api, unsigned int nbBytes, unsigned char *pByteBuffer)
{
  ...
  start_time = system_time();
 // printf("start_time %d\n",start_time );
  while (system_time() - start_time <1000) 
  { 
  #if 1
...
unsigned char Si2151_L1_PART_INFO       (L1_Si2151_Context *api)
{
    ...
    if (L0_WriteCommandBytes(api->i2c, 1, cmdByteBuffer) != 1) 
   {
	...
	//printf("rspByteBuffer[ 8] =%02x\n",rspByteBuffer[ 8]  );
	//printf("rspByteBuffer[ 9] =%02x\n",rspByteBuffer[ 9]  );
	//printf("rspByteBuffer[ 10] =%02x\n",rspByteBuffer[ 10]  );
	//printf("rspByteBuffer[ 11] =%02x\n",rspByteBuffer[ 11]  );
	//printf("api->rsp->part_info.serial =%x\n",api->rsp->part_info.serial );
...
#ifdef    Si2151_UNLOCK_CMD
/*---------------------------------------------------*/
/* Si2151_UNLOCK COMMAND                                     */
/*---------------------------------------------------*/
unsigned char Si2151_L1_UNLOCK          (L1_Si2151_Context *api,
                                         unsigned long    key0,
                                         unsigned long    key1)
{
    unsigned char error_code = 0;
    unsigned char cmdByteBuffer[12];
    unsigned char rspByteBuffer[1];
    (void)rspByteBuffer;
    (void)error_code;
    //api->rsp->unlock.STATUS = api->status;

	printf("Si2151 UNLOCK \n");
	//KEY0 2033001900
       //KEY1 675519505
	//printf("KEY0 %d \n", key0 );
	//printf("KEY1 %d \n", key1 );
   ...
   // SiTRACE("\n");
    cmdByteBuffer[0] = Si2151_UNLOCK_CMD;
    cmdByteBuffer[1] = (unsigned char)0x00;
    cmdByteBuffer[2] = (unsigned char)0x00;
    cmdByteBuffer[3] = (unsigned char)0x00;
    cmdByteBuffer[4] = (unsigned char) ( ( key0 & Si2151_UNLOCK_CMD_KEY0_MASK ) << Si2151_UNLOCK_CMD_KEY0_LSB);
    cmdByteBuffer[5] = (unsigned char) ((( key0 & Si2151_UNLOCK_CMD_KEY0_MASK ) << Si2151_UNLOCK_CMD_KEY0_LSB)>>8);
    cmdByteBuffer[6] = (unsigned char) ((( key0 & Si2151_UNLOCK_CMD_KEY0_MASK ) << Si2151_UNLOCK_CMD_KEY0_LSB)>>16);
    cmdByteBuffer[7] = (unsigned char) ((( key0 & Si2151_UNLOCK_CMD_KEY0_MASK ) << Si2151_UNLOCK_CMD_KEY0_LSB)>>24);
    cmdByteBuffer[8] = (unsigned char) ( ( key1 & Si2151_UNLOCK_CMD_KEY1_MASK ) << Si2151_UNLOCK_CMD_KEY1_LSB);
    cmdByteBuffer[9] = (unsigned char) ((( key1 & Si2151_UNLOCK_CMD_KEY1_MASK ) << Si2151_UNLOCK_CMD_KEY1_LSB)>>8);
    cmdByteBuffer[10] = (unsigned char) ((( key1 & Si2151_UNLOCK_CMD_KEY1_MASK ) << Si2151_UNLOCK_CMD_KEY1_LSB)>>16);
    cmdByteBuffer[11] = (unsigned char) ((( key1 & Si2151_UNLOCK_CMD_KEY1_MASK ) << Si2151_UNLOCK_CMD_KEY1_LSB)>>24);
  
    if (L0_WriteCommandBytes(api->i2c, 12, cmdByteBuffer) != 12) 
{
      printf("Error writing UNLOCK bytes!\n");
      return ERROR_Si2151_SENDING_COMMAND;
    }
 #if 0
    error_code = Si2151_pollForResponse(api, 1, rspByteBuffer);
    ...
    if (error_code) 
   {
      SiTRACE("Error polling UNLOCK response\n");
      return error_code;
    }
#endif
    return NO_Si2151_ERROR;
}
#endif /* Si2151_UNLOCK_CMD */
...
unsigned char   Si2151_L1_SendCommand2(L1_Si2151_Context *api, unsigned int cmd_code)
{
    ...
    #endif /*     Si2151_TUNER_TUNE_FREQ_CMD */
    #ifdef        Si2151_UNLOCK_CMD
     case         Si2151_UNLOCK_CMD_CODE:
       return Si2151_L1_UNLOCK (api, api->cmd->unlock.key0, api->cmd->unlock.key1 );
     break;
    #endif /*     Si2151_UNLOCK_CMD */
    #ifdef        Si2151_WAKE_UP_CMD
