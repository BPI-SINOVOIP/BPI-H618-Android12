...
{
#if 1
    /* Cdtech support sample code which is used clock() function
     * if used other function, Si2151 may not lock with dvb-c standard
     */
...
int L0_ReadBytes(L0_Context* i2c, unsigned int iI2CIndex, int iNbBytes, unsigned char *pucDataBuffer) 
{
    ...
    switch (i2c->connectionType) 
    {
        ...
        */
        {
            nbReadBytes = I2cRead(pucDataBuffer, iNbBytes);
            if(nbReadBytes == -1)
            {
                nbReadBytes = 0;
            }
    ...
    if (i2c->trackRead==1) 
    {
        ...
        {
            sprintf(trace_L0_message, "%s READ  ERROR!", trace_L0_message);
        }
    ...
    trace_skip_info = 0;
    #endif /* SiTRACES */
    //printf("nbReadBytes: %d, iNbBytes: %d, pucDataBuffer:0x%x\n", nbReadBytes, iNbBytes, pucDataBuffer[0]);
    return nbReadBytes;
}
...
int L0_WriteBytes(L0_Context* i2c, unsigned int iI2CIndex, int iNbBytes, unsigned char *pucDataBuffer) 
{
    ...
    switch (i2c->connectionType) 
    {
        ...
        case CUSTOMER:
        {
            ...
            */
            {
                nbWrittenBytes = I2cWrite(pucDataBuffer, iNbBytes);
                if(nbWrittenBytes == -1)
                {
                    write_error++;
                }
    ...
    if (i2c->trackWrite == 1) 
    {
        ...
        if (write_error) 
        { 
            sprintf(trace_L0_message, "%s WRITE ERROR! (", trace_L0_message); 
        }
        for ( i=0 ; i < iNbBytes ; i++ ) 
        {
            sprintf(trace_L0_message, "%s 0x%02x", trace_L0_message, pucDataBuffer[i]);
        ...
        if (write_error) 
        { 
            sprintf(trace_L0_message, "%s )", trace_L0_message); 
        }
        SiTRACE("%s\n", trace_L0_message);
    }
    trace_skip_info = 0;
