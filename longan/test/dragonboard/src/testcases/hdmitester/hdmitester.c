...
static int display1_show_de_colorbar(void)
{
	int fd = open("/sys/class/disp/disp/attr/disp", O_WRONLY, 0);
	write(fd, "1", 2);
	close(fd);

	fd = open("/sys/class/disp/disp/attr/colorbar", O_WRONLY, 0);
	write(fd, "8", 2);
	close(fd);
	return 0;
}
...
int main(int argc, char *argv[])
{
   ...
   /* test main loop */
	while (1) {
		...
		fd_hdmi = open("/sys/class/extcon/extcon0/state", O_RDONLY);
		if (fd_hdmi < 0){
			db_error("hdmitester: open /sys/class/extcon/extcon0/state failed\n");
			if (!count--)
				goto err;
		}
#else
...
#if defined (_SUN50IW9P)
				ret = display1_show_de_colorbar();
#else
				ret = init_layer();
#endif
				if (ret < 0) {
