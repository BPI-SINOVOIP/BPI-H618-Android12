 ...
 * author:		AW1638
 * descriptions:
 *		add the inital version for spdif module's dragonboard test of sun50iw12p1 platform.
 *
 * func:
 *		1. SPDIF TX Playback;
 *      2. SPDIF TX->RX loopback and then playback the rec wav by audiocodec spk;
 *
 */
...
{
	tinymix_set_value_byname(mixer,"sunxi spdif loopback debug", 1);
}
...
int main(int argc, char **argv)
{
	...
	if (script_fetch("sound", "spdif_card", &card2, 1)) {
		printf("mictester: can't fetch soundcard, use default value: %d\n", card);
		card2 = 1;
	}
	if (script_fetch("sound", "spdif_dev", &device2, 1)) {
		printf("mictester: can't fetch sounddevice, use default value: %d\n", device);
		device2 = 0;
	}

	printf("audiocodec card: %d, audiocodec device: %d\n", card, device);
	printf("spdif card: %d, spdif device: %d\n", card2, device2);
    ...
    if (strcmp(argv[1], "TX") == 0) {
		printf("\n\n SPDIF Loopback TX Playback Test start......\n\n");
		set_to_loopback(mixer2);
		play_sample(card2, device2, "/dragonboard/data/boot_start_test.wav", argc, argv);
    } else if (strcmp(argv[1], "RX") == 0) {
		printf("\n\n SPDIF Loopback RX Capture Test start......\n\n");
		set_to_loopback(mixer2);
		cap_sample(card2, device2, "/dragonboard/data/spdif_loop_rec.wav", channels, sample_rate, bits, argc, argv);
    } else if (strcmp(argv[1], "CHECK") == 0) {
		printf("\n\n SPDIF Loopback Wav Playback Check by AudioCodec SPK......\n\n");
		set_to_speaker(mixer);
		play_sample(card, device, "/dragonboard/data/spdif_loop_rec.wav", argc, argv);
    } else {
