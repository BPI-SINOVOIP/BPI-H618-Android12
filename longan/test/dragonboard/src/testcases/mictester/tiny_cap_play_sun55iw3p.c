...
// int mic1_gain, mic2_gain, mic3_gain;
int mic1_used, mic2_used, mic3_used;
// int volume;
int mic1_threshold;
int mic2_threshold;
int mic3_threshold;
...
unsigned int test_play_sample(unsigned int card, unsigned int device,
			      char path[256], int argc, char **argv);
...
void set_to_speaker()
{
	tinymix_set_value_byname(mixer, "DAC Volume", 63);
	tinymix_set_value_byname(mixer, "DACL Volume", 160);
	tinymix_set_value_byname(mixer, "DACR Volume", 160);
	tinymix_set_value_byname(mixer, "DACL Gain ", 31);
	tinymix_set_value_byname(mixer, "DACR Gain", 31);
	tinymix_set_value_byname(mixer, "LINEOUT Gain", 31);
	tinymix_set_value_byname(mixer, "LINEOUTL Switch", 1);
	tinymix_set_value_byname(mixer, "LINEOUTR Switch", 1);
	tinymix_set_value_byname(mixer, "SPK Switch", 1);
}

void close_speaker()
{
	tinymix_set_value_byname(mixer, "LINEOUTL Switch", 0);
	tinymix_set_value_byname(mixer, "LINEOUTR Switch", 0);
...
void set_to_jack()
{
	...
	tinymix_set_value_byname(mixer, "DACL Gain ", 31);
	tinymix_set_value_byname(mixer, "DACR Gain", 31);
	tinymix_set_value_byname(mixer, "HPOUT Gain", 7);
...
void set_to_mainmic()
{
	tinymix_set_value_byname(mixer, "ADC1 Volume", 160);
	tinymix_set_value_byname(mixer, "ADC2 Volume", 160);
	tinymix_set_value_byname(mixer, "ADC1 Gain ", 31);
	tinymix_set_value_byname(mixer, "ADC2 Gain", 31);
...
void set_to_headmic()
{
	tinymix_set_value_byname(mixer, "ADC3 Volume", 160);
	tinymix_set_value_byname(mixer, "ADC3 Gain", 31);
	tinymix_set_value_byname(mixer, "MIC3 Switch", 1);
...
void close_headmic()
{
	tinymix_set_value_byname(mixer, "MIC3 Switch", 0);
...
void check_event(char *path)
{
	...
	while ((dp = readdir(dfd)) != NULL) {
		...
		printf("[%s, %d] edevname_file:%s\n", __func__, __LINE__, edevname_file);
...
int setup_mixer(int mixer_num)
{
	...
	if (script_fetch("mic", "mic3_threshold", &mic3_threshold, 1)) {
		mic3_threshold = 0;
		db_msg("mictester: can't fetch mic3_threshold, use default threshold: %d\n", mic3_threshold);

	}
	mic3_threshold = CLAMP(mic3_threshold, 0, 32767);
	...
	if (script_fetch("mic", "mic3_used", &mic3_used, 1)) {
		mic3_used = 1;
		db_msg("mictester: can't fetch mic3_used, set to default: %d\n", mic3_used);
...
int main(int argc, char **argv)
{
	...
	printf("soundcard: card: %d, device: %d, mic_spk_test: %d, dual_spk_channel_test: %d\n",
			card, device, mic_spk_test, dual_spk_channel_test);
	...
	if (mic_spk_test) {
		while (1) {
			...
			if (strcmp(buffer, "PASS") == 0 || strcmp(buffer, "FAIL") == 0) {
				...
				cap_and_play_sample(card, device,
						    header.num_channels,
						    header.sample_rate,
			     ...
			     unsigned int bits,int argc, char **argv)
{
	...
	while (capturing) {
		...
		if (ret == 0) {
			...
			if (mic1_passed && mic2_passed && mic3_passed) {
				mic_passed = true;
