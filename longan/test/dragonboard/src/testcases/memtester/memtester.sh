...
LOGD "read from config: platform=$platform"
if [ -z "$platform" ] ; then
	for parm in $(cat /proc/cmdline) ; do
		case $parm in
			androidboot.hardware=*)
				platform=`echo $parm | awk -F\= '{print $2}'`
				;;
		esac
	done
	LOGD "read from cmdline: platform=$platform"
fi
...
elif [ "$platform" == "sun50iw10p1" -o "$platform" == "sun55iw3p1" ] ; then
	dram_freq=-1
	for parm in $(cat /proc/cmdline) ; do
		case $parm in
			androidboot.dramfreq=*)
