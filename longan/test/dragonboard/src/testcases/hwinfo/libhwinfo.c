...
/*  DeviceID  ModuleName      DriverName DriverModuleName  VendorName  BtSupport */
    ...
    {0x1, 0x024cb852, "rtl8852bs",     "8852bs",     "8852bs",     "realtek",  1},
...
static struct wifi_hardware_info *get_wifi_hardware_info(void)
{
    ...
    if (wifiinfo == NULL) {
        ...
        ALOGV("Start to load wireless module hardware info table...");
        head = load_merged_config();
        cur  = head;
        while (cur != NULL) {
            i++;
            ALOGV("Entry [%02d], BUS: %4s, ID: 0x%08lX, Name: %s.", i, (cur->hwinfo.bus_type == 0) ? "USB" : "SDIO",
