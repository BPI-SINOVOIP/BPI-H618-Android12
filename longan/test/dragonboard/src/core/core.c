...
static int motor_case_id =0;
static int soundtest_case_id =0;
...
int get_motor_case_id(void)
{
return motor_case_id;
}

int get_gsensor_case_id(void)
{
return accel_case_id;
}

int get_soundtest_case_id(void)
{
return soundtest_case_id;
}
    ...
    for (i = 0, j = 0; i < mainkey_cnt; i++) {
        ...
        if (display_name[0] && activated == 1) {
			...
			if(!strcmp("compass",info[j].name))
				mag_case_id = j;
            if(!strcmp("motor",info[j].name))
				motor_case_id = j;
            if(!strcmp("mic",info[j].name))
				soundtest_case_id = j;
