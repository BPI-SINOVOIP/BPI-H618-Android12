...
#if defined(CONF_KERNEL_IOMMU) && !(defined _CONFIG_LINUX_5_4)
...
#else
    alloc_data.heap_id_mask = AW_ION_DMA_HEAP_MASK;
#endif
...
void sunxi_ion_alloc_pfree(void * pbuf)
{
    ...
    if (!flag) {
           loge("free fail! can not find %lu.\n", addr_vir);
           return;
    }
    pthread_mutex_lock(&g_ion_mutex_alloc);
    /* unmmap */
    ret = munmap((void*)pbuf, tmp->size);
    if (ret) {
        loge("munmap err, ret %d\n", ret);
    }
