...
#elif defined (_SUN8IW6P) || (_SUN50IW1P) || (_SUN50IW3P) \
	|| (_SUN50IW6P) || (_SUN50IW9P) || (_SUN8IW7P)
...
#include "df_view.h"
	...
	|| (_SUN8IW15P)  || (_SUN50IW6P) || (_SUN50IW9P) || (_SUN8IW7P) || (_SUN50IW10P)
...
#if defined (_CONFIG_LINUX_5_10) || (_CONFIG_LINUX_5_15)
...
#if defined (_CONFIG_LINUX_5_4) || (_CONFIG_LINUX_5_10) || (_CONFIG_LINUX_5_15)
static struct disp_layer_config2 layer_config;
#elif defined (_SUN8IW6P) || (_SUN50IW1P) || (_SUN50IW3P)
static disp_layer_config layer_config;
#elif defined (_SUN8IW5P) || (_SUN9IW1P)
static disp_layer_info info;
#elif defined (_SUN8IW15P) || (_SUN50IW6P) || (_SUN50IW9P) || (_SUN8IW7P) || (_SUN50IW10P)
...
int tp_track_init(void)
{
...
#if defined (_SUN8IW5P) || (_SUN9IW1P)
...
#if !defined (_CONFIG_LINUX_5_10) && !defined (_CONFIG_LINUX_5_15)
	/* 1. open ion device */
...
#if defined (_SUN8IW15P)  || (_SUN50IW6P) || (_SUN50IW9P) || (_SUN8IW7P) || (_SUN50IW10P)
	...
	if(ret) {
		db_error("######################ION_IOC_ALLOC err, ret %d\n", ret);
		goto out1;
	}
	/* 3. get dmabuf fd */
#if defined (_CONFIG_LINUX_5_4)
	fd_data.fd = alloc_data.fd;
#else
...
#if defined (_SUN8IW11P)
	fd = open("/dev/dma_heap/reserved", O_RDONLY | O_CLOEXEC);
#else
	fd = open("/dev/dma_heap/system-uncached", O_RDONLY | O_CLOEXEC);
#endif
	if(fd < 0) {
		db_error("####################err open /dev/dma_heap/system-uncached\n");
		return -1;
	}
	struct dma_heap_allocation_data heap_data;
	memset((void*)&heap_data, 0, sizeof(heap_data));
	alloc_data.len = screen_width * screen_height * 4;
	heap_data.len = alloc_data.len;
	heap_data.fd_flags = O_RDWR | O_CLOEXEC;

	ret = ioctl(fd, DMA_HEAP_IOCTL_ALLOC, &heap_data);
	if(ret) {
		db_error("######################DMA_HEAP_IOCTL_ALLOC err, ret %d\n", ret);
		goto out1;
	}

	fd_data.fd = heap_data.fd;
#endif
...
#if !defined(_CONFIG_LINUX_5_4) && !defined (_SUN8IW15P) \
	&& !defined(_CONFIG_LINUX_5_10) && !defined(_CONFIG_LINUX_5_15) \
	&& !defined (_SUN50IW10P) && !defined (_SUN50IW6P) \
	&& !defined (_SUN8IW7P) && !defined(_UNIVERSAL_IMPLEMENT) \
	&& !defined (_SUN50IW12P) && !defined(_SUN50IW9P)
...
#elif defined (_SUN8IW15P)  || (_SUN50IW6P) \
	|| (_SUN8IW7P) || (_SUN50IW10P) \
	|| (_SUN50IW12P) || (_SUN50IW9P) \
	|| (_SUN8IW11P) || (_SUN55IW3P)

	//设置图层参数

#if defined (_CONFIG_LINUX_5_4) || (_CONFIG_LINUX_5_10) || (_CONFIG_LINUX_5_15)
	memset(&layer_config, 0, sizeof(struct disp_layer_config2));
#else
