...
struct test_layer_info {
	...
	int mem_id;
#if defined (_CONFIG_LINUX_5_4) || (_CONFIG_LINUX_5_10) || (_CONFIG_LINUX_5_15)
	struct disp_layer_config2 layer_config;
#else
...
#if defined (_CONFIG_LINUX_5_4) || (_CONFIG_LINUX_5_10) || (_CONFIG_LINUX_5_15)
static struct disp_rectsz input_size;
static struct disp_rectsz display_size;
#else
...
static int read_frame(int fd)
{
	...
	if(a == 50) {
		...
		ret = fwrite(buffers[buf.index].start,buffers[buf.index].length, 1, frame_file);
		db_debug("fwrite ret is %d\n",ret);
#endif
		fclose(frame_file);
		a++;
...
static int disp_init(int x,int y,int width,int height)
{
	...
	for (screen_id = 0;screen_id < 3;screen_id++) {
		arg[0] = screen_id;
		output_type = (enum disp_output_type)ioctl(test_info.dispfh, DISP_GET_OUTPUT_TYPE, (void*)arg);
...
#if defined (_CONFIG_LINUX_5_4) || (_CONFIG_LINUX_5_10) || (_CONFIG_LINUX_5_15)
	test_info.layer_config.info.zorder = 17;
#else
...
#else
	buffer_fd = open("/dev/dma_heap/system-uncached", O_RDONLY | O_CLOEXEC);
	if (buffer_fd < 0) {
		db_error("can't open dma_heap\n",strerror(errno));
		goto err;
	}
#endif
...
*/
		...
		heap_data.len = (display_size.width * display_size.height * 1.5) * 4;
		heap_data.fd_flags = O_RDWR | O_CLOEXEC;
		if (ioctl(buffer_fd, DMA_HEAP_IOCTL_ALLOC, &heap_data) == -1) {
			db_error("DMA_HEAP_IOCTL_ALLOC error\n");
			goto err;
		}

		buffers[nbuffers].length = buf.m.planes[0].length;
		buffers[nbuffers].start = (void *)mmap(NULL, heap_data.len, \
		PROT_READ|PROT_WRITE, MAP_SHARED, heap_data.fd, 0);
		buffers[nbuffers].fd = heap_data.fd;
...
int camera_test_init(int x,int y,int width,int height)
{
	...
	// if (script_fetch("camera", "dev_no", &dev_no, 0) ||
	// 		(dev_no != 0 && dev_no != 8)) {
	// 	dev_no = 0;
	// }
	dev_no = 0;
