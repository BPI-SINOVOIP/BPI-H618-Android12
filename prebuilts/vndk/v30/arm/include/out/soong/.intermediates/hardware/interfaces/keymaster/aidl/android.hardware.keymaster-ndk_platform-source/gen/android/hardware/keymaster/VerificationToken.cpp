...
binder_status_t VerificationToken::readFromParcel(const AParcel* parcel) {
  ...
  _aidl_ret_status = ::ndk::AParcel_readParcelable(parcel, &timestamp);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
  ...
  _aidl_ret_status = AParcel_readInt32(parcel, reinterpret_cast<int32_t*>(&securityLevel));
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
...
binder_status_t VerificationToken::writeToParcel(AParcel* parcel) const {
  binder_status_t _aidl_ret_status;
  size_t _aidl_start_pos = AParcel_getDataPosition(parcel);
  _aidl_ret_status = AParcel_writeInt32(parcel, 0);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = AParcel_writeInt64(parcel, challenge);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = ::ndk::AParcel_writeParcelable(parcel, timestamp);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = AParcel_writeInt32(parcel, static_cast<int32_t>(securityLevel));
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
