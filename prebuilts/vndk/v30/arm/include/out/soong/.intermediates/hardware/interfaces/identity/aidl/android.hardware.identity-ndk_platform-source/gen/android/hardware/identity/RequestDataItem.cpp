...
binder_status_t RequestDataItem::readFromParcel(const AParcel* parcel) {
  ...
  _aidl_ret_status = AParcel_readInt64(parcel, &size);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
  ...
  _aidl_ret_status = ::ndk::AParcel_readVector(parcel, &accessControlProfileIds);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
...
binder_status_t RequestDataItem::writeToParcel(AParcel* parcel) const {
  ...
  _aidl_ret_status = AParcel_writeInt64(parcel, size);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = ::ndk::AParcel_writeVector(parcel, accessControlProfileIds);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
