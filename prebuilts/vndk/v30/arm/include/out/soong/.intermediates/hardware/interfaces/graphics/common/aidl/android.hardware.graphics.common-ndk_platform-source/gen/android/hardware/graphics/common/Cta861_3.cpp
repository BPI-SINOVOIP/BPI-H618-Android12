...
binder_status_t Cta861_3::readFromParcel(const AParcel* parcel) {
  ...
  _aidl_ret_status = AParcel_readFloat(parcel, &maxContentLightLevel);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
  ...
  _aidl_ret_status = AParcel_readFloat(parcel, &maxFrameAverageLightLevel);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
...
binder_status_t Cta861_3::writeToParcel(AParcel* parcel) const {
  binder_status_t _aidl_ret_status;
  size_t _aidl_start_pos = AParcel_getDataPosition(parcel);
  _aidl_ret_status = AParcel_writeInt32(parcel, 0);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = AParcel_writeFloat(parcel, maxContentLightLevel);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = AParcel_writeFloat(parcel, maxFrameAverageLightLevel);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
