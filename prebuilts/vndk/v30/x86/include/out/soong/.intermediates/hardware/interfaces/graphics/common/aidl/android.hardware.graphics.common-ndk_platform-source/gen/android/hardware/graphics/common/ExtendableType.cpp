...
binder_status_t ExtendableType::readFromParcel(const AParcel* parcel) {
  ...
  _aidl_ret_status = AParcel_readInt64(parcel, &value);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
...
binder_status_t ExtendableType::writeToParcel(AParcel* parcel) const {
  ...
  _aidl_ret_status = AParcel_writeInt64(parcel, value);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
