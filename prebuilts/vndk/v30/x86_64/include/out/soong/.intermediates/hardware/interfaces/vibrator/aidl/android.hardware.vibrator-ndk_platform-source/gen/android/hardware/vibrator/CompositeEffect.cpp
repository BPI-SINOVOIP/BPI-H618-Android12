...
binder_status_t CompositeEffect::readFromParcel(const AParcel* parcel) {
  ...
  _aidl_ret_status = AParcel_readInt32(parcel, reinterpret_cast<int32_t*>(&primitive));
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
...
binder_status_t CompositeEffect::writeToParcel(AParcel* parcel) const {
  binder_status_t _aidl_ret_status;
  size_t _aidl_start_pos = AParcel_getDataPosition(parcel);
  _aidl_ret_status = AParcel_writeInt32(parcel, 0);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = AParcel_writeInt32(parcel, delayMs);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = AParcel_writeInt32(parcel, static_cast<int32_t>(primitive));
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = AParcel_writeFloat(parcel, scale);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
