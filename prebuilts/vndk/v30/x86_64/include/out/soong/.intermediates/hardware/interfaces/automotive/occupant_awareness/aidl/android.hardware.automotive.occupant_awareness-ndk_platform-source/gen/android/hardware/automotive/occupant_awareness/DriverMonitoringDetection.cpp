...
binder_status_t DriverMonitoringDetection::readFromParcel(const AParcel* parcel) {
  ...
  _aidl_ret_status = AParcel_readByte(parcel, reinterpret_cast<int8_t*>(&confidenceScore));
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
  ...
  _aidl_ret_status = AParcel_readBool(parcel, &isLookingOnRoad);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
  ...
  _aidl_ret_status = AParcel_readInt64(parcel, &gazeDurationMillis);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
...
binder_status_t DriverMonitoringDetection::writeToParcel(AParcel* parcel) const {
  binder_status_t _aidl_ret_status;
  size_t _aidl_start_pos = AParcel_getDataPosition(parcel);
  _aidl_ret_status = AParcel_writeInt32(parcel, 0);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = AParcel_writeByte(parcel, static_cast<int8_t>(confidenceScore));
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = AParcel_writeBool(parcel, isLookingOnRoad);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = AParcel_writeInt64(parcel, gazeDurationMillis);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
