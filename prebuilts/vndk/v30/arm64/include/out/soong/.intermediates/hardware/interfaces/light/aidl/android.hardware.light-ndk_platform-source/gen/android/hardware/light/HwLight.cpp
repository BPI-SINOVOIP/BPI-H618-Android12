...
binder_status_t HwLight::readFromParcel(const AParcel* parcel) {
  ...
  _aidl_ret_status = AParcel_readInt32(parcel, &ordinal);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
  ...
  _aidl_ret_status = AParcel_readByte(parcel, reinterpret_cast<int8_t*>(&type));
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
...
binder_status_t HwLight::writeToParcel(AParcel* parcel) const {
  ...
  _aidl_ret_status = AParcel_writeInt32(parcel, ordinal);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = AParcel_writeByte(parcel, static_cast<int8_t>(type));
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
